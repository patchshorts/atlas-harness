...
describe('jsonrpc-agent keyless smoke', () => {
  it.each([
    ...
    const child = execa(process.execPath, [
      ...
      env: {
        ...
        ATSH_CWD: root,
        ATSH_SESSION_ROOT: join(root, '.sessions'),
        ...(envValue === undefined ? {} : { ATSH_MAX_TOKENS_AS_SUCCESS: envValue }),
      },
      timeout: 35_000,
      killSignal: 'SIGKILL',
      reject: false,
  ...
  it('rejects an invalid max-token success env value', async () => {
    const { exitCode, stdout, stderr } = await execa(process.execPath, [
      ...
      env: {
        DEEPSEEK_API_KEY: 'keyless-smoke-no-call',
        ATSH_MAX_TOKENS_AS_SUCCESS: 'sometimes',
      },
      stdin: 'ignore',
      timeout: 25_000,
      killSignal: 'SIGKILL',
    ...
    expect(stderr).toContain('failed to apply loader entry sdk-jsonrpc-server (@atlasai/atsh-sdk-jsonrpc-server)')
