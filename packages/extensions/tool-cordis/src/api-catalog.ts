...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  {
    key: 'accounting',
    summary: 'Token accounting ledger.',
    description: 'Token accounting ledger. Load as a plugin (`ctx.plugin(AccountingService, config)`); it registers as `ctx.accounting` (one ledger per context — loading a second throws, cordis\' standard duplicate-service behavior) and, when enabled, listens on the `llm/stream` waterfall (debits) and the `tools/execute` waterfall (budget-cap vetoes). The SQLite backend closes when the owning fiber unloads.',
    methods: [
      {
        signature: 'readonly db: DatabaseSync',
        description: 'Open ledger database handle (public for tests/inspection).',
        parameters: [],
      },
      {
        signature: 'grant(amount: number, reason: string, account: string = \'default\'): number',
        description: 'Credit an account: upsert its `accounts` row (`balance += amount`), append a `\'grant\'` ledger row, and emit `accounting/grant`.',
        parameters: [{ name: 'amount', description: 'token count to grant (positive).' }, { name: 'reason', description: 'why the grant was written (surfaced in the ledger row).' }, { name: 'account', description: 'account id (default `\'default\'`).' }],
        returns: 'the account balance after the grant.',
      },
      {
        signature: 'charge(account: string, amount: number, reason: string, meta: Record<string, unknown>): void',
        description: 'Charge an account: append a `\'debit\'` ledger row (negative amount) and emit `accounting/debit`. The public entry point for non-llm charges (e.g. judge replan cost); identical ledger semantics to the intercepted `llm/stream` debits.',
        parameters: [{ name: 'account', description: 'account id to charge.' }, { name: 'amount', description: 'token count to charge (positive; the ledger row is negative).' }, { name: 'reason', description: 'why the charge was written (surfaced in the ledger row).' }, { name: 'meta', description: 'free-form metadata for the ledger row.' }],
        throws: ['{TypeError} When amount is not a positive finite number.'],
      },
      {
        signature: 'getBalance(account: string = \'default\'): number',
        description: 'Read an account\'s current balance; `0` when no account row exists.',
        parameters: [{ name: 'account', description: 'account id (default `\'default\'`).' }],
        returns: 'the account\'s current balance.',
      },
      {
        signature: 'spendFor(account: string = \'default\'): number',
        description: 'Total debit spend for an account: the sum of |amount| of its debit rows.',
        parameters: [{ name: 'account', description: 'account id (default `\'default\'`).' }],
        returns: 'the account\'s total debit spend.',
      },
      {
        signature: 'listLedger(limit: number = 50): LedgerRow[]',
        description: 'Read the most recent ledger rows, newest first.',
        parameters: [{ name: 'limit', description: 'maximum rows to return (default 50).' }],
        returns: 'hydrated ledger rows.',
      },
      {
        signature: 'getStats(): AccountingStats',
        description: 'Snapshot of the ledger\'s table-level counters.',
        parameters: [],
  ...
  },
  {
    ...
    description: 'Agent service (`ctx.agents`): tracks live agents and carries the initiating Agent through one process-local asynchronous driver chain. Agent *creation* is provided by whichever plugin implements the AgentFactory (`@atlasai/atsh-agent-loop`), registered via setFactory.\n\nInitiator methods provide same-process causal attribution only. Ambient presence is neither liveness proof nor authorization; subjects and owners remain explicit, as does identity at worker, process, persistence, and wire boundaries. Returned Promise boundaries drain during teardown, except a nested lineage that starts an owning-fiber unload is excluded from its own drain.',
    methods: [
      {
  ...
  {
    key: 'bench',
    summary: 'The bench service seam\'s public surface.',
    description: 'The bench service seam\'s public surface. Registers `ctx.bench` and exposes the frozen per-run configuration for the deterministic C1..C5 correction classification and N-sessions-per-arm runner added by the the bench workstream tasks. Carries no behavior yet — it is the ADD-only scaffold later bench tasks build on; classification stays deterministic rules, never LLM judgment.',
    methods: [],
  },
  {
    key: 'budgetRouter',
    summary: 'The Fix 2 + Fix 9 seam: hard token budget enforced by accounting, per-stage model routing with cumulative cost conditioning, batch prompting for shared system prompts, and cost reported alongside pass rate.',
    description: 'The Fix 2 + Fix 9 seam: hard token budget enforced by accounting, per-stage model routing with cumulative cost conditioning, batch prompting for shared system prompts, and cost reported alongside pass rate.\n\nGolden-rule guarantee: the service holds no model-visible state and never writes to `options.messages`/`options.system` — route decisions rewrite only `provider`/`model` metadata on non-frozen requests, so the request prefix is byte-identical across stages (prefix-cache-friendly history).',
    methods: [
      {
        signature: 'budgetState(account: string): { spend: number; budget: number; remaining: number }',
        description: 'Read the spend/budget/remaining snapshot for one account.\n\nSpend comes from accounting\'s ledger via `ctx.get` (0 when accounting is not mounted); the budget comes from this service\'s config.',
        parameters: [{ name: 'account', description: 'the account id (`options.sessionId ?? \'default\'` for llm calls).' }],
        returns: 'spend, configured budget (0 when unconfigured), and remaining (`max(0, budget - spend)`).',
      },
      {
        signature: 'routeForStage(stage: string, cumulativeCost: number): StageRoute | undefined',
        description: 'Resolve the route for a stage at a cumulative cost.',
        parameters: [{ name: 'stage', description: 'the stage key (\'general\', ...).' }, { name: 'cumulativeCost', description: 'cumulative spend for the account/stage.' }],
        returns: 'the selected tier, or `undefined` when the stage has no ladder.',
        throws: ['{Error} When the service is disabled.'],
      },
      {
        signature: 'planBatches(requests: BatchRequest[]): BatchPlan',
        description: 'Plan batches by grouping requests that share the identical system prompt.',
        parameters: [{ name: 'requests', description: 'the requests to group; only `system` is read.' }],
        returns: 'the grouping plan plus estimated cache-read savings, or an empty plan when batch prompting is disabled.',
        throws: ['{Error} When the service is disabled.'],
      },
      {
        signature: 'price(usage: Parameters<typeof priceTokens>[0]): number',
        description: 'Price a usage record with the merged model cost (pinned DeepSeek constants overridden by `config.modelCost`).',
        parameters: [{ name: 'usage', description: 'token counts; cache-read/write tokens default 0.' }],
        returns: 'USD cost (never negative; 0 on empty usage).',
      },
      {
        signature: 'reportCostAndPassRate(entries: CostReportEntry[]): CostReport',
        description: 'Report cost alongside pass rate for a batch of entries.\n\nEntries may carry `cachedTokens`/`uncachedTokens` meta (accessed via `entry[\'cachedTokens\'] ?? 0` / `entry[\'uncachedTokens\'] ?? 0`), summed into the report for the cache-hit-rate.',
        parameters: [{ name: 'entries', description: 'one entry per finished call: id, USD cost, and pass flag.' }],
        returns: 'totals, passRate (0 when totalCount is 0), costPerPassUsd (0 when passCount is 0), and the cache-hit rate (0 when no tokens).',
      },
      {
        signature: 'cacheRatio(): number',
        description: 'The computed input/cacheRead price gap for the merged model cost.',
        parameters: [],
  ...
  {
    key: 'clientModules',
    summary: 'The web plugin table service: incremental `atsh.client` scan + wire composition + bundle route + index tap.',
  ...
  {
    key: 'contextDebt',
    summary: 'The context-debt management seam: retrieval over stuffing, fold-only compaction plans, positional placement.',
    description: 'The context-debt management seam: retrieval over stuffing, fold-only compaction plans, positional placement.\n\nStateless by design: the fold is a pure function and the JSONL log is the only state — this service never writes it. Golden rule: every call reads the committed snapshot (`session.events`, frozen) and returns derived values; the log stays byte-identical after any scan/plan/report call.',
    methods: [
      {
        signature: 'scan(session: Session): ContextDebtScan',
        description: 'Scan a session\'s committed events for context debt: a `\'stuffed\'` report when non-essential context (tool results, verbatim logs) exceeds the configured threshold, plus `\'positioned\'` reports for content outside the critical head/tail bands. Pure read: the log is never touched.',
        parameters: [{ name: 'session', description: 'the live session; its frozen committed snapshot is read.' }],
        returns: 'the scan result, reflecting the last committed seq.',
        throws: ['{Error} When the service is disabled (`\'context-debt disabled\'`).'],
      },
      {
        signature: 'plan(session: Session, budgetTokens: number): CompactionPlan',
        description: 'Produce a fold-only compaction plan for a session: `foldSummary` over the committed events → summary; the shadowed range is the stuffed span (the whole committed range when nothing is stuffed); shadowedSeqs and count come from the same span; `foldOnly` is ALWAYS `true`. The plan is a derived value — the log is never rewritten.',
        parameters: [{ name: 'session', description: 'the live session; its frozen committed snapshot is read.' }, { name: 'budgetTokens', description: 'the summary token budget for the fold.' }],
        returns: 'the fold-only plan; `isFoldOnly(plan, lastCommittedSeq)` holds.',
        throws: ['{Error} When the service is disabled (`\'context-debt disabled\'`).'],
      },
      {
        signature: 'report(plan: CompactionPlan): string',
        description: 'One-line report string for a plan, for observability and accounting.',
        parameters: [{ name: 'plan', description: 'the plan to describe.' }],
        returns: 'a single-line, deterministic description of the plan.',
      },
      {
        signature: 'reposition(plan: CompactionPlan): { head: string[]; tail: string[] }',
        description: 'Split a plan\'s summary into the critical head and tail bands per positional placement: leading lines fill the head band up to the configured head token budget, trailing lines fill the tail band up to the configured tail token budget, and the middle is dropped — critical context lands at head/tail. Assembly of the actual model-visible context from these bands is the caller\'s wiring.',
        parameters: [{ name: 'plan', description: 'the plan whose summary is split.' }],
  ...
  {
    key: 'coordination',
    summary: 'C2 orchestration service.',
    description: 'C2 orchestration service. Load as a plugin (`ctx.plugin(CoordinationService, config)`); it registers as `ctx.coordination` (one per context — loading a second throws, cordis\' standard duplicate-service behavior) and requires the subagent registry (`ctx.subagents`), which it consumes but never modifies. The SQLite backend closes when the owning fiber unloads.',
    methods: [
      {
        signature: 'readonly db: DatabaseSync',
        description: 'Open coordination database handle (public for tests/inspection).',
        parameters: [],
      },
      {
        signature: 'async spawnWorker(provider: string, request: SubagentStartRequest): Promise<string>',
        description: 'Spawn one subagent worker on the named provider and record its lifecycle in the `coord_workers` table. The row is inserted as `\'running\'` before the run settles; this resolves only after the run does, with the row flipped to `\'completed\'` (outcome = joined text output, or `JSON.stringify(structured)` when the run returned one) or `\'failed\'` (outcome = the error text). Emits `coordination/worker-started` after the row lands and `coordination/worker-completed` after the update lands.',
        parameters: [{ name: 'provider', description: 'a registered subagent provider name.' }, { name: 'request', description: 'the subagent delegation request (label, prompt, parent, signal).' }],
        returns: 'the worker record id.',
        throws: ['when the provider is not registered or coordination is disabled.'],
      },
      {
        signature: 'getWorker(id: string): WorkerRecord | undefined',
        description: 'Read one worker record by id.',
        parameters: [{ name: 'id', description: 'the worker record id returned by {@link spawnWorker}.' }],
        returns: 'the hydrated record, or `undefined` when no such row exists.',
      },
      {
        signature: 'listWorkers(status?: WorkerStatus): WorkerRecord[]',
        description: 'List worker records, optionally filtered by status, newest first.',
        parameters: [{ name: 'status', description: 'optional status filter.' }],
        returns: 'hydrated worker records.',
      },
      {
        signature: 'getStats(): CoordinationStats',
        description: 'Snapshot of the coordination tables\' counters.',
        parameters: [],
        returns: 'the worker/channel row counts.',
      },
      {
        signature: 'postState(channel: string, key: string, value: unknown): { revision: number }',
        description: 'Write one shared-state entry, bumping the per-(channel, key) revision monotonically (starting at 1) and upserting the row.',
        parameters: [{ name: 'channel', description: 'channel name.' }, { name: 'key', description: 'entry key within the channel.' }, { name: 'value', description: 'JSON-serializable value; `undefined` and functions reject.' }],
        returns: 'the new revision.',
        throws: ['{TypeError} when the value cannot be JSON-serialized.', 'when coordination is disabled.'],
      },
      {
        signature: 'getState(channel: string, key: string): SharedStateEntry | undefined',
        description: 'Read one shared-state entry.',
        parameters: [{ name: 'channel', description: 'channel name.' }, { name: 'key', description: 'entry key within the channel.' }],
        returns: 'the hydrated entry, or `undefined` when absent.',
      },
      {
        signature: 'listChannel(channel: string): SharedStateEntry[]',
        description: 'List one channel\'s entries ordered by revision.',
        parameters: [{ name: 'channel', description: 'channel name.' }],
  ...
  {
    key: 'factory',
    summary: 'The factory seam: plan-contract registry, deterministic BAR critic scoring, and immutable role objectives for the ralph tool.',
    description: 'The factory seam: plan-contract registry, deterministic BAR critic scoring, and immutable role objectives for the ralph tool.',
    methods: [
      {
        signature: 'registerPlanContract(planId: string, tasks: FactoryPlanTask[]): void',
        description: 'Register (or replace) the atomic-task contract for a plan id.',
        parameters: [{ name: 'planId', description: 'The plan id, a non-empty normalized string.' }, { name: 'tasks', description: 'The atomic tasks, at least one, each with normalized id/verb/object/verifies fields and unique ids, at most maxPlanTasks.' }],
        throws: ['{Error} When the service is disabled.', '{TypeError} When the plan id, the task array, or any task field is invalid, ids are duplicated, or the array exceeds maxPlanTasks.'],
      },
      {
        signature: 'getPlanContract(planId: string): FactoryPlanTask[] | undefined',
        description: 'Read the atomic-task contract for a plan id, as a fresh copy.',
        parameters: [{ name: 'planId', description: 'The plan id.' }],
        returns: 'The registered tasks (a fresh array), or undefined when the plan id is not registered.',
      },
      {
        signature: 'listPlanIds(): string[]',
        description: 'List all registered plan ids, in registration order.',
        parameters: [],
        returns: 'The registered plan ids.',
      },
      {
        signature: 'scoreTask(planId: string, submission: BarSubmission): BarVerdict',
        description: 'Deterministic BAR judge for a single task submission.\n\nPASS requires a non-empty normalized summary, a non-empty array of non-empty normalized evidence strings, and a non-empty array of non-empty normalized file paths. Any failed clause is reported with an exact reason string.',
        parameters: [{ name: 'planId', description: 'The plan id the submission targets.' }, { name: 'submission', description: 'The submitted work for one atomic task.' }],
        returns: 'The BAR verdict for the submission.',
        throws: ['{Error} When the plan id is unknown or the task id is not in the plan contract.'],
      },
      {
        signature: 'scoreContract(planId: string, submissions: BarSubmission[]): ContractScore',
        description: 'Aggregate BAR score over a whole plan contract.\n\nEach contract task is scored against the FIRST submission with a matching task id; tasks without a submission count as NOT_SUBMITTED.',
        parameters: [{ name: 'planId', description: 'The plan id to score.' }, { name: 'submissions', description: 'The submissions for the plan\'s tasks.' }],
        returns: 'The aggregate contract score.',
        throws: ['{Error} When the plan id is unknown.'],
      },
      {
        signature: 'buildRoleObjective(role: \'planner\', input: PlannerInput): string',
        description: 'Build the immutable planner objective for the ralph tool.',
        parameters: [{ name: 'role', description: 'the planner role selector.' }, { name: 'input', description: 'the planner input.' }],
        returns: 'the immutable planner objective string.',
      },
      {
        signature: 'buildRoleObjective(role: \'developer\', task: FactoryPlanTask, options?: { workspace?: string }): string',
        description: 'Build the immutable developer objective for one atomic task.',
        parameters: [{ name: 'role', description: 'the developer role selector.' }, { name: 'task', description: 'the atomic task to build the objective for.' }, { name: 'options', description: 'optional workspace override.' }],
        returns: 'the immutable developer objective string.',
      },
      {
        signature: 'buildRoleObjective(role: \'critic\', task: FactoryPlanTask, work: { summary: string; files: string[] }): string',
        description: 'Build the immutable critic objective reviewing work against one atomic task.',
        parameters: [{ name: 'role', description: 'the critic role selector.' }, { name: 'task', description: 'the atomic task the work claims to complete.' }, { name: 'work', description: 'the critic input: summary and changed files.' }],
  ...
  {
    key: 'factoryJudge',
    summary: 'The Pass 4 judge seam: unanimous three-panel verdicts with a bounded replan loop, votes in the event stream, and replan cost charged to accounting.',
    description: 'The Pass 4 judge seam: unanimous three-panel verdicts with a bounded replan loop, votes in the event stream, and replan cost charged to accounting.\n\nFresh-context guarantee: ballots are computed independently from the request + the role charter only; the service shares NO ballot state between roles. Golden rule: the judge never touches session log, message history, or projections — it operates on plan artifacts only.',
    methods: [
      {
        signature: 'judge(request: JudgeRequest): JudgeVerdict',
        description: 'Judge one request: compute the ballots (single → decomposition only; panel → decomposition + feasibility + verification), emit each ballot, and settle the verdict — PASS when every ballot is YES; REPLAN when any NO and replan budget remains (charge replanCost to accounting, emit judge/replan); ESCALATE when any NO and the budget is exhausted (no new charge). A plan PASS records the approval; PASS and ESCALATE clear the judgment\'s replan entry. Emits judge/verdict last.',
        parameters: [{ name: 'request', description: 'the judgment request; mode defaults \'panel\' and account defaults \'default\'.' }],
        returns: 'the settled verdict with every ballot and the replan budget.',
        throws: ['{Error} When the service is disabled.', '{TypeError} When the request is malformed: a non-empty normalized judgmentId/planId/revision is required, kind must be valid, \'plan\' requests require a non-empty tasks array, \'completion\' requests require a submission, and \'triage\' requests require a triage.'],
      },
      {
        signature: 'isPlanApproved(planId: string, revision?: string): boolean',
        description: 'Whether a plan revision was approved by a panel PASS.',
        parameters: [{ name: 'planId', description: 'the plan id.' }, { name: 'revision', description: 'when given, the approval must match this revision exactly; when omitted, any recorded approval counts.' }],
        returns: 'true when the plan (revision) was approved.',
      },
      {
        signature: 'replanState(judgmentId: string): JudgeReplanState',
        description: 'Read the replan budget for one judgment.',
        parameters: [{ name: 'judgmentId', description: 'the judgment id.' }],
        returns: 'the replans used and the configured maximum (0 for an unknown judgment id).',
      },
      {
        signature: 'resetJudgment(judgmentId: string): void',
        description: 'Reset a judgment\'s replan budget. Approvals persist.',
        parameters: [{ name: 'judgmentId', description: 'the judgment id to clear.' }],
      },
    ],
  },
  {
  ...
  {
    key: 'guardToolAllowlist',
    summary: 'The gate Service.',
    description: 'The gate Service. Constructing it registers the `tools/execute` wrapper; mounting the plugin is what arms the gate. The allowlist is a plain set for O(1) membership; the auditable event rides the declared `guard/allowlist-deny` event.',
    methods: [],
  },
  {
    key: 'invariants',
  ...
  {
    key: 'judgeGate',
    summary: 'The Pass 4 judge gate: enforces the three-panel panel at the three SAD moments.',
    description: 'The Pass 4 judge gate: enforces the three-panel panel at the three SAD moments. Resolves the panel via ctx.get(\'factoryJudge\') at INVOCATION time (never at load), so compositions without the factory packages still boot; invoking the gate without the panel fails closed with a clear error.',
    methods: [
      {
        signature: 'admitPlan(input: JudgeGateAdmissionInput): JudgeVerdict',
        description: 'Gate the plan admission moment: parse the presented plan, run the panel (kind \'plan\'), and fail closed unless every ballot is YES. A PASS records the approval (panel-side) and admits the plan for completion/exit votes.',
        parameters: [{ name: 'input', description: 'the presented plan: plan id, revision (presentation ordinal — a re-present after a NO bumps it), and the plan markdown.' }],
        returns: 'the settled PASS verdict.',
        throws: ['{JudgeGateError} When the plan does not pass (REPLAN/ESCALATE), when it has no parseable task rows, or when the panel is missing.', '{Error} When the gate is disabled.'],
      },
      {
        signature: 'checkCompletion(input: JudgeGateCompletionInput): JudgeVerdict',
        description: 'Gate the task completion moment: run the panel with kind \'completion\' against the ADMITTED plan\'s tasks. Fail closed when the plan was never admitted or the claim is not evidence-backed.',
        parameters: [{ name: 'input', description: 'the admitted plan id + revision and the claimed submission.' }],
        returns: 'the settled PASS verdict.',
        throws: ['{JudgeGateError} On NOT PASS, on an unknown or mismatched admission, or when the panel is missing.', '{Error} When the gate is disabled.'],
      },
      {
        signature: 'reviewExit(input: JudgeGateCompletionInput): JudgeVerdict',
        description: 'Gate the exit review moment: the final completion verdict over the admitted plan. Same verification contract as checkCompletion.',
        parameters: [{ name: 'input', description: 'the admitted plan id + revision and the claimed submission.' }],
        returns: 'the settled PASS verdict.',
        throws: ['{JudgeGateError} On NOT PASS, on an unknown or mismatched admission, or when the panel is missing.', '{Error} When the gate is disabled.'],
      },
  ...
  {
    key: 'kgraph',
    summary: 'Abstract OKR knowledge-graph service.',
    description: 'Abstract OKR knowledge-graph service. Subclass, implement the six operations, and load the subclass as a plugin — it registers as `ctx.kgraph` (one implementation per context; loading a second throws, cordis\' standard duplicate-service behavior).\n\nSemantics every implementation must honor:\n\n- upsertObjective creates a new objective when `id` is omitted and updates the matching row (name, description, status) when it is present.\n- listObjectives returns all objectives with their key results attached.\n- addKeyResult appends one key result to an existing objective.\n- addEvidence persists one evidence row pointing at a session-log event; rows are unique per `(sessionId, seq)` so replays never duplicate.\n- buildGraphFromSession derives objectives and evidence from one session log deterministically (no LLM judgment) and is idempotent per `(sessionId, seq)`.\n- getStats reports aggregate row counts and the number of sessions ingested.',
    methods: [
      {
        signature: 'abstract upsertObjective(input: UpsertObjectiveInput): Promise<Objective>',
        description: 'Create or update one objective.',
        parameters: [{ name: 'input', description: 'name plus optional id (update) and description.' }],
        returns: 'the stored objective with its (possibly empty) key result list.',
      },
      {
        signature: 'abstract listObjectives(): Promise<Objective[]>',
        description: 'List all objectives with their key results attached.',
        parameters: [],
        returns: 'objectives ordered by creation time, oldest first.',
      },
      {
        signature: 'abstract addKeyResult(input: AddKeyResultInput): Promise<KeyResult>',
        description: 'Append one key result to an objective.',
        parameters: [{ name: 'input', description: 'owning objective id plus the key result fields.' }],
        returns: 'the stored key result.',
      },
      {
        signature: 'abstract addEvidence(input: AddEvidenceInput): Promise<Evidence>',
        description: 'Persist one evidence row pointing at a session-log event.',
        parameters: [{ name: 'input', description: 'objective linkage plus the source event\'s identity and excerpt.' }],
        returns: 'the stored evidence row.',
      },
      {
        signature: 'abstract buildGraphFromSession(sessionId: string): Promise<GraphBuildResult>',
        description: 'Derive objectives and evidence from one session log.',
        parameters: [{ name: 'sessionId', description: 'session whose log is ingested.' }],
        returns: 'counts of objectives and evidence added by THIS call (replays add none).',
      },
      {
        signature: 'abstract getStats(): Promise<KGraphStats>',
        description: 'Report aggregate counts.',
        parameters: [],
  ...
  {
    key: 'laneGuard',
    summary: 'The Fix 12/6 lane-separation seam: channel-based instruction marking, the tool-call allowlist gate at the harness boundary, the PromptArmor-pattern sanitization pass, and taint-aware verification for the in-band class.',
    description: 'The Fix 12/6 lane-separation seam: channel-based instruction marking, the tool-call allowlist gate at the harness boundary, the PromptArmor-pattern sanitization pass, and taint-aware verification for the in-band class.\n\nState is config only — the gate is a pure function of the tool name and the passes are pure — which is what makes the service golden-rule safe.',
    methods: [
      {
        signature: 'guardReason(execution: { name: string; arguments?: unknown }): string | undefined',
        description: 'The tool-call allowlist gate: evaluate the execution against the policy and return the denial reason, or undefined when allowed. Emits \'lane/veto\' on denial. This is the exact function handed to `ctx.tools.guard`.',
        parameters: [{ name: 'execution', description: 'the tool execution: name plus optional parsed arguments.' }],
        returns: 'the allowlist denial reason, or undefined when the call is allowed (or the service is disabled — passive, and the gate is not registered either).',
      },
      {
        signature: 'evaluateGate(exec: { name: string; arguments?: unknown }): AllowDecision',
        description: 'Evaluate one tool call against the allowlist policy.',
        parameters: [{ name: 'exec', description: 'the tool execution: name plus optional parsed arguments.' }],
        returns: 'the allow decision (allowed, matched pattern, denial reason).',
        throws: ['when the service is disabled (\'lane-guard disabled\').'],
      },
      {
        signature: 'markChannels(messages: ReadonlyArray<{ role: string; content: string }>): ChanneledMessage[]',
        description: 'Derive channel markings for a message list without mutating it.',
        parameters: [{ name: 'messages', description: 'the message list (role + content per entry).' }],
        returns: 'a NEW array of channeled messages; the input array and every input object are never mutated (golden rule).',
        throws: ['when the service is disabled (\'lane-guard disabled\').'],
      },
      {
        signature: 'sanitize(text: string): SanitizeResult',
        description: 'Strip injected prompts from text (PromptArmor-pattern deterministic pass). Passive when disabled or when the sanitize pass is off — this pass is defense-in-depth, not a gate.',
        parameters: [{ name: 'text', description: 'the raw untrusted text (typically tool output).' }],
        returns: 'the sanitized text, strip hits, and total chars removed.',
      },
      {
        signature: 'verifyComposed(output: string, triples: FactTriple[]): TaintVerdict',
        description: 'Taint-aware verification of a composed output against extracted triples.',
        parameters: [{ name: 'output', description: 'the composed output text.' }, { name: 'triples', description: 'the fact triples extracted from CONTENT.' }],
        returns: 'the verdict: verified, traced count, and untraced clauses.',
        throws: ['when the service is disabled (\'lane-guard disabled\').'],
      },
      {
        signature: 'defend(payload: InjectionPayload): DefenseResult',
        description: 'The fixture seam: one deterministic call, zero LLM, that defends a single payload — sanitize first (resisted when any strip fired), then the allowlist gate (resisted when the directed tool is not allowed), else not resisted.',
        parameters: [{ name: 'payload', description: 'the injection payload (id, class, content, directedTool).' }],
        returns: 'the defense result: resisted, via (\'sanitize\' | \'allowlist\' | \'none\'), and detail (strip hit markers or the allowlist reason).',
        throws: ['when the service is disabled (\'lane-guard disabled\').'],
      },
    ],
  },
  ...
  {
    key: 'llmCache',
    summary: 'Deterministic + semantic LLM response cache.',
    description: 'Deterministic + semantic LLM response cache. Load as a plugin (`ctx.plugin(LlmCache, config)`); it registers as `ctx.llmCache` (one cache per context — loading a second throws, cordis\' standard duplicate-service behavior) and, when enabled, listens on the `llm/stream` waterfall. The SQLite backend closes when the owning fiber unloads.',
    methods: [
      {
        signature: 'readonly db: DatabaseSync',
        description: 'Open cache database handle (public for tests/inspection).',
        parameters: [],
      },
      {
        signature: 'embedder: Embedder = defaultEmbedder',
        description: 'Message → vector embedder for the semantic tier. Defaults to a deterministic local bag-of-words embedder; production deployments assign a real embedding model here.',
        parameters: [],
      },
      {
        signature: 'getStats(): CacheStats',
        description: 'Snapshot of the table-level counters: rows, total hits served, rows never served from cache, and `hits / (hits + misses)` (`0` when nothing has been served).',
        parameters: [],
  ...
  {
    key: 'llmRouter',
    summary: 'Capability-gated LLM router.',
    description: 'Capability-gated LLM router. Load as a plugin (`ctx.plugin(LlmRouter, config)`); it registers as `ctx.llmRouter` (one router per context — loading a second throws, cordis\' standard duplicate-service behavior) and, when enabled, listens on the `llm/stream` waterfall.',
    methods: [
      {
        signature: 'readonly db: DatabaseSync',
        description: 'Open call-log database handle (public for tests/inspection).',
        parameters: [],
      },
      {
        signature: 'routeFor(capability: string): RouterRoute | undefined',
        description: 'Return the configured route for a capability, if any.',
        parameters: [{ name: 'capability', description: 'capability name (`\'general\'`, `\'reasoning\'`, ...).' }],
        returns: 'the configured route, or `undefined` when unconfigured.',
      },
      {
        signature: 'listCalls(limit: number = 50): RouterCallRecord[]',
        description: 'Read the most recent logged calls, newest first.',
        parameters: [{ name: 'limit', description: 'maximum records to return (default 50).' }],
        returns: 'hydrated call records.',
      },
      {
        signature: 'countCalls(): number',
        description: 'Total number of logged calls.',
        parameters: [],
  ...
  {
    key: 'memoryStore',
    summary: 'Abstract semantic memory service.',
    description: 'Abstract semantic memory service. Subclass, implement recall, retain, and reflect, and load the subclass as a plugin — it registers as `ctx.memoryStore` (one implementation per context; loading a second throws, cordis\' standard duplicate-service behavior).\n\nSemantics every implementation must honor:\n\n- retain persists a record\'s FULL `content` verbatim and returns the stored record (id and timestamp assigned by the backend).\n- get returns the single record in a namespace whose content matches `key` exactly (byte-identical), not a top-k ranked subset — the exact-recovery path.\n- recall ranks stored content against the query and returns the best matches with a 0..1 relevance `score`, ordered best-first, honoring `namespace` and `limit`.\n- reflect reports totals, per-namespace counts, and the most recent records.',
    methods: [
      {
        signature: 'abstract recall(query: string, opts?: MemoryQueryOptions): Promise<MemoryRecallResult[]>',
        description: 'Rank stored memories against `query` and return the best matches.',
        parameters: [{ name: 'query', description: 'the model-facing search text; matched by token overlap (or embeddings, when the backend has them).' }, { name: 'opts', description: 'optional namespace scope and result limit.' }],
        returns: 'matches ordered by relevance (0..1 score), best first, capped at the smaller of `opts.limit` (default 10) and {@link RECALL_LIMIT_MAX}. Returns a ranked top-limit subset that may be incomplete; use {@link list} or {@link reflect} for the full store.',
      },
      {
        signature: 'abstract retain(record: MemoryRetainInput): Promise<MemoryRecord>',
        description: 'Persist one memory record verbatim.',
        parameters: [{ name: 'record', description: 'content to store, plus optional namespace and metadata.' }],
        returns: 'the stored record including the backend-assigned id and timestamp; rejects on a storage failure.',
      },
      {
        signature: 'abstract get(key: string, opts?: MemoryGetOptions): Promise<MemoryRecord | undefined>',
        description: 'Fetch the record whose content equals `key` byte-exactly within `opts.namespace` (or the default namespace when omitted). Unlike recall, this is the exact path: it returns the retained content verbatim, or `undefined` when no record matches exactly.',
        parameters: [{ name: 'key', description: 'the exact content string to match, byte for byte.' }, { name: 'opts', description: 'optional namespace scope.' }],
        returns: 'the single exact match, or `undefined` when none exists.',
      },
      {
        signature: 'abstract list(opts?: MemoryListOptions): Promise<MemoryRecord[]>',
        description: 'List ALL records from the store verbatim — the exhaustive counterpart to recall\'s ranked top-limit subset. Returns every stored record within `opts.namespace` (or the whole store when omitted), newest first, with NO result cap. Unlike recall, this is not a relevance-ranked sample: it gives the complete, byte-exact contents of the store so an agent can recover every fact it retained (the exact-recovery path).',
        parameters: [{ name: 'opts', description: 'optional namespace scope.' }],
        returns: 'all matching records, newest first, no limit applied.',
      },
      {
        signature: 'abstract reflect(opts?: MemoryReflectOptions): Promise<MemorySummary>',
        description: 'Summarize the store: total records, per-namespace counts, and the most recent records.',
        parameters: [{ name: 'opts', description: 'optional namespace scope and recent-count limit.' }],
  ...
  {
    key: 'observability',
    summary: 'The Fix 3/6/7 observability seam: an in-memory append-only ring buffer of structured events, predictive failure signals over the stream, the deterministic completion verifier (TNR gate), and the replay-with-patch debugging substrate.',
    description: 'The Fix 3/6/7 observability seam: an in-memory append-only ring buffer of structured events, predictive failure signals over the stream, the deterministic completion verifier (TNR gate), and the replay-with-patch debugging substrate. The buffer is a derived projection — the session log and message history are never touched (golden rule). The verifier methods are pure filters and stay available even when the service is disabled.',
    methods: [
      {
        signature: 'record(event: ObsEvent): void',
        description: 'Append one structured event to the ring buffer (a derived projection — the event object is copied, never retained by reference). When the buffer exceeds `windowSize`, the oldest event is dropped. Emits `observability/report` with the current report ONLY when the signal ids changed since the last emission.',
        parameters: [{ name: 'event', description: 'the structured event to append.' }],
        throws: ['{Error} When the service is disabled.'],
      },
      {
        signature: 'report(): SignalReport',
        description: 'The current signal report over the buffered events.',
        parameters: [],
        returns: 'the composed report: metrics, firing signals, and the verdict.',
        throws: ['{Error} When the service is disabled.'],
      },
      {
        signature: 'signalAt(index: number, event: ObsEvent): ReplayResult',
        description: 'Replay the buffered stream with one event patched, attributing signal changes to the patch — the debugging substrate: the error\'s surfacing step is not necessarily the cause step.',
        parameters: [{ name: 'index', description: 'the event index in the buffered stream to replace.' }, { name: 'event', description: 'the replacement event (the "patch").' }],
        returns: 'the before/after reports and the signal ids that changed.',
        throws: ['{Error} When the service is disabled.', '{RangeError} When `index` is out of bounds.'],
      },
      {
        signature: 'verifyCompletion(claim: CompletionClaim, checks: CompletionCheck[]): CompletionVerdict',
        description: 'Verify a completion claim against the checks. Completion is owned by the verifier — a self-declared completion without evidence is rejected. Works when disabled (the verifier is a pure filter, always available).',
        parameters: [{ name: 'claim', description: 'the completion claim (summary, evidence, selfDeclared).' }, { name: 'checks', description: 'the checks the evidence must satisfy.' }],
        returns: 'the verdict: PASS with empty reasons, or FAIL with every unsatisfied check cited.',
      },
      {
        signature: 'validateVerifier(fixtures: VerifierFixture[]): VerifierStats',
        description: 'Validate the verifier against fixtures, reporting TPR/TNR — the TNR gate: a verifier that accepts negative fixtures certifies garbage. Works when disabled.',
        parameters: [{ name: 'fixtures', description: 'the positive and negative fixtures with expected verdicts.' }],
        returns: 'the verifier stats (tpr, tnr, positives, negatives).',
      },
      {
        signature: 'stream(): readonly ObsEvent[]',
        description: 'A read-only view of the ring buffer. Callers must not mutate the returned array — the golden-rule tests assert the buffer never changes externally.',
        parameters: [],
        returns: 'the buffered events.',
      },
      {
        signature: 'reset(): void',
        description: 'Clear the ring buffer and the last-emitted signal ids.',
        parameters: [],
        throws: ['{Error} When the service is disabled.'],
      },
    ],
  ...
  {
    key: 'promptContextTrim',
    summary: 'Register `ctx.promptContextTrim` to trim the conversation surface verbatim after the cached core is fixed.',
    description: 'Register `ctx.promptContextTrim` to trim the conversation surface verbatim after the cached core is fixed.',
    methods: [
      {
        signature: 'trim( surface: readonly SurfaceLine[], overrides: Partial<TrimOptions> = {}, ): TrimResult',
        description: 'Trim the surface verbatim, honoring the configured budget and floor.\n\nWhen `enabled` is off, or the surface is already within the threshold, the surface is returned unchanged. Never mutates the input.',
        parameters: [{ name: 'surface', description: 'the rolling conversation surface, oldest first.' }, { name: 'overrides', description: 'per-call budget/floor/measure overrides.' }],
  ...
  {
    key: 'promptCorpus',
    summary: 'Register `ctx.promptCorpus` — the L2 RAG index over instruction corpora served on the MemoryStore seam.',
    description: 'Register `ctx.promptCorpus` — the L2 RAG index over instruction corpora served on the MemoryStore seam.\n\nConsumers receive the memory store via the declared `memoryStore` injection (load ordering + typed `ctx.memoryStore` access); the constructor runs synchronously, so `ctx.promptCorpus` is available as soon as this service\'s load settles.',
    methods: [
      {
        signature: 'async ingest(document: string, options: IngestOptions = {}): Promise<IngestResult>',
        description: 'Chunk a corpus document at semantic boundaries and retain each typed chunk into the memory store under the `prompt:<corpus>` namespace.\n\nEvery record carries structured metadata: `kind: \'prompt-corpus-chunk\'`, `corpus`, `scope`, `specificityRank`, `cacheStable`, plus the chunk\'s `index`, `heading`, and `depth`. This is the metadata later stages (recall routing, cross-encoder re-rank, budget allocation) act on, so the model / assembler never re-parses the corpus.',
        parameters: [{ name: 'document', description: 'the corpus text to index.' }, { name: 'options', description: 'corpus/scope labels plus an optional namespace override.' }],
        returns: 'counts + labels of the retained chunks.',
      },
      {
        signature: 'async reflect(opts?: MemoryReflectOptions): Promise<PromptCorpusSummary>',
        description: 'Report the prompt-corpus index size: total retained chunks and the count per corpus.',
        parameters: [{ name: 'opts', description: 'optional namespace scope to limit the summary.' }],
        returns: 'total chunk count and per-corpus breakdown.',
      },
      {
        signature: 'async recall(query: string, options: PromptRecallOptions = {}): Promise<PromptRecallResult[]>',
        description: 'Rank the prompt-corpus index against a query and return the best chunks.\n\nThe router delegates to the underlying MemoryStore hybrid recall (lexical overlap by default; embeddings when the backend is configured with one), so it works with zero embedding config out of the box. When a single corpus is named, recall runs scoped to that corpus\' `prompt:<corpus>` namespace; when no corpus is named it spans all prompt corpora. Results keep their 0..1 backend score and the corpus label each chunk was ingested under, so an assembler can order by germane corpus (skills for tool turns, workspace instructions for that dir) without re-parsing the store.',
        parameters: [{ name: 'query', description: 'the working-intent text to match chunks against.' }, { name: 'options', description: 'result limit and optional single-corpus scope.' }],
  ...
  {
    key: 'promptLume',
    summary: 'Register `ctx.promptLume`: a `system-prompt/assemble` listener stages the relevance-gated, provenance-labeled task-aligned region for that turn, and a pre-step flushes it as a self-superseding tail user/message via emitRegion.',
    description: 'Register `ctx.promptLume`: a `system-prompt/assemble` listener stages the relevance-gated, provenance-labeled task-aligned region for that turn, and a pre-step flushes it as a self-superseding tail user/message via emitRegion. The region is never committed to the byte-stable core section list, so the provider KV prefix cache read stays alive across turns.\n\nThe constructor registers the assemble listener, so once this service\'s load settles every subsequent assembly is relevance-gated (when the caller has primed a turn surface). With no primed turn or an empty intent the listener passes the assembly through unchanged — core only, and no region is staged for emission.',
    methods: [
      {
        signature: 'primeTurn(turn: TurnSurface): void',
        description: 'Record the current turn surface before the next assembly.\n\nThe assemble event carries no turn text, so the caller (the agent loop) primes the distilled working intent here; the listener consumes and clears it on the next assembly. An unprimed or empty intent yields core-only.',
        parameters: [{ name: 'turn', description: 'the distilled intent and optional corpus-affinity kind.' }],
      },
      {
        signature: 'costSummary(): ReturnType<CostSidecar[\'summary\']>',
        description: 'Cumulative cost-sidecar totals across every recorded assembly.\n\nFeeds the corrections-per-session benchmark without requiring a listener to have captured the per-call `prompt-lume/cost` events.',
        parameters: [],
        returns: 'the aggregated cost summary (calls, cache hits/misses, token and region totals) as a detached immutable record.',
      },
      {
        signature: 'emitRegion(session: Session): SessionEvent<\'user/message\'> | undefined',
        description: 'Emit the pending task-aligned region as a self-superseding user message appended AFTER the retained conversation history.\n\nThe message supersedes the previous region message (when one was emitted) via `surfaceOp: { op: \'replace\', start, end }` spanning exactly the prior region node, so exactly one region wall is ever on the surface and the byte-stable core prefix ahead of history stays provider-cache-stable. The first emission appends normally (no prior node to replace).',
        parameters: [{ name: 'session', description: 'the live agent session to append the region onto.' }],
        returns: 'the emitted user message, or undefined when no region is pending.',
      },
      {
        signature: 'hasPendingRegion(): boolean',
        description: 'True when the current turn\'s assembled region is still pending emission. Lets the driving pre-step decide whether an emission is merited.',
        parameters: [],
  ...
  {
    key: 'research',
    summary: 'External research service.',
    description: 'External research service. Load as a plugin (`ctx.plugin(ResearchService, config)`); it registers as `ctx.research` (one per context — loading a second throws, cordis\' standard duplicate-service behavior). The service is stateless apart from the in-memory counters returned by `getStats()`, so it owns no resources to dispose.',
    methods: [
      {
        signature: 'async searchPosts(query: string, options: { limit?: number } = {}): Promise<ResearchPost[]>',
        description: 'Search social posts through the `xurl` CLI.',
        parameters: [{ name: 'query', description: 'the search query.' }, { name: 'options', description: 'optional result cap; defaults to the config `maxResults`.' }],
        returns: 'the mapped posts; `[]` when disabled, the xurl binary is missing (ENOENT), or the lookup fails (failures are counted in `getStats()`).',
      },
      {
        signature: 'async searchPapers(query: string, options: { maxResults?: number } = {}): Promise<ResearchPaper[]>',
        description: 'Search arXiv papers through the Atom API.',
        parameters: [{ name: 'query', description: 'the search query (sent as `all:<query>`).' }, { name: 'options', description: 'optional result cap; defaults to the config `maxResults`.' }],
        returns: 'the parsed papers; `[]` when disabled or the lookup fails (failures are counted in `getStats()`).',
      },
      {
        signature: 'async fetchPaper(id: string): Promise<ResearchPaper | null>',
        description: 'Fetch one arXiv paper by id through the Atom API.',
        parameters: [{ name: 'id', description: 'the arXiv id (with or without the `https://arxiv.org/abs/` prefix).' }],
        returns: 'the parsed paper, or `null` when disabled, not found, or the lookup fails.',
      },
      {
        signature: 'getStats(): ResearchStats',
        description: 'Snapshot of the per-source counters.',
        parameters: [],
  ...
  {
    key: 'routerTrainer',
    summary: 'Collects one `router/call-logged` record per routed call as a training sample, in arrival order, plus `CorrectionRecord` rewards threaded onto corrected samples.',
    description: 'Collects one `router/call-logged` record per routed call as a training sample, in arrival order, plus `CorrectionRecord` rewards threaded onto corrected samples. A downstream trainer drains records (or the optional JSONL output sink) at its own cadence; reset starts a fresh queue.',
    methods: [
      {
        signature: 'count(): number',
        description: 'Number of samples collected since the last reset.',
        parameters: [],
        returns: 'the sample count.',
      },
      {
        signature: 'records(): readonly TrainingSample[]',
        description: 'Samples collected so far, in arrival order.',
        parameters: [],
        returns: 'the collected training samples.',
      },
      {
        signature: 'reset(): void',
        description: 'Drop all collected samples (does not truncate an `outputPath` file).',
        parameters: [],
      },
      {
        signature: 'corrections(): readonly CorrectionRecord[]',
        description: 'Corrections threaded into the trainer as rewards so far, in arrival order.',
        parameters: [],
        returns: 'the consumed correction records.',
      },
      {
        signature: 'rewards(): readonly TrainingSample[]',
        description: 'Samples whose call received a threaded correction reward, in arrival order. A sample\'s CorrectionRecord reward is set only after a correction referenced its call id, so this proves which corrections were consumed as rewards.',
        parameters: [],
        returns: 'the rewarded training samples.',
      },
      {
        signature: 'recordCorrection(correction: CorrectionRecord): void',
        description: 'Thread one correction into the trainer\'s sample log as a reward signal: append it to the optional JSONL sink (the same `outputPath` log as the samples) and, when it references a recorded call, attach it to that sample as its reward. A correction referencing a call the trainer has not seen is still recorded, so no correction is lost.',
        parameters: [{ name: 'correction', description: 'the correction to consume as a reward.' }],
      },
      {
        signature: 'onCall(record: TrainingSample): void',
        description: 'Append one routed call record to the sample queue and the optional JSONL sink.',
        parameters: [{ name: 'record', description: 'the routed call record to append.' }],
      },
    ],
  },
  {
  ...
  {
    key: 'shellEnv',
    summary: 'Registry (`ctx.shellEnv`) for trusted, per-execution `ATSH_*` variables.',
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Build the trusted `ATSH_*` snapshot for one shell tool execution.',
...
export const EVENT_API: readonly EventApiEntry[] = [
  {
    name: 'accounting/debit',
    mode: 'emit',
    signature: '\'accounting/debit\'(record: AccountingDebitRecord): void',
    summary: 'A debit ledger row was written for a completed `llm/stream` call.',
    description: 'A debit ledger row was written for a completed `llm/stream` call. Fires once per stream that reported usage, after the stream finished.',
    parameters: [{ name: 'record', description: 'the debit row: account, token amount, reason, and balance after.' }],
  },
  {
    name: 'accounting/grant',
    mode: 'emit',
    signature: '\'accounting/grant\'(record: AccountingGrantRecord): void',
    summary: 'A credit grant ledger row was written (initial credits or an explicit `grant()` call).',
    description: 'A credit grant ledger row was written (initial credits or an explicit `grant()` call). Fires after the row is committed.',
    parameters: [{ name: 'record', description: 'the grant row: account, token amount, reason, and balance after.' }],
  },
  {
  ...
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.agent - the newly registered agent with its live session and completed setup. Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/disposed',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.agent - the exact agent removed from the registry. Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/error',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.error - the failure, verbatim. Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.turn - the owning turn. Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.message - the discarded message. Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.message - the inserted message. Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/pre-step',
    mode: 'waterfall',
    ...
    parameters: [{ name: 'payload', description: '.signal - the current turn\'s cancellation signal. Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/request',
    mode: 'waterfall',
    ...
    parameters: [{ name: 'payload', description: '.signal - the current turn\'s explicit abort signal. Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.signal - the turn abort signal. Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.source - why the session started (fresh startup, resume, …). Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/status',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.status - the status just entered (the transition\'s destination). Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
  ...
  },
  {
    ...
    description: 'Ask composed answerers for one decision. Return an outcome to claim the request or call `next()`; failure yields the fail-closed default. Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.',
    parameters: [{ name: 'req', description: 'the pending decision (agent, tool identity, reason, signal).' }],
  },
  {
    name: 'bench/contract-checklist',
    mode: 'emit',
    signature: '\'bench/contract-checklist\'(event: { clauseCount: number source: string checklist: string ts: number }): void',
    summary: 'Diagnostic emitted when the contract pre-flight reads the contract source on the first edit.',
    description: 'Diagnostic emitted when the contract pre-flight reads the contract source on the first edit. Carries the extracted clause count and the rendered checklist. Never model-visible.',
    parameters: [{ name: 'event', description: '.ts - emission timestamp (epoch ms).' }],
  },
  {
    name: 'bench/contract-coverage',
    mode: 'emit',
    signature: '\'bench/contract-coverage\'(event: { uncoveredCount: number report: string ts: number }): void',
    summary: 'Diagnostic emitted when the session is closing: name the clauses the visible tests do NOT cover (the §4.2 gap the verifier still enforces).',
    description: 'Diagnostic emitted when the session is closing: name the clauses the visible tests do NOT cover (the §4.2 gap the verifier still enforces).',
    parameters: [{ name: 'event', description: '.ts - emission timestamp (epoch ms).' }],
  },
  {
    name: 'bench/failure-memory-pin',
    mode: 'emit',
    signature: '\'bench/failure-memory-pin\'(event: { tool: string failure?: string records: number ts: number directive?: string }): void',
    summary: 'Diagnostic emitted when the failure-signature memory records a new failure signature or vetoes a repeat of a recorded same-signature call.',
    description: 'Diagnostic emitted when the failure-signature memory records a new failure signature or vetoes a repeat of a recorded same-signature call. Carries the tool, the failure, and the record count. Never model-visible.',
    parameters: [{ name: 'event', description: '.directive - the pivot directive the veto embeds.' }],
  },
  {
    name: 'bench/guard-veto',
    mode: 'emit',
    signature: '\'bench/guard-veto\'( event: { reason: GuardReason count: number ceiling: number ts: number directive?: string }, ): void',
    summary: 'Diagnostic emitted when a guard stops the session.',
    description: 'Diagnostic emitted when a guard stops the session. Carries the reason, observed call count, ceiling, and the summarize & submit directive the vetoed tool result embeds — the fallback a downstream log consumer can observe.',
    parameters: [{ name: 'event', description: '.directive - the summarize-and-submit fallback directive.' }],
  },
  {
    name: 'bench/mistake-ledger-pin',
    mode: 'emit',
    signature: '\'bench/mistake-ledger-pin\'( event: { tool: string failure?: string records: number ts: number directive?: string }, ): void',
    summary: 'Diagnostic emitted when the mistake ledger pins a record or vetoes a re-try of a ledger-listed tool.',
    description: 'Diagnostic emitted when the mistake ledger pins a record or vetoes a re-try of a ledger-listed tool. Carries the tool, the pinned record, and the pivot directive the vetoed tool result embeds. Never model-visible.',
    parameters: [{ name: 'event', description: '.directive - the pivot directive the vetoed result embeds.' }],
  },
  {
    name: 'bench/overnight-patch',
    mode: 'emit',
    signature: '\'bench/overnight-patch\'( event: { artifactPath: string clusters: number ts: number }, ): void',
    summary: 'Diagnostic emitted when the overnight-patch coordinator drafts and writes the artifact.',
    description: 'Diagnostic emitted when the overnight-patch coordinator drafts and writes the artifact. Carries the artifact path + recurring-cluster count. Never model-visible.',
    parameters: [{ name: 'event', description: '.ts - emission timestamp (epoch ms).' }],
  },
  {
    name: 'bench/retry-judge-veto',
    mode: 'emit',
    signature: '\'bench/retry-judge-veto\'( event: { tool: string previousFailures: number maxConsecutive: number ts: number directive?: string }, ): void',
    summary: 'Diagnostic emitted when the retry judge vetoes a tool retry.',
    description: 'Diagnostic emitted when the retry judge vetoes a tool retry. Carries the tool, the observed consecutive failures, the ceiling, and the pivot directive the vetoed tool result embeds. Never model-visible.',
    parameters: [{ name: 'event', description: '.directive - the pivot directive the vetoed result embeds.' }],
  },
  {
    name: 'bench/tripwire-alarm',
    mode: 'emit',
    signature: '\'bench/tripwire-alarm\'( event: { predicted: number actual: number ratio: number ts: number directive?: string }, ): void',
    summary: 'Diagnostic emitted when the tripwire stops the session.',
    description: 'Diagnostic emitted when the tripwire stops the session. Carries the predicted count, observed actual count, trip ratio, and the checkpoint-and-replan directive the vetoed tool result embeds — the fallback a downstream log consumer can observe.',
    parameters: [{ name: 'event', description: '.directive - the checkpoint-and-replan fallback directive.' }],
  },
  {
    name: 'bench/verify-required',
    mode: 'emit',
    signature: '\'bench/verify-required\'( event: { matched: string[] forcedPurpose: string ts: number }, ): void',
    summary: 'Diagnostic emitted when the verify-required bit is armed for a session: carries the matched patterns and the forced-purpose value.',
    description: 'Diagnostic emitted when the verify-required bit is armed for a session: carries the matched patterns and the forced-purpose value. Never model-visible.',
    parameters: [{ name: 'event', description: '.ts - emission timestamp (epoch ms).' }],
  },
  {
    name: 'budget/route',
    mode: 'emit',
    signature: '\'budget/route\'(record: BudgetRouteDecision): void',
    summary: 'An `llm/stream` call settled with its cost-conditioned route decision.',
    description: 'An `llm/stream` call settled with its cost-conditioned route decision.',
    parameters: [{ name: 'record', description: 'the decision: account, stage, cumulative cost, requested and resolved provider/model, and the route state.' }],
  },
  {
    name: 'budget/veto',
    mode: 'emit',
    signature: '\'budget/veto\'(record: BudgetVetoRecord): void',
    summary: 'A hard budget veto stopped an `llm/stream` call: the account\'s spend reached its configured budget, so the call is rejected with `BUDGET_EXCEEDED` before any adapter runs.',
    description: 'A hard budget veto stopped an `llm/stream` call: the account\'s spend reached its configured budget, so the call is rejected with `BUDGET_EXCEEDED` before any adapter runs.',
    parameters: [{ name: 'record', description: 'the veto: account, spend, budget, stage, and timestamp.' }],
  },
  {
    name: 'cache/hit',
    mode: 'emit',
    signature: '\'cache/hit\'(record: CacheHitRecord): void',
    summary: 'A cached completion was served from the `llm_cache` table without an upstream call.',
    description: 'A cached completion was served from the `llm_cache` table without an upstream call. Fires once per hit, carrying the stored key and the tier that matched.',
    parameters: [{ name: 'record', description: 'the hit: stored key, timestamp, and matching tier.' }],
  },
  {
    name: 'cache/miss',
    mode: 'emit',
    signature: '\'cache/miss\'(record: CacheMissRecord): void',
    summary: 'A call missed the cache and was forwarded upstream; the completion will be stored on success.',
    description: 'A call missed the cache and was forwarded upstream; the completion will be stored on success. Fires once per miss, before the stream is consumed.',
    parameters: [{ name: 'record', description: 'the miss: request key and timestamp.' }],
  },
  {
  ...
  {
    name: 'context-debt/plan',
    mode: 'emit',
    signature: '\'context-debt/plan\'(plan: CompactionPlan): void',
    summary: 'A fold-only compaction plan was produced for a session.',
    description: 'A fold-only compaction plan was produced for a session.',
    parameters: [{ name: 'plan', description: 'the produced plan; foldOnly is always `true`.' }],
  },
  {
    name: 'context-debt/scan',
    mode: 'emit',
    signature: '\'context-debt/scan\'(scan: ContextDebtScan): void',
    summary: 'A context-debt scan completed over a session\'s committed events.',
    description: 'A context-debt scan completed over a session\'s committed events.',
    parameters: [{ name: 'scan', description: 'the scan result (reports + foldSeq).' }],
  },
  {
    name: 'coordination/worker-completed',
    mode: 'emit',
    signature: '\'coordination/worker-completed\'(payload: { workerId: string provider: string status: WorkerStatus }): void',
    summary: 'A subagent worker settled and its `coord_workers` row was updated to `\'completed\'` or `\'failed\'` with the outcome text.',
    description: 'A subagent worker settled and its `coord_workers` row was updated to `\'completed\'` or `\'failed\'` with the outcome text. Fires once per spawn, after the row lands.',
    parameters: [{ name: 'payload', description: '.status - `\'completed\'` or `\'failed\'`.' }],
  },
  {
    name: 'coordination/worker-started',
    mode: 'emit',
    signature: '\'coordination/worker-started\'(payload: { workerId: string; provider: string }): void',
    summary: 'A subagent worker was published by the subagent registry and recorded as `\'running\'` in the `coord_workers` table.',
    description: 'A subagent worker was published by the subagent registry and recorded as `\'running\'` in the `coord_workers` table. Fires once per spawn, after the row lands.',
    parameters: [{ name: 'payload', description: '.provider - the provider key that spawned it.' }],
  },
  {
  ...
  {
    name: 'factory/contract-registered',
    mode: 'emit',
    signature: '\'factory/contract-registered\'(payload: { planId: string; count: number }): void',
    summary: 'A factory plan contract was registered or replaced.',
    description: 'A factory plan contract was registered or replaced.',
    parameters: [{ name: 'payload', description: 'The contract-registration event payload.' }],
  },
  {
    name: 'fs/edit-intent',
    mode: 'waterfall',
  ...
  },
  {
    name: 'goal/changed',
    mode: 'emit',
    signature: '\'goal/changed\'(this: import(\'@atlasai/atsh-scope\').Scoped<Agent>, payload: { agent: Agent; change: GoalChanged }): void',
    summary: 'Goal mutation accepted by one live agent.',
    description: 'Goal mutation accepted by one live agent. The matching `goal/change` session event has already committed. Listener failures are contained. Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.',
    parameters: [{ name: 'payload', description: '.change - fresh current projection or clear tombstone.' }],
  },
  {
    name: 'guard/allowlist-deny',
    mode: 'emit',
    signature: '\'guard/allowlist-deny\'(data: ToolAllowlistDenyEvent): void',
    summary: 'Emitted when an out-of-list tool call is denied at the gate.',
    description: 'Emitted when an out-of-list tool call is denied at the gate.',
    parameters: [{ name: 'data', description: 'the denial payload: the rejected tool name and the calling agent (when the call has one).' }],
  },
  {
    name: 'judge/ballot',
    mode: 'emit',
    signature: '\'judge/ballot\'(vote: JudgeVote): void',
    summary: 'One judge ballot (per panel role) was cast for a judgment round.',
    description: 'One judge ballot (per panel role) was cast for a judgment round.',
    parameters: [{ name: 'vote', description: 'The role\'s ballot: role, vote, and exact reasons.' }],
  },
  {
    name: 'judge/replan',
    mode: 'emit',
    signature: '\'judge/replan\'(payload: { judgmentId: string; planId: string; kind: JudgeKind; round: number; cost: number }): void',
    summary: 'A replan was granted: any NO ballot while replan budget remains.',
    description: 'A replan was granted: any NO ballot while replan budget remains.',
    parameters: [{ name: 'payload', description: 'judgment id, plan id, kind, round, and charged cost.' }],
  },
  {
    name: 'judge/verdict',
    mode: 'emit',
    signature: '\'judge/verdict\'(verdict: JudgeVerdict): void',
    summary: 'A judgment round settled with an aggregate verdict.',
    description: 'A judgment round settled with an aggregate verdict.',
    parameters: [{ name: 'verdict', description: 'The settled verdict with every ballot of the round.' }],
  },
  {
    name: 'lane/veto',
    mode: 'emit',
    signature: '\'lane/veto\'(record: LaneVetoRecord): void',
    summary: 'A tool call was denied by the lane-guard allowlist gate.',
    description: 'A tool call was denied by the lane-guard allowlist gate.',
    parameters: [{ name: 'record', description: 'the veto: the denied tool name, the allowlist reason, and the epoch-ms timestamp.' }],
  },
  {
  ...
  },
  {
    name: 'llm/cost',
    mode: 'emit',
    signature: '\'llm/cost\'(event: LlmCostEvent): void',
    summary: 'Emitted after one usage chunk is recorded into the per-session cost ledger.',
    description: 'Emitted after one usage chunk is recorded into the per-session cost ledger. Carries the session total after the record. Listeners observe and never veto; the event is advisory accounting state.',
    parameters: [{ name: 'event', description: 'the recorded chunk and its session total.' }],
  },
  {
    name: 'llm/stream',
    mode: 'waterfall',
  ...
  },
  {
    ...
    mode: 'emit',
    signature: '\'observability/report\'(report: SignalReport): void',
    summary: 'Emitted when the observability signal report changes (the signal ids differ from the last emission).',
    description: 'Emitted when the observability signal report changes (the signal ids differ from the last emission).',
    parameters: [{ name: 'report', description: 'the current signal report (metrics, signals, verdict).' }],
  },
  {
    name: 'prompt-lume/cost',
    mode: 'emit',
    signature: '\'prompt-lume/cost\'(this: Context, record: import(\'./cost.ts\').PromptLumeCostRecord): void',
    summary: 'Fired after each prompt-lume assembly with a primed turn.',
    description: 'Fired after each prompt-lume assembly with a primed turn.\n\nCarries the per-call cost record (core/region/input heuristic tokens, cache-hit vs miss, budget vs actual region bytes) and lets cost consumers feed the corrections-per-session benchmark. The cumulative totals are also readable via PromptLumeService.costSummary.',
    parameters: [{ name: 'record', description: 'the detached immutable per-call cost record.' }],
  },
  {
    name: 'research/papers-searched',
    mode: 'emit',
    signature: '\'research/papers-searched\'(payload: { query: string; count: number }): void',
    summary: 'A paper search succeeded and returned `count` papers.',
    description: 'A paper search succeeded and returned `count` papers.',
    parameters: [{ name: 'payload', description: '.count - number of returned papers.' }],
  },
  {
    name: 'research/posts-searched',
    mode: 'emit',
    signature: '\'research/posts-searched\'(payload: { query: string; count: number }): void',
    summary: 'A social-post search succeeded and returned `count` posts.',
    description: 'A social-post search succeeded and returned `count` posts.',
    parameters: [{ name: 'payload', description: '.count - number of returned posts.' }],
  },
  {
    name: 'router/call-logged',
    mode: 'emit',
    signature: '\'router/call-logged\'(record: RouterCallRecord): void',
    summary: 'A routed model call completed and was appended to the call log.',
    description: 'A routed model call completed and was appended to the call log. Fires once per call, after the row lands, carrying the same record that was inserted.',
    parameters: [{ name: 'record', description: 'the persisted call-log row.' }],
  },
  {
  ...
  },
  {
    ...
    description: 'Emitted once when an announced session leaves the store, including publication rollback, but never for an entry whose creation announcement did not begin. Listener failures are logged and contained. Scope-filtered dispatch (`@atlasai/atsh-scope`) reuses the owner scope.',
  ...
  },
  {
    ...
    description: 'Awaited parallel durability checkpoint: every listener runs and the caller awaits all of them, with no waterfall veto. Scope-filtered dispatch (`@atlasai/atsh-scope`) reuses the session\'s owner scope.',
    parameters: [{ name: 'session', description: 'the session whose buffered events must reach durable storage.' }],
  },
  {
  ...
  },
  {
    ...
    description: 'Observe the frozen, lossless-JSON final outcome. Listener failures are contained. Scope-filtered dispatch (`@atlasai/atsh-scope`): keyed by `exec.agent`.',
...
export const TYPE_API: readonly TypeApiEntry[] = [
  {
    name: 'AccountingDebitRecord',
    declaration: 'export interface AccountingDebitRecord {\n    account: string;\n    amount: number;\n    reason: string;\n    balanceAfter: number;\n}',
  },
  {
    name: 'AccountingGrantRecord',
    declaration: 'export interface AccountingGrantRecord {\n    account: string;\n    amount: number;\n    reason: string;\n    balanceAfter: number;\n}',
  },
  {
    name: 'AccountingStats',
    declaration: 'export interface AccountingStats {\n    grants: number;\n    debits: number;\n    accounts: number;\n}',
  },
  {
  ...
  {
    name: 'AddEvidenceInput',
    declaration: 'export interface AddEvidenceInput {\n    objectiveId: string;\n    krId?: string;\n    sessionId: string;\n    seq: number;\n    eventType: string;\n    excerpt: string;\n    time: number;\n}',
  },
  {
    name: 'AddKeyResultInput',
    declaration: 'export interface AddKeyResultInput {\n    objectiveId: string;\n    name: string;\n    metric?: string;\n    target?: string;\n}',
  },
  {
    name: 'Agent',
  ...
  {
    name: 'AllowDecision',
    declaration: 'export interface AllowDecision {\n    allowed: boolean;\n    reason?: string;\n    matched: string | null;\n}',
  },
  {
  ...
  {
    name: 'BarStatus',
    declaration: 'export type BarStatus = \'PASS\' | \'FAIL\' | \'NOT_SUBMITTED\';',
  },
  {
    name: 'BarSubmission',
    declaration: 'export interface BarSubmission {\n    taskId: string;\n    summary: string;\n    evidence: string[];\n    files: string[];\n    blockers?: string[];\n}',
  },
  {
    name: 'BarVerdict',
    declaration: 'export interface BarVerdict {\n    taskId: string;\n    status: BarStatus;\n    passedChecks: string[];\n    reasons: string[];\n}',
  },
  {
  ...
  {
    name: 'BatchGroup',
    declaration: 'export interface BatchGroup {\n    system: string | null;\n    requestIndexes: number[];\n    sharedPrefixTokens: number;\n}',
  },
  {
    name: 'BatchPlan',
    declaration: 'export interface BatchPlan {\n    groups: BatchGroup[];\n    cacheReadSavingsTokens: number;\n}',
  },
  {
    name: 'BatchRequest',
    declaration: 'export interface BatchRequest {\n    system?: string;\n    messages: unknown[];\n}',
  },
  {
    name: 'Branded',
  ...
  {
    name: 'BudgetRouteDecision',
    declaration: 'export interface BudgetRouteDecision {\n    account: string;\n    stage: string;\n    cumulativeCost: number;\n    requestedProvider: string;\n    requestedModel: string;\n    resolvedProvider: string;\n    resolvedModel: string;\n    routeState: BudgetRouteState;\n}',
  },
  {
    name: 'BudgetRouteState',
    declaration: 'export type BudgetRouteState = \'none\' | \'matched\' | \'rewritten\' | \'advisory\';',
  },
  {
    name: 'BudgetVetoRecord',
    declaration: 'export interface BudgetVetoRecord {\n    account: string;\n    spend: number;\n    budget: number;\n    stage: string;\n    ts: number;\n}',
  },
  {
    name: 'CacheHitRecord',
    declaration: 'export interface CacheHitRecord {\n    key: string;\n    ts: number;\n    source: CacheSource;\n}',
  },
  {
    name: 'CacheMissRecord',
    declaration: 'export interface CacheMissRecord {\n    key: string;\n    ts: number;\n}',
  },
  {
    name: 'CacheSource',
    declaration: 'export type CacheSource = \'exact\' | \'semantic\';',
  },
  {
    name: 'CacheStats',
    declaration: 'export interface CacheStats {\n    entries: number;\n    hits: number;\n    misses: number;\n    hitRate: number;\n}',
  },
  {
    name: 'CallStatus',
    declaration: 'export type CallStatus = \'ok\' | \'error\';',
  },
  {
    name: 'CancelOptions',
  ...
  {
    name: 'Channel',
    declaration: 'export type Channel = \'system\' | \'user\' | \'tool\' | \'assistant\';',
  },
  {
    name: 'ChanneledMessage',
    declaration: 'export interface ChanneledMessage {\n    role: string;\n    content: string;\n    channel: Channel;\n    authority: number;\n}',
  },
  {
    name: 'ChunkOptions',
    declaration: 'export interface ChunkOptions {\n    corpus?: string;\n    scope?: string;\n}',
  },
  {
    name: 'ClientResponse',
  ...
  {
    name: 'CompactionPlan',
    declaration: 'export interface CompactionPlan {\n    sessionId: string;\n    summary: string;\n    shadowedRange: {\n        start: number;\n        end: number;\n    };\n    shadowedSeqs: number[];\n    shadowedTokenCount: number;\n    foldOnly: true;\n}',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface CompletionCheck {\n    id: string;\n    clause: string;\n}',
  },
  {
    name: 'CompletionClaim',
    declaration: 'export interface CompletionClaim {\n    taskId: string;\n    summary: string;\n    evidence: string[];\n    selfDeclared: boolean;\n}',
  },
  {
    name: 'CompletionVerdict',
    declaration: 'export interface CompletionVerdict {\n    taskId: string;\n    status: \'PASS\' | \'FAIL\';\n    reasons: string[];\n}',
  },
  {
    name: 'ConfinedArgv',
  ...
  {
    name: 'ContextDebtKind',
    declaration: 'export type ContextDebtKind = \'stuffed\' | \'unretrieved\' | \'positioned\';',
  },
  {
    name: 'ContextDebtReport',
    declaration: 'export interface ContextDebtReport {\n    sessionId: string;\n    kind: ContextDebtKind;\n    measure: number;\n    detail: string;\n}',
  },
  {
    name: 'ContextDebtScan',
    declaration: 'export interface ContextDebtScan {\n    sessionId: string;\n    reports: ContextDebtReport[];\n    foldSeq: number;\n}',
  },
  {
    name: 'ContextFormed',
  ...
  {
    name: 'ContractScore',
    declaration: 'export interface ContractScore {\n    planId: string;\n    total: number;\n    submitted: number;\n    passed: number;\n    failed: number;\n    verdict: \'ALL_PASS\' | \'FAIL\';\n}',
  },
  {
    name: 'CoordinationStats',
    declaration: 'export interface CoordinationStats {\n    workers: {\n        total: number;\n        running: number;\n        completed: number;\n        failed: number;\n    };\n    channels: number;\n}',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface CorrectionRecord {\n    id: string;\n    callId: string;\n    ts: number;\n    classification?: string;\n    note?: string;\n}',
  },
  {
    name: 'CostReport',
    declaration: 'export interface CostReport {\n    totalCostUsd: number;\n    passCount: number;\n    totalCount: number;\n    passRate: number;\n    costPerPassUsd: number;\n    cachedTokens: number;\n    uncachedTokens: number;\n    cacheHitRate: number;\n}',
  },
  {
    name: 'CostReportEntry',
    declaration: 'export interface CostReportEntry {\n    id: string;\n    costUsd: number;\n    pass: boolean;\n}',
  },
  {
    name: 'CostSidecar',
    declaration: 'export class CostSidecar {\n    record(core: string, region: string, budgetBytes: number): PromptLumeCostRecord;\n    summary(): PromptLumeCostSummary;\n}',
  },
  {
  ...
  {
    name: 'DefenseResult',
    declaration: 'export interface DefenseResult {\n    payloadId: string;\n    resisted: boolean;\n    via: \'sanitize\' | \'allowlist\' | \'none\';\n    detail?: string;\n}',
  },
  {
    name: 'DiffCallView',
  ...
  },
  {
    ...
    declaration: 'export type DshEnvironmentKey = `${typeof ATSH_ENV_PREFIX}${string}`;',
  },
  {
  ...
  {
    name: 'Embedder',
    declaration: 'export type Embedder = (messages: unknown[]) => number[];',
  },
  {
    name: 'EpochHeader',
  ...
  {
    name: 'FactoryPlanTask',
    declaration: 'export interface FactoryPlanTask {\n    id: string;\n    verb: string;\n    object: string;\n    verifies: string;\n}',
  },
  {
    name: 'FailureSignal',
    declaration: 'export interface FailureSignal {\n    id: string;\n    severity: SignalSeverity;\n    detail: string;\n}',
  },
  {
    name: 'FileDiff',
  ...
  {
    name: 'GraphBuildResult',
    declaration: 'export interface GraphBuildResult {\n    sessionId: string;\n    objectivesCreated: number;\n    evidenceAdded: number;\n}',
  },
  {
    name: 'GuardReason',
    declaration: 'export type GuardReason = \'call-ceiling\' | \'accounting-cap\' | \'repeated-call\' | \'p-ratio\';',
  },
  {
  ...
  {
    name: 'IngestOptions',
    declaration: 'export interface IngestOptions extends ChunkOptions {\n    namespace?: string;\n}',
  },
  {
    name: 'IngestResult',
  ...
  },
  {
    ...
    declaration: 'export interface InjectionPayload {\n    id: string;\n    klass: PayloadClass;\n    content: string;\n    directedTool?: string;\n}',
  },
  {
  ...
  {
    name: 'JudgeGateAdmissionInput',
    declaration: 'export interface JudgeGateAdmissionInput {\n    planId: string;\n    revision: string;\n    planMarkdown: string;\n}',
  },
  {
    name: 'JudgeGateCompletionInput',
    declaration: 'export interface JudgeGateCompletionInput {\n    planId: string;\n    revision: string;\n    submission: JudgeSubmission;\n}',
  },
  {
    name: 'JudgeKind',
    declaration: 'export type JudgeKind = \'plan\' | \'triage\' | \'completion\';',
  },
  {
    name: 'JudgePanelRole',
    declaration: 'export type JudgePanelRole = \'decomposition\' | \'feasibility\' | \'verification\';',
  },
  {
    name: 'JudgePlanTask',
    declaration: 'export interface JudgePlanTask {\n    id: string;\n    verb: string;\n    object: string;\n    verifies: string;\n}',
  },
  {
    name: 'JudgeReplanState',
    declaration: 'export interface JudgeReplanState {\n    replansUsed: number;\n    maxReplans: number;\n}',
  },
  {
    name: 'JudgeRequest',
  ...
  },
  {
    ...
    declaration: 'export interface JudgeSubmission {\n    summary: string;\n    evidence: string[];\n    files: string[];\n}',
  },
  {
    name: 'JudgeTriage',
    declaration: 'export interface JudgeTriage {\n    failure: string;\n    nextAction: string;\n    evidence: string[];\n}',
  },
  {
    name: 'JudgeVerdict',
  ...
  {
    name: 'JudgeVerdictKind',
    declaration: 'export type JudgeVerdictKind = \'PASS\' | \'REPLAN\' | \'ESCALATE\';',
  },
  {
    name: 'JudgeVote',
    declaration: 'export interface JudgeVote {\n    role: JudgePanelRole;\n    judgmentId: string;\n    planId: string;\n    kind: JudgeKind;\n    vote: \'YES\' | \'NO\';\n    reasons: string[];\n}',
  },
  {
    name: 'KeyResult',
    declaration: 'export interface KeyResult {\n    id: string;\n    objectiveId: string;\n    name: string;\n    metric?: string;\n    target?: string;\n    current?: string;\n    status: \'on-track\' | \'at-risk\' | \'achieved\';\n    createdAt: number;\n    updatedAt: number;\n}',
  },
  {
    name: 'KGraphStats',
    declaration: 'export interface KGraphStats {\n    objectives: number;\n    keyResults: number;\n    evidence: number;\n    sessionsIngested: number;\n}',
  },
  {
    name: 'KnobState',
  ...
  {
    name: 'LaneVetoRecord',
    declaration: 'export interface LaneVetoRecord {\n    name: string;\n    reason: string;\n    ts: number;\n}',
  },
  {
    name: 'LedgerKind',
    declaration: 'export type LedgerKind = \'grant\' | \'debit\';',
  },
  {
    name: 'LedgerRow',
    declaration: 'export interface LedgerRow {\n    id: string;\n    ts: number;\n    account: string;\n    kind: LedgerKind;\n    amount: number;\n    balanceAfter: number;\n    reason: string;\n    meta: Record<string, unknown>;\n}',
  },
  {
    name: 'LlmAdapter',
  ...
  {
    name: 'LlmCostConfig',
    declaration: 'export interface LlmCostConfig {\n    rates?: ModelRateTable;\n    budgetCents?: number;\n}',
  },
  {
    name: 'LlmCostEvent',
    declaration: 'export interface LlmCostEvent {\n    sessionKey: string;\n    model: string;\n    usage: TokenUsage;\n    spentCents: number;\n}',
  },
  {
  ...
  {
    name: 'MeasuredSurfaceLine',
    declaration: 'export interface MeasuredSurfaceLine extends SurfaceLine {\n    readonly bytes: number;\n}',
  },
  {
    name: 'MemoryGetOptions',
    declaration: 'export interface MemoryGetOptions {\n    namespace?: string;\n}',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface MemoryQueryOptions {\n    namespace?: string;\n    limit?: number;\n}',
  },
  {
  ...
  {
    name: 'MemoryReflectOptions',
    declaration: 'export interface MemoryReflectOptions {\n    namespace?: string;\n    limit?: number;\n}',
  },
  {
    name: 'MemoryRetainInput',
    declaration: 'export interface MemoryRetainInput {\n    content: string;\n    namespace?: string;\n    metadata?: Record<string, unknown>;\n}',
  },
  {
    name: 'MemorySummary',
    declaration: 'export interface MemorySummary {\n    total: number;\n    byNamespace: Record<string, number>;\n    recent: MemoryRecord[];\n}',
  },
  {
    name: 'Message',
  ...
  {
    name: 'ModelRate',
    declaration: 'export interface ModelRate {\n    inputPerM: number;\n    outputPerM: number;\n    cacheReadPerM?: number;\n    cacheWritePerM?: number;\n}',
  },
  {
    name: 'ModelRateTable',
    declaration: 'export type ModelRateTable = Readonly<Record<string, ModelRate>>;',
  },
  {
    name: 'Objective',
    declaration: 'export interface Objective {\n    id: string;\n    name: string;\n    description?: string;\n    status: \'active\' | \'achieved\' | \'abandoned\';\n    createdAt: number;\n    updatedAt: number;\n    keyResults: KeyResult[];\n}',
  },
  {
  ...
  {
    name: 'ObsEvent',
    declaration: 'export interface ObsEvent {\n    ts: number;\n    stage: ObsStage;\n    kind: string;\n    detail?: string;\n}',
  },
  {
    name: 'ObsStage',
    declaration: 'export type ObsStage = \'plan\' | \'explore\' | \'evaluate\' | \'verify\';',
  },
  {
  ...
  {
    name: 'PayloadClass',
    declaration: 'export type PayloadClass = 1 | 2 | 3;',
  },
  {
  ...
  {
    name: 'PlannerInput',
    declaration: 'export interface PlannerInput {\n    scope: string;\n    constraints?: string[];\n}',
  },
  {
  ...
  {
    name: 'PromptCorpusSummary',
    declaration: 'export interface PromptCorpusSummary {\n    total: number;\n    byCorpus: Record<string, number>;\n}',
  },
  {
    name: 'PromptLumeCostRecord',
    declaration: 'export interface PromptLumeCostRecord {\n    readonly callCount: number;\n    readonly coreTokens: number;\n    readonly regionTokens: number;\n    readonly inputTokens: number;\n    readonly cacheHit: boolean;\n    readonly budgetBytes: number;\n    readonly regionBytes: number;\n}',
  },
  {
    name: 'PromptLumeCostSummary',
    declaration: 'export interface PromptLumeCostSummary {\n    readonly calls: number;\n    readonly cacheHits: number;\n    readonly cacheMisses: number;\n    readonly totalInputTokens: number;\n    readonly totalRegionTokens: number;\n    readonly totalRegionBytes: number;\n}',
  },
  {
    name: 'PromptRecallOptions',
    declaration: 'export interface PromptRecallOptions {\n    limit?: number;\n    corpus?: string;\n}',
  },
  {
    name: 'PromptRecallResult',
    declaration: 'export interface PromptRecallResult {\n    id: string;\n    content: string;\n    namespace: string;\n    corpus: string;\n    score: number;\n}',
  },
  {
    name: 'PromptSection',
  ...
  {
    name: 'ReplayResult',
    declaration: 'export interface ReplayResult {\n    before: SignalReport;\n    after: SignalReport;\n    changed: string[];\n}',
  },
  {
    name: 'RequestContext',
  ...
  {
    name: 'ResearchPaper',
    declaration: 'export interface ResearchPaper {\n    id: string;\n    title: string;\n    summary: string;\n    authors: string[];\n    published: string;\n    categories: string[];\n    pdfUrl: string;\n}',
  },
  {
    name: 'ResearchPost',
    declaration: 'export interface ResearchPost {\n    id: string;\n    text: string;\n    author: {\n        id: string;\n        username: string;\n        name?: string;\n    };\n    createdAt: string;\n    metrics?: {\n        likeCount: number;\n        repostCount: number;\n        replyCount: number;\n    };\n}',
  },
  {
    name: 'ResearchStats',
    declaration: 'export interface ResearchStats {\n    postsSearched: number;\n    papersSearched: number;\n    papersFetched: number;\n    failures: number;\n}',
  },
  {
  ...
  {
    name: 'RouterCallRecord',
    declaration: 'export interface RouterCallRecord {\n    id: string;\n    ts: number;\n    sessionId?: string;\n    capability: string;\n    requestedProvider: string;\n    requestedModel: string;\n    resolvedProvider: string;\n    resolvedModel: string;\n    routeState: RouteState;\n    status: CallStatus;\n    chunkCount: number;\n    errorCode: string | null;\n    durationMs: number;\n    meta: Record<string, unknown>;\n}',
  },
  {
    name: 'RouterRoute',
    declaration: 'export interface RouterRoute {\n    provider: string;\n    model: string;\n}',
  },
  {
    name: 'RouteState',
    declaration: 'export type RouteState = \'none\' | \'matched\' | \'rewritten\' | \'advisory\';',
  },
  {
    name: 'RpcError',
  ...
  {
    name: 'SanitizeHit',
    declaration: 'export interface SanitizeHit {\n    index: number;\n    marker: string;\n}',
  },
  {
    name: 'SanitizeResult',
    declaration: 'export interface SanitizeResult {\n    text: string;\n    hits: SanitizeHit[];\n    stripped: number;\n}',
  },
  {
  ...
  {
    name: 'SharedStateEntry',
    declaration: 'export interface SharedStateEntry {\n    channel: string;\n    key: string;\n    value: unknown;\n    revision: number;\n    updatedAt: number;\n}',
  },
  {
  ...
  {
    name: 'SignalMetrics',
    declaration: 'export interface SignalMetrics {\n    totalEvents: number;\n    pRatio: number;\n    eToV: number;\n    pxpSpirals: number;\n    maxRepeatRun: number;\n}',
  },
  {
    name: 'SignalReport',
    declaration: 'export interface SignalReport {\n    metrics: SignalMetrics;\n    signals: FailureSignal[];\n    verdict: SignalVerdict;\n}',
  },
  {
    name: 'SignalSeverity',
    declaration: 'export type SignalSeverity = \'warn\' | \'alarm\';',
  },
  {
    name: 'SignalVerdict',
    declaration: 'export type SignalVerdict = \'CLEAR\' | \'WARN\' | \'ALARM\';',
  },
  {
    name: 'SkillCandidate',
  ...
  {
    name: 'StageRoute',
    declaration: 'export interface StageRoute {\n    provider: string;\n    model: string;\n    maxCumulativeCost?: number;\n}',
  },
  {
    name: 'StorageBackend',
  ...
  {
    name: 'SurfaceLine',
    declaration: 'export interface SurfaceLine {\n    readonly seq: number | string;\n    readonly text: string;\n}',
  },
  {
    name: 'SurfaceOp',
  ...
  {
    name: 'ToolAllowlistDenyEvent',
    declaration: 'export interface ToolAllowlistDenyEvent {\n    name: string;\n    agent?: string;\n}',
  },
  {
    name: 'ToolCallKind',
  ...
  {
    name: 'TurnSurface',
    declaration: 'export interface TurnSurface {\n    intent: string;\n    kind?: \'tool\' | \'workspace\' | \'identity\' | \'general\';\n}',
  },
  {
    name: 'TypertCodec',
  ...
  {
    name: 'UpsertObjectiveInput',
    declaration: 'export interface UpsertObjectiveInput {\n    id?: string;\n    name: string;\n    description?: string;\n}',
  },
  {
    name: 'UserMessage',
  ...
  },
  {
    ...
    declaration: 'export interface VerifierFixture {\n    id: string;\n    kind: \'positive\' | \'negative\';\n    claim: CompletionClaim;\n    checks: CompletionCheck[];\n    expected: \'PASS\' | \'FAIL\';\n}',
  },
  {
    name: 'VerifierStats',
    declaration: 'export interface VerifierStats {\n    tpr: number;\n    tnr: number;\n    positives: number;\n    negatives: number;\n}',
  },
  {
    name: 'WebBootEntry',
  ...
  {
    name: 'WorkerRecord',
    declaration: 'export interface WorkerRecord {\n    id: string;\n    provider: string;\n    status: WorkerStatus;\n    startedAt: number;\n    finishedAt: number | null;\n    outcome: string | null;\n}',
  },
  {
    name: 'WorkerStatus',
    declaration: 'export type WorkerStatus = \'running\' | \'completed\' | \'failed\';',
  },
  {
