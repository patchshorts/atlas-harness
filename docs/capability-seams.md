...
| `ctx.shellEnv` | `core` | [`shell-env`](../packages/shell/shell-env) | - | [`tool-bash`](../packages/shell/tool-bash), [`tool-pwsh`](../packages/shell/tool-pwsh) | - | Plugins declare effect-scoped ATSH_* facts; each shell tool collects one trusted snapshot per execution and its executor rebuilds the namespace. |
...
| `ctx.clientModules` | `core` | `modules` | - | `hmr` | - | Composes the __ATSH_BOOT__ entry graph from an incremental atsh.client scan, serves plugin bundles, and notifies rebuilt/graph-changed subscribers. |
