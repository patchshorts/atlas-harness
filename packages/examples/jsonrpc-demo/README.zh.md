...
第一个非空通道生效：先 `$ATSH_CORDIS_CONFIG`，再位置参数 `argv[2]`。如果二者都没有指向现有文件，bin 会向 stderr 打印单行用法并以 1 退出；没有工作目录回退或内置回退。[`dsh-app-boot`](../../boot/app-boot/README.md) 会使插件加载失败成为致命错误。此协议不使用 `ATSH_SNAPSHOT`。
...
- **不存在内置或默认配置**：每次启动都必须提供 `ATSH_CORDIS_CONFIG` 或位置路径；部署方负责完整的插件树和 stdout 纪律。
