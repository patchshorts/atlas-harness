...
import { Inbox } from '@atlasai/atsh-agent'
import type { Agent } from '@atlasai/atsh-agent'
import { runLoaderSmoke } from '@atlasai/atsh-loader-smoke'
import {
  FileNotFoundError,
  Sandbox,
  SandboxNotFoundError,
} from '@atlasai/atsh-e2b'
import TerminalSessionService, { TerminalSessionId } from '@atlasai/atsh-terminal'
...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', ATSH_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
      lifecycle: { onTimeout: 'kill' },
    })
    try {
      const profileLeakPath = '/home/user/atsh-e2b-bootstrap-profile-leak'
      ...
      ctx.provide('e2b', {
        cwd: '/home/user',
        runtimeRoot: '/home/user/.atsh-e2b',
      ...
      const environmentProbe = ctx.subprocess.spawn({
        argv: ['/bin/bash', '-c', [
          'atsh_leak=0',
          'for atsh_pid in "$PPID" $(ps -o pid= --ppid "$PPID"); do',
          '  [[ "$atsh_pid" == "$$" ]] && continue',
          '  if tr "\\0" "\\n" < "/proc/$atsh_pid/environ" 2>/dev/null | grep -Fqx "NPM_TOKEN=sentinel-secret"; then atsh_leak=1; fi',
          'done',
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> ATSH=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$ATSH_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> ATSH=<> KEEP=<visible>')
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      ...
      tempDirPrefix: 'atsh-e2b-composition-',
      binScript,
    ...
    expect(terminalMotd).not.toContain('.atsh-e2b/terminals/')
