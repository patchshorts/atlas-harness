...
import { normalizeSessionLog, scrubRequestHeaders, type NormalizeContext } from '@atlasai/atsh-acp-snapshot'
...
import SessionStore, { SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@atlasai/atsh-session'
  ...
  it('confines a delegated child through the assembled headless app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ...
        ATSH_SNAPSHOT_FILE: replayOverride,
        ATSH_SNAPSHOT_OVERRIDE: replayOverride,
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@atlasai/atsh-system-prompt') return []
        ...
        for (const context of policyContexts) {
          expect(context).toContain('Any available operation enforced by the ATSH file sandbox cannot modify files in the standing mode.')
