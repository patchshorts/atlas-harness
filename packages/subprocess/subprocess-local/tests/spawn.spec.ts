...
import type { SubprocessHandle, SubprocessOutputReader } from '@atlasai/atsh-subprocess'
import { MAX_TIMER_DELAY_MS } from '@atlasai/atsh-timeout'
...
describe('tree-survivor escalation (terminate and bounded waits reach helpers the leader left behind)', () => {
  ...
  it('service teardown awaits tree survivors, not just handle settlement', async () => {
    ...
    const { default: LocalSubprocessRuntime } = await import('@atlasai/atsh-subprocess-local')
...
describe('environment and spill-file hardening', () => {
  it('scrubs credential-shaped and ambient ATSH env vars from child processes', async () => {
    process.env.ATSH_TEST_API_KEY = 'super-secret'
    process.env.ATSH_TEST_TOKEN = 'also-secret'
    process.env.SUBPROCESS_TEST_PASSWORD = 'password-secret'
    process.env.ATSH_TEST_PLAIN = 'visible'
    try {
      const result = await finish(spawnSubprocess(spec(
        'echo "[${ATSH_TEST_API_KEY:-absent}|${ATSH_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${ATSH_TEST_PLAIN:-absent}]"',
      )))
      expect(result.stdout.text.trim()).toBe('[absent|absent|absent|absent]')
    } finally {
      delete process.env.ATSH_TEST_API_KEY
      delete process.env.ATSH_TEST_TOKEN
      delete process.env.SUBPROCESS_TEST_PASSWORD
      delete process.env.ATSH_TEST_PLAIN
    }
  })

  it('forwards explicit ATSH_* env entries while scrubbing ambient ones', async () => {
    // Both facts through one explicit map: the ambient ATSH_STALE is dropped by
    // the scrub, and the deliberately supplied current values merge after it.
    process.env.ATSH_STALE = 'old-value'
    try {
      const result = await finish(spawnSubprocess(spec('echo "[${ATSH_STALE:-absent}|$ATSH_SHELL|$ATSH_SESSION_ID]"', {
        env: { ATSH_SHELL: '1', ATSH_SESSION_ID: 'current-session' },
      })))
      expect(result.stdout.text.trim()).toBe('[absent|1|current-session]')
    } finally {
      delete process.env.ATSH_STALE
    }
  })
