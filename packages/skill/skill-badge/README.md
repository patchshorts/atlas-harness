# @atlasai/atsh-skill-badge

English | [中文](README.zh.md)

Optional bundled skill provider that contributes `atsh-badge` to `ctx.skills`. The skill supplies the official “powered by atsh” Markdown snippets and the packaged PNG for systems that cannot import a remote image reliably.
...
The provider exposes its packaged `assets/` directory as the skill resource base. `atsh-badge.png` is the 726×120 source asset, and consumers render it at 121×20.

## Model Experience

Indirectly, through `@atlasai/atsh-tool-skill`, which renders the catalog entry and selected skill body.
