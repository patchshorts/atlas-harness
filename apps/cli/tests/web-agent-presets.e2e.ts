...
import { boot, healProfilesModuleFallback, loadOverlayPatches } from '@atlasai/atsh-app-boot'
...
import { resolveSessionPreset, SETTINGS_NAMESPACE } from '@atlasai/atsh-agent-presets'
import { applyChildComposition, childSessionMeta } from '@atlasai/atsh-subagent'
import { CallId } from '@atlasai/atsh-llm'
import type {} from '@atlasai/atsh-compaction-basic'
...
import type {} from '@atlasai/atsh-token-meter'
...
async function bootWeb(
  ...
  const patches: PatchOptions[] = [
    ...
    // The settings row defaults to `$ATSH_HOME/settings.yaml`. Left alone it
    ...
    // storage-json's root is anchored to the real $ATSH_HOME. Unpinned, this
    // file writes the developer's own `~/.atsh/storages/` — and then reads it
    ...
    // developer's own `~/.atsh/.preset` cannot change this test's outcome.
...
describe('the shipped Web composition', () => {
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    ...
    await mkdir(join(proj, '.atsh', 'skills', 'project-proof'), { recursive: true })
    await writeFile(join(proj, '.atsh', 'skills', 'project-proof', 'SKILL.md'), [
      '---',
      'name: project-proof',
    ...
    const handle = await ctx.agents.create({
      // Unique per run: the composition persists into the ambient ATSH home,
    ...
    try {
      ...
      expect((await ctx.skills.list({ cwd: proj })).map(skill => skill.name)).toEqual(['atsh-badge'])
      ...
      expect(scoped).toContain('atsh-badge')
      ...
      expect(JSON.stringify(loaded.content)).toContain('powered by atsh')
    } finally {
      await handle.dispose()
    }
  })
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect((await ctx.skills.list({ scope: handle.agent })).map(skill => skill.name)).toContain('atsh-badge')
...
describe('product subagent rows in user presets', () => {
  ...
  beforeAll(async () => {
    ...
    productCtx = await bootWeb(settingsFile, [
      { insert: [
        { id: 'subagent-codex', name: '@atlasai/atsh-subagent-codex' },
        { id: 'subagent-claude-code', name: '@atlasai/atsh-subagent-claude-code' },
      ] },
      {
        id: 'agent-presets',
        config: {
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<atshHome>/.agent-presets` itself. `$ATSH_HOME` is pointed
  ...
  afterAll(async () => {
    if (previousHome === undefined) delete process.env.ATSH_HOME
    else process.env.ATSH_HOME = previousHome
