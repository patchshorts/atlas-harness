 ...
 * trusted, per-execution `ATSH_*` variables consumed by the model-facing shell
 ...
 *
 * @module @atlasai/atsh-shell-env
 */

import { Service, type Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { ATSH_ENV_PREFIX } from '@atlasai/atsh-shell'
import type { DshEnvironment, DshEnvironmentKey } from '@atlasai/atsh-shell'
import { ATSH_HOME_ENV, resolveDshHome } from '@atlasai/atsh-home-paths'
import type { ToolExecution } from '@atlasai/atsh-tools'
import type {} from '@atlasai/atsh-session-persistence'
...
/** Model-visible metadata for one managed `ATSH_*` environment variable. */
...
export interface BashEnvContributor {
  ...
  name: string
  /** Complete set of `ATSH_*` keys this contributor may return. */
...
export interface BashEnvVariableInfo extends BashEnvVariable {
  ...
  contributor: string
  /** Declared `ATSH_*` environment variable name. */
  key: DshEnvironmentKey
}

const ATSH_SHELL_KEY = `${ATSH_ENV_PREFIX}SHELL` as const
const ATSH_SESSION_ID_KEY = `${ATSH_ENV_PREFIX}SESSION_ID` as const
const ATSH_SESSION_JSONL_KEY = `${ATSH_ENV_PREFIX}SESSION_JSONL` as const
...
/**
 * Registry (`ctx.shellEnv`) for trusted, per-execution `ATSH_*` variables.
 * The namespace is rebuilt for every model shell call: ambient `ATSH_*` values
...
export class ShellEnvRegistry extends Service {
  ...
  private readonly atshHome: string
  ...
  constructor(ctx: Context, config: Config = {}) {
    super(ctx, 'shellEnv')
    this.atshHome = resolveDshHome(config.atshHome)
  }
  ...
  register(contributor: BashEnvContributor): () => void {
    const dispose = this.ctx.effect(function* (this: ShellEnvRegistry) {
      ...
      for (const [key, variable] of variables) {
        if (!key.startsWith(ATSH_ENV_PREFIX)
          || !BASH_ENV_KEY_SUFFIX.test(key.slice(ATSH_ENV_PREFIX.length))) {
  ...
  /**
   * Build the trusted `ATSH_*` snapshot for one shell tool execution.
   * @param execution - the current tool execution.
   * @returns an immutable environment overlay containing built-ins and current contributions.
   */
  collect(execution: ToolExecution): DshEnvironment {
    const values: Record<DshEnvironmentKey, string> = {
      [ATSH_HOME_ENV]: this.atshHome,
      [ATSH_SHELL_KEY]: '1',
    }
    if (execution.agent !== undefined) {
      values[ATSH_SESSION_ID_KEY] = execution.agent.session.header.id
    }
 ...
 * shell-agnostic persistence contributor (`ATSH_SESSION_JSONL`).
 * @param ctx - Cordis context that owns the service and registrations.
 * @param config - home-directory configuration for the built-in variables.
 */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  registry.register({
    ...
    resolve(execution) {
      ...
      return location?.kind === 'jsonl' ? { [ATSH_SESSION_JSONL_KEY]: location.path } : {}
    },
  })
}
