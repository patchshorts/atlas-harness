...
Keys are write-only. The page receives a redacted descriptor after saving, never the literal secret. The key is stored in `$ATSH_HOME/.credentials.yaml`, while settings retain only its credential reference.
...
A vision model on a custom provider therefore needs one line. The form has no field for it; add `input` to the model in `$ATSH_HOME/settings.yaml`:
