...
import { createUserMessage, CallId } from '@atlasai/atsh-llm'
import SystemPrompt from '@atlasai/atsh-system-prompt'
import ToolRuntime, { RUN_CODE_NAME, defineContentToolFixture } from '@atlasai/atsh-tools'
import { Session, SessionId, type UserMessage } from '@atlasai/atsh-session'
import AgentRegistry, { agentEvents, type Agent } from '@atlasai/atsh-agent'
...
import CommandRuntime from '@atlasai/atsh-commands'
import { CodeRuntime, type CodeRunRequest, type CodeRunResult } from '@atlasai/atsh-code-runtime'
...
describe('exit_plan_mode', () => {
  ...
  it('composed judge gate: rejects a non-decomposed plan before the user review', async () => {
    const ctx = await setup()
    await ctx.plugin(AgentRegistry)
    await ctx.plugin(UserQuestionService)
    ...
    const agent = await agentWithSession(ctx, 'judge-bad', { active: true })
    const result = await callExit(ctx, agent, '# Bad plan\n\n1. [fix] everything — verifies: ')
    // The gate runs BEFORE the review question: fail-closed with the ballot
    // reasons, and the user is never asked. Every NO ballot's reason rides
    // the error, so the model revises with exact artifact-citing feedback.
    expect(result.isError).toBe(true)
    const text = (result.content[0] as { text?: string }).text ?? ''
    expect(text).toMatch(/^Error: the three-panel judge rejected this plan; revise it and present it again\. Ballot reasons: /)
    expect(text).toContain('task T1 lacks a non-empty verifies')
    expect(asked).toHaveLength(0)
    expect(foldPlanMode(agent.session.events)).toBe(true)
  })

  it('composed judge gate: admits a well-decomposed plan to the user review', async () => {
    const ctx = await setup()
    await ctx.plugin(AgentRegistry)
    await ctx.plugin(UserQuestionService)
    await ctx.plugin(JudgeService, {})
    ...
    const agent = await agentWithSession(ctx, 'judge-good', { active: true })
    const result = await callExit(ctx, agent, [
      '# Good plan',
    ...
    // Unanimous PASS: the review question fires exactly as without the gate.
    expect(result.isError).toBe(false)
    expect(asked).toHaveLength(1)
    expect(foldPlanMode(agent.session.events)).toBe(true)
  })

  it('composed judge gate: bounds re-presentation at the N≤2 replan budget', async () => {
    const ctx = await setup()
    await ctx.plugin(AgentRegistry)
    await ctx.plugin(UserQuestionService)
    await ctx.plugin(JudgeService, {})
    ...
    const agent = await agentWithSession(ctx, 'judge-budget', { active: true })
    const bad = '# Bad plan\n\n1. [fix] everything — verifies: '
    const rejection = (result: { content: { type: string; text?: string }[] }): string =>
      (result.content[0] as { text?: string }).text ?? ''
    // Rounds 1-2: NO → REPLAN, the model revises and re-presents.
    for (let round = 1; round <= 2; round++) {
      const result = await callExit(ctx, agent, bad)
      expect(result.isError).toBe(true)
      expect(rejection(result)).toContain('task T1 lacks a non-empty verifies')
    }
    // Round 3: the gate's pre-check escalates without a new panel round —
    // no fabricated ballots, no unbounded revision loop.
    const escalated = await callExit(ctx, agent, bad)
    expect(escalated.isError).toBe(true)
    expect(rejection(escalated)).toContain('replan budget exhausted: max 2 replans per judgment')
    expect(asked).toHaveLength(0)
    expect(foldPlanMode(agent.session.events)).toBe(true)
  })
