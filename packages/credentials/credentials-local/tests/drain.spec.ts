...
vi.mock('@atlasai/atsh-atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@atlasai/atsh-atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@atlasai/atsh-atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
  mocked.__setGate(next)
}

const KEY = credentialRef('ATSH_CRED_DRAIN_A')
const OTHER = credentialRef('ATSH_CRED_DRAIN_B')
