...
import AgentDefaultModelConfig from '@atlasai/atsh-agent-default-model'
import { createAssistantMessage } from '@atlasai/atsh-llm'
import SessionStore from '@atlasai/atsh-session'
import type { Session, UserMessage } from '@atlasai/atsh-session'
...
describe('headless runner', () => {
  ...
  it('prints the durable model failure when the final turn ends in error', async () => {
    ...
    expect(await test.run()).toMatchObject({
      code: 1,
      out: '\n',
      err: 'atsh: SERVER: provider unavailable\n',
    })
    await test.ctx.fiber.dispose()
  })
  ...
  it('reports a direct Agent creation failure', async () => {
    ...
    expect(err).toBe('atsh: factory exploded\n')
