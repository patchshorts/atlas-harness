...
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@atlasai/atsh': ['lib/*.js', 'config'],
  // The Web build emits sourcemaps for browser debugging; publishing them is
  // what the payload policy forbids, so the bundle ships without them.
  '@atlasai/atsh-web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  // Profile bundles publish their atsh.bundle.patch layer beside the lib.
  '@atlasai/atsh-base': ['cordis.patch.yml'],
  '@atlasai/atsh-web-app': ['cordis.patch.yml'],
  '@atlasai/atsh-headless': ['cordis.patch.yml'],
  '@atlasai/atsh-client-ui-theme': ['lib/styles'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@atlasai/atsh-sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@atlasai/atsh-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  '@atlasai/atsh-skill-badge': ['assets'],
  '@atlasai/atsh-subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    // them, while the atsh family stays restricted until its own sequence goes
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@atlasai/atsh-')) {
...
function checkRepositoryVersion(): string[] {
  // The root carries the atsh release family's version, so a prerelease such as
  // 0.0.1-rc.1 is a valid state between `release:atsh` and its publication.
