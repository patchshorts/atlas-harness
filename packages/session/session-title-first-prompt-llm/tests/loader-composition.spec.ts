...
import LlmRuntime, { createUserMessage, LlmAdapter  } from '@atlasai/atsh-llm'
import type { GenerateOptions, StreamChunk } from '@atlasai/atsh-llm'
import SessionStore, { SessionId } from '@atlasai/atsh-session'
import SessionTitleService from '@atlasai/atsh-session-title'
import * as providerPlugin from '@atlasai/atsh-session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    "- name: '@atlasai/atsh-llm'",
    "- name: '@atlasai/atsh-session'",
    "- name: '@atlasai/atsh-session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@atlasai/atsh-session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ['@atlasai/atsh-llm', LlmRuntime],
    ['@atlasai/atsh-session', SessionStore],
    ['@atlasai/atsh-session-title', SessionTitleService],
    ['@atlasai/atsh-session-title-first-prompt-llm', providerPlugin],
  ])
