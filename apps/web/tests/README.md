...
the deliberate composition divergences from `atsh web` — are documented in
...
## Do not import `@atlasai/atsh-client-*` here
...
`@atlasai/atsh-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@atlasai/atsh-client-web` and the boot-manifest
type from `@atlasai/atsh-client-modules/client`: booting the real shell is what
...
`@atlasai/atsh-client-runtime/client` because `client/runtime` is reachable
