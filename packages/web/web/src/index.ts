...
export class WebRuntime extends Service {
   ...
   * `$ATSH_WEB_SEARCH_PROVIDER` / `$ATSH_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.ATSH_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.ATSH_WEB_FETCH_PROVIDER
  }
