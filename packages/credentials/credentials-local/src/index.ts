/**
 * File-backed credentials provider over `$ATSH_HOME/.credentials.yaml`, layered
 * against the environment by how much each layer is trusted:
 *
 * ```text
 * inherited process environment      (read-only, wins)
 * > $ATSH_HOME/.credentials.yaml      (provider-managed, writable)
 * > <invocation cwd>/.env            (read-only fallback)
 * > $ATSH_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `DEEPSEEK_API_KEY=… atsh`, a CI
 ...
 * @module @atlasai/atsh-credentials-local
 */
...
import { withFileLock, writeFileAtomic } from '@atlasai/atsh-atomic-write'
import { canonicalizeWatchPath, resolveDshHome } from '@atlasai/atsh-home-paths'
import { launchEnvironmentOf } from '@atlasai/atsh-launch-environment'
import { CredentialProvider, credentialRef } from '@atlasai/atsh-credentials'
import type { CredentialInfo, CredentialRef, ResolvedCredential } from '@atlasai/atsh-credentials'
import type { LaunchEnvironmentEntry } from '@atlasai/atsh-launch-environment'
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveDshHome(config.atshHome), CREDENTIALS_FILENAME)),
...
/** File-backed credentials provider (`$ATSH_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start atsh from instead',
      )
    }
  }
