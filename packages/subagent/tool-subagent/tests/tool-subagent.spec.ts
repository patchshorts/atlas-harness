...
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@atlasai/atsh-tools'
import { assembleContextFor, type Agent } from '@atlasai/atsh-agent'
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @atlasai/atsh-jobs')
  })
