...
import { defineAcpSnapshotSuite, type Scenario, type SnapshotSuiteOptions } from '@atlasai/atsh-acp-snapshot'
import { resolvePwshPath } from '@atlasai/atsh-pwsh-local'
import { decodeStorageRecord } from '@atlasai/atsh-session'

/**
 * The acp-agent example's snapshot suite: the scenario table for
 * `atsh-acp-snapshot`'s suite factory, which owns every compare/guard mechanic
...
function snapshotModeFromEnv(value: string | undefined): SnapshotSuiteOptions['mode'] {
  switch (value) {
    ...
    case 'refresh':
      return 'refresh'
    default:
      throw new Error(`unknown ATSH_SNAPSHOT mode: ${value}`)
  }
}
...
const SCENARIOS: Scenario[] = [
  ...
  // `false` command exits 1, rather than becoming SANDBOX_UNAVAILABLE.
  {
    ...
    env: { ATSH_PERMISSION_MODE: 'read-only' },
    posixOnly: true,
  },
  ...
  // foreground error and background job marker without a platform runner.
  {
    ...
    configPath: PARTIAL_LANDLOCK_CONFIG,
    env: {
      ATSH_PERMISSION_MODE: 'read-only',
      ATSH_SNAPSHOT_MISSING_SANDBOX_RUNNER: '1',
    },
    posixOnly: true,
  },
  ...
  // published-handle disposal failure.
  {
    ...
    env: { ATSH_SUBAGENT_PUBLISHED_FAILURE: '1' },
    hasModelTurn: true,
    recorded: false,
    overridden: true,
    configPath: SUBAGENT_DURABILITY_FAILURE_CONFIG,
  ...
  // no session-scoped UI picker on the automation protocol.
  {
    ...
    systemPromptSource: 'text-turn',
    toolSchemasSource: 'text-turn',
    env: { ATSH_PERMISSION_MODE: 'workspace-write' },
  },
  {
...
defineAcpSnapshotSuite({
  ...
  scenarios: SCENARIOS,
  mode: snapshotModeFromEnv(process.env.ATSH_SNAPSHOT),
  hasPwsh,
})
