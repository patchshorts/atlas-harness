...
处于激活状态时，`plan:policy` 会渲染配置的 `section`。插件始终注册 `exit_plan_mode`，使工具 schema 在状态切换期间保持稳定；其 execute 路径仅在 plan mode 激活时接受调用，并且只有在通过 `ctx.userQuestions` 获得用户明确批准后才退出。

当部署组合了 [`@atlasai/atsh-judge-gate`](../../guard/judge-gate/README.md) 时，`exit_plan_mode` 会在 plan 准入阶段（即用户评审之前）运行三人评审组（three-panel judge）。NOT PASS 裁决会带着各 ballot 的理由使调用失败，模型据此修订并重新提交；评审组的 replan 预算（每个 judgment 最多 N≤2 次）约束该循环。未组合该 gate 的部署保持原有行为；gate 从不读取或修改会话日志、消息历史或投影——只处理 plan 工件。
