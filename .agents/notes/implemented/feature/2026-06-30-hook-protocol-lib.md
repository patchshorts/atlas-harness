# Agent Note: atsh-hook-protocol — the shared Claude Code / Codex hook wire-protocol core
...
This Agent Note introduces `@atlasai/atsh-hook-protocol`, a **library** (not a plugin — it registers and injects nothing) holding the genuinely-identical primitives both bridges build on. The split between shared and per-dialect is the design's center of gravity.

## Decision

A new `packages/hooks/` group with `hook-protocol` as a pure library. It owns four primitive families and the `hook/*` session events; each bridge plugin (`atsh-hooks-claude-code`, `atsh-hooks-codex`) owns what genuinely differs.
