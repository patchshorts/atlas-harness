/**
 * Node half of the client module system (`atsh.client` dual-face package): scans
 * the host Loader's entries for packages declaring `atsh.client`, composes the
 * `window.__ATSH_BOOT__` entry graph (wire single source: {@link WebBootEntry}
 ...
 * @module @atlasai/atsh-client-modules
 */
...
/** package.json `atsh.client` declaration fields, validated one by one after reading the file. */
...
/** Resolved package metadata for one `atsh.client` package (cached per name, never expires). */
interface PkgMeta {
  clientPath: string
  inject?: string[]
  immediately: boolean
...
/** Narrow an unknown parsed JSON value to the `atsh.client` declaration, throwing on malformed fields. */
function parseDshClient(pkgName: string, value: unknown): DshClientDeclaration | undefined {
  if (value === undefined) return undefined
  if (typeof value !== 'object' || value === null) {
    throw new Error(`client-modules: ${pkgName} has a non-object atsh.client declaration`)
  }
  const decl = value as Record<string, unknown>
  if (typeof decl.platform !== 'string') {
    throw new Error(`client-modules: ${pkgName} atsh.client.platform must be a string`)
  }
  if (decl.inject !== undefined && (!Array.isArray(decl.inject) || decl.inject.some(i => typeof i !== 'string'))) {
    throw new Error(`client-modules: ${pkgName} atsh.client.inject must be a string array`)
  }
  if (decl.immediately !== undefined && typeof decl.immediately !== 'boolean') {
    throw new Error(`client-modules: ${pkgName} atsh.client.immediately must be a boolean`)
  }
  return {
...
/**
 * Inject the boot entry graph into index.html: `window.__ATSH_BOOT__` as the
...
export function injectBootManifest(html: string, graph: WebBootGraph): string {
  ...
  const script = `<script>window.__ATSH_BOOT__ = ${json}</script>`
...
/**
 * The web plugin table service: incremental `atsh.client` scan + wire composition
...
export class ClientModuleRegistry extends Service {
  ...
  // subpath rows — or a package without a web `atsh.client` declaration) are
   ...
   * @returns the graph served as `window.__ATSH_BOOT__`.
   */
  graph(): WebBootGraph {
  ...
  private resolveMeta(pkgName: string): PkgMeta | null {
    ...
    const decl = parseDshClient(
      pkgName,
      atsh !== null && typeof atsh === 'object' ? (atsh as Record<string, unknown>).client : undefined,
    )
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${pkgName} declares atsh.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
