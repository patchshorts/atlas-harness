...
import { renderPrompt, TOOL_ORDER_REST } from '@atlasai/atsh-system-prompt'
import * as agentCore from '../src/index.ts'
import { agentEvents, type Agent } from '@atlasai/atsh-agent'
...
declare module '@atlasai/atsh-jobs' {
...
/**
 * Unit coverage for the @atlasai/atsh-agent-spine-demo bundle: mounting it brings
...
async function mount(config: agentCore.Config, withBash = false): Promise<Context> {
  const oldDshHome = process.env.ATSH_HOME
  const oldAgentsHome = process.env.ATSH_AGENTS_HOME
  process.env.ATSH_HOME = await mkdtemp(join(tmpdir(), 'dsh-agent-spine-demo-home-'))
  process.env.ATSH_AGENTS_HOME = await mkdtemp(join(tmpdir(), 'dsh-agent-spine-demo-agents-'))
  ...
  try {
    ...
    if (oldDshHome === undefined) {
      delete process.env.ATSH_HOME
    } else {
      process.env.ATSH_HOME = oldDshHome
    }
    if (oldAgentsHome === undefined) {
      delete process.env.ATSH_AGENTS_HOME
    } else {
      process.env.ATSH_AGENTS_HOME = oldAgentsHome
    }
  }
}
...
describe('dsh-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@atlasai/atsh-agent$'] },
      { package_blocklist: ['^@atlasai/atsh-session$'] },
    ]) {
  ...
  it('shares top-level atshHome between local skills and the managed bash environment', async () => {
    ...
    expect(ctx.shellEnv.collect(execution)).toMatchObject({ ATSH_HOME: home, ATSH_SHELL: '1' })
    await ctx.fiber.dispose()
  })

  it('rejects conflicting global and nested ATSH home directories', () => {
    expect(() => {
      agentCore.apply(new Context(), {
        atshHome: '/global-dsh-home',
        workspaceContext: false,
        skills: { filesystem: { atshHome: '/nested-dsh-home' } },
      })
    }).toThrow('agent-spine-demo: atshHome and skills.filesystem.atshHome must resolve to the same directory')
  })
