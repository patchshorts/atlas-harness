...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/shell/shell-env': { kind: 'indirect', reason: 'The env service exposes managed ATSH_* facts through the shell tools (dsh-tool-bash/dsh-tool-pwsh); it registers no prompt or schema of its own.' },
  ...
  'packages/runtime-diagnostics/events': { kind: 'none', reason: 'The event stream is a diagnostic projection that never alters prompts, messages, schemas, streams, or tool results.' },
  'packages/runtime-diagnostics/alarms': { kind: 'none', reason: 'The alarm detectors are diagnostic folds that never alter prompts, messages, schemas, streams, or tool results.' },
  'packages/runtime-diagnostics/verifiers': { kind: 'none', reason: 'The verifier evaluation is a pure fold over labeled fixtures that never alters prompts, messages, or tool results.' },
