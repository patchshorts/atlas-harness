/**
 * Shared boot glue for the app bins (`atsh`, `dsh-acp-demo`): load the gitignored
 * `.env`, install the fail-loud Loader guards, resolve the config path (snapshot-aware), load the
 * optional user patch layers from the Harness home (`~/.atsh`), expose its path resolver to
 * config expressions, and drive the Cordis Loader against a leaf `cordis.yml` until the tree settles.
 * @module @atlasai/atsh-app-boot
 */
...
import { atshHomePath, resolveDshHome } from '@atlasai/atsh-home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@atlasai/atsh-launch-environment'
...
declare module '@deepseek-ai/cordis' {
  interface Context {
    ...
    atshHomePath?: typeof atshHomePath
  }
}
...
export {
  ...
  type AtshBundleManifest,
  type AtshManifestSection,
  type AtshProfileManifest,
  type Profile,
  type ProfileLayer,
 ...
 * @param snapshotMode - the bin's `$ATSH_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_PREFIXES = ['ATSH_', 'XDG_', 'DYLD_', 'BASH_FUNC_']
