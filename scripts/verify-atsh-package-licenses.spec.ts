...
import { inspectDshPackageLicenses } from './verify-atsh-package-licenses.ts'
...
describe('ATSH package license gate', () => {
  it('checks root, unhyphenated CLI, and dsh-prefixed package names while ignoring other families', () => {
    const root = createWorkspace()
    writeManifest(root, 'apps/cli/package.json', { name: '@atlasai/atsh', license: 'MIT' })
    ...
    expect(inspectDshPackageLicenses(root)).toEqual({
      packageCount: 3,
      failures: [
        'packages/core/agent/package.json: @atlasai/atsh-agent must declare "license": "MIT"; found "BSD-3-Clause".',
      ],
    })
  })
  ...
  it('rejects a missing license declaration', () => {
    const root = createWorkspace()
    writeManifest(root, 'packages/core/agent/package.json', { name: '@atlasai/atsh-agent' })

    expect(inspectDshPackageLicenses(root).failures).toEqual([
      'packages/core/agent/package.json: @atlasai/atsh-agent must declare "license": "MIT"; found undefined.',
    ])
  })
})
