...
import { startMockLlmServer } from '@atlasai/atsh-llm-mock-server'
...
async function runBuiltBin(
  ...
  if (result.timedOut) {
    throw new Error(`atsh built bin did not exit within 25s. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
  }
...
async function waitForFile(file: string): Promise<void> {
  ...
  while (!existsSync(file)) {
    if (Date.now() >= deadline) throw new Error(`atsh profile lifecycle marker did not appear: ${file}`)
 ...
 * atsh.profile.bundles, no dsh-base — proving out-of-box composition machinery without
...
function createProfileLifecycleFixture(): ProfileLifecycleFixture {
  ...
  writeFileSync(join(bundleDir, 'plugin.mjs'), [
    ...
    "  writeFileSync(join(process.env.ATSH_HOME, 'config-echo'), String(config.generation ?? 'bundle-default'))",
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    atsh: { profile: { bundles: ['dsh-lifecycle-bundle'] } },
  }, undefined, 2))
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    atsh: { profile: { bundles: ['@atlasai/atsh-base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@atlasai/atsh-cmdline` and `commander` through the profile module
...
function createStartupFixture(): StartupFixture {
  ...
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    ...
    "import { parseCmdline } from '@atlasai/atsh-cmdline'",
  ...
  writeFileSync(join(bundleDir, 'witness.mjs'), [
    ...
    'export function apply(ctx, config = {}) {',
    "  writeFileSync(join(process.env.ATSH_HOME, 'witness'), String(config.generation ?? 'bundle-default'))",
    '}',
    '',
  ].join('\n'))
  writeFileSync(join(bundleDir, 'cordis.patch.yml'), [
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    atsh: { profile: { bundles: ['dsh-startup-bundle'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  return {
    home,
...
describe.skipIf(!existsSync(dshBin))('atsh BUILT bin (node lib/bin.js, no tsx)', () => {
  it('requires --profile and rejects removed commands', async () => {
    ...
    expect(help.stdout).toContain('atsh --profile web')
    expect(help.stdout).toContain('atsh plugin --profile')
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    ...
    try {
      const web = await runBuiltBin(['--profile', 'web', '--help'], {
        ATSH_HOME: home,
        ATSH_TELEMETRY_DISABLED: '1',
      })
      expect(web.code).toBe(0)
      expect(web.stderr).toBe('')
      expect(web.stdout).toContain('Usage: atsh --profile web')
      expect(web.stdout).toContain('--port <port>')
      expect(web.stdout).not.toContain('atsh web: http://')
      ...
      expect(wildcardHost.stderr).not.toContain('atsh web: http://')
      ...
      expect(headlessHelp.stdout).toContain('Usage: atsh --profile headless')
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    ...
    try {
      const result = await runBuiltBin(['--profile', 'nope'], { ATSH_HOME: home })
      expect(result.code).toBe(1)
      expect(result.stderr).toContain('profile "nope" does not exist')
      expect(result.stderr).toContain('atsh plugin --profile nope add')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  it('reports a patch-overlay boot failure without hanging', async () => {
    ...
    // refresh drain: atsh exited 13 with no diagnostic instead of settling
  ...
  it('fully settles a custom profile, hot-reloads its patch layer with removal reverting, and disposes on a signal', async () => {
    ...
    try {
      ...
      // The home-level user layer ($ATSH_HOME/cordis.patch.yml) is live too
  ...
  it('anchors a relative add spec to the invoking directory, not the profile', async () => {
    // `atsh plugin --profile x add .` from a plugin checkout must install THAT
    ...
    try {
      ...
      const manifest = JSON.parse(readFileSync(join(home, 'profiles', 'anchor', 'package.json'), 'utf8')) as {
        ...
        atsh: { profile: { bundles: string[] } }
      }
      expect(Object.keys(manifest.dependencies)).toEqual(['anchored-bundle'])
      expect(manifest.atsh.profile.bundles).toContain('anchored-bundle')
    } finally {
      rmSync(home, { recursive: true, force: true })
  ...
  it('activates a dependency that gained atsh.bundle in a later update', async () => {
    ...
    // version declares atsh.bundle. Simulated without a registry: hand-place
    ...
    try {
      ...
      writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
      // v1: no atsh manifest — a plain dependency.
      writeFileSync(join(installed, 'package.json'), JSON.stringify({ name: 'late-bundle', version: '1.0.0' }))
      const first = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { ATSH_HOME: home })
      expect(first.code).toBe(0)
      let manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { atsh: { profile: { bundles: string[] } } }
      expect(manifest.atsh.profile.bundles).toEqual(['@atlasai/atsh-base'])
      // v2: the installed package now declares atsh.bundle (an update landed).
      writeFileSync(join(installed, 'package.json'), JSON.stringify({
        name: 'late-bundle', version: '2.0.0', atsh: { bundle: { patch: './cordis.patch.yml' } },
      }))
      writeFileSync(join(installed, 'cordis.patch.yml'), '[]\n')
      const second = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { ATSH_HOME: home })
      expect(second.code).toBe(0)
      manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { atsh: { profile: { bundles: string[] } } }
      expect(manifest.atsh.profile.bundles).toEqual(['@atlasai/atsh-base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      const { stdout, code, stderr } = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { ATSH_HOME: home })
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@atlasai/atsh-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @atlasai/atsh-base')
      expect(stdout).toContain("name: '@atlasai/atsh-host-webserver'")
    }, 30_000)
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@atlasai/atsh-headless'")
      expect(stdout).not.toMatch(/name: '@deepseek-ai\/dsh-host-/)
      expect(stdout).not.toContain("name: '@atlasai/atsh-web-app'")
    ...
    it('composes the profile user layer and a --patch overlay in order', async () => {
      ...
      const init = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { ATSH_HOME: home })
