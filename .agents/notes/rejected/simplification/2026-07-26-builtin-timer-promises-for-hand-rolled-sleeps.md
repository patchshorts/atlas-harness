...
Three packages hand-roll promise-wrapped timers that the `node:timers/promises` builtin already provides, while other packages (`dsh-llm-mock-server` `pause()`, `dsh-lsp-stdio`, `atsh-acp-snapshot`) already use the builtin — so the hand-rolled copies are also a consistency gap:
