...
import type { SubprocessOutcome } from '@atlasai/atsh-subprocess'
...
describe('child env layering (through the subprocess seam)', () => {
  ...
  it('forwards explicit ATSH_* config entries to the child', async () => {
    // A deployment sets child-harness facts like ATSH_PERMISSION_MODE in
    ...
    const ctx = await setup({ MOCK_ECHO_ENV: 'ATSH_ACP_TEST_FACT', ATSH_ACP_TEST_FACT: 'managed' })
...
describe('cwd resolution', () => {
  ...
  it('resolves a relative config cwd against the launch directory at load', async () => {
    ...
    // ABSOLUTE form — ATSH's own ACP server rejects a relative session cwd, and
