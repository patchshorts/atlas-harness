...
`SDK` means the JSON-RPC-based client/server protocol used by the supported Python and TypeScript SDKs. The repository keeps `@atlasai/atsh-sdk-client`, `@atlasai/atsh-sdk-protocol`, and the wire identity `deepseek-harness-sdk-runtime`; the JSON-RPC server belongs to the same family. DeepSeek Harness itself is not an SDK, and the removed project generator, launcher, helper, and launcher telemetry packages stay absent.
...
Do not remove an intentional vendor qualifier to avoid repetition. `atsh-subagent-dsh-sdk` names the DeepSeek Harness SDK provider and avoids confusion with another SDK. Its private class becomes `SdkSubagentProvider` because the class also needs to say what it provides.
...
|---|---|---|
| `@atlasai/atsh-jsonrpc` | `@atlasai/atsh-sdk-jsonrpc-server` | It is the server half of the SDK protocol. `jsonrpc` alone names an encoding; `sdk-jsonrpc-server` gives the family, mechanism, and role. |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | The class is one JSON-RPC server implementation, not every possible SDK server. |

Keep `@atlasai/atsh-sdk-client`, `@atlasai/atsh-sdk-protocol`, and `deepseek-harness-sdk-runtime`. Exclude `@deepseek-ai/create-sdk`, `@atlasai/atsh-scripts`, `@atlasai/atsh-helper`, and `@atlasai/atsh-telemetry`; the separate removal decision deletes them and their support graph.
...
| `@atlasai/atsh-bash`, `ctx.bash` | `@atlasai/atsh-shell`, `ctx.shell` | PowerShell already implements this seam. The capability is shell execution, not Bash. |
...
| `@atlasai/atsh-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `@atlasai/atsh-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | The environment registry is shared by Bash and PowerShell tools. |
...
| `@atlasai/atsh-pty`, `ctx.pty`, `PtyService` | `@atlasai/atsh-terminal`, `ctx.terminals`, `TerminalSessionService` | Callers manage multiple named terminal sessions. They do not allocate raw PTYs through this service. |
...
| `@atlasai/atsh-pty-local`, `LocalPtyBackend` | `@atlasai/atsh-terminal-bash`, `BashTerminalBackend` | The provider depends on Bash prompt and shell behavior. `local` hides the actual dialect. |
| `@atlasai/atsh-tool-pty` | `@atlasai/atsh-tool-terminal` | The model-facing tools are already `terminal_*`; the package should use the same product noun. |
...
| Former | Current | Reason |
|---|---|---|
| `@atlasai/atsh-lsp-local` | `@atlasai/atsh-lsp-stdio` | The provider speaks LSP over stdio through replaceable filesystem and subprocess services. It is not necessarily local. |
...
| `@atlasai/atsh-tasks`, `ctx.tasks`, `TaskService` | `@atlasai/atsh-jobs`, `ctx.jobs`, `JobRegistry` | The service registers, owns, observes, waits for, and cancels multiple background jobs. It is a registry, not a general task service. |
...
| `@atlasai/atsh-tasks-local`, `LocalTaskService` | `@atlasai/atsh-jobs-local`, `LocalJobRegistry` | This is the process-local provider of the job registry. Here `local` is meaningful because the jobs and callbacks live in one process. |
| `@atlasai/atsh-tool-tasks` | `@atlasai/atsh-tool-jobs` | The consumer controls the job registry and should use the same domain noun. |
...
| `@atlasai/atsh-client-ui-task`, `client/ui-task/` | `@atlasai/atsh-client-ui-jobs`, `client/ui-jobs/` | The client package presents the background-job collection. It is not one user task. |
...
| Former | Current | Reason |
|---|---|---|
| `@atlasai/atsh-client-ui-slash`, `ui-slash/` | `@atlasai/atsh-client-ui-input-trigger`, `ui-input-trigger/` | The client handles `/`, `@`, keyboard arbitration, candidate menus, and programmatic launch. It is not only slash commands. |
...
| `@atlasai/atsh-agent-tool-mode`, plugin `tool-mode` | `@atlasai/atsh-agent-tool-presentation`, plugin `tool-presentation` | The plugin changes how tools are presented to the model. It does not change execution behavior. Keep local `Config.mode` and `ToolPresentationMode`. |
...
| `@atlasai/atsh-permission`, `ctx.permission`, `PermissionService` | `@atlasai/atsh-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | The service selects and persists presets. Sandbox and approval services enforce the result. |
| `@atlasai/atsh-client-ui-permission` | `@atlasai/atsh-client-ui-permission-presets` | The UI edits and selects permission presets. |
...
| `@atlasai/atsh-user-interaction`, `user-interaction/` | `@atlasai/atsh-user-questions`, `user-questions/` | The seam supports question batches and answers only. Approval, commands, and directory picking are separate interaction seams. |
| `ctx.userInteraction`, `UserInteractionService`, `UserInteractionProvider`, `UserInteractionError` | `ctx.userQuestions`, `UserQuestionService`, `UserQuestionProvider`, `UserQuestionError` | These names state the one supported interaction form. Keep `AskUserQuestion*`, the `ask_user_question` tool, and `@atlasai/atsh-tool-ask-user`. |
...
| Former | Current | Reason |
|---|---|---|
| `packages/typert/type-meta/`, `@atlasai/atsh-type-meta` | `typert/protocol/`, `@atlasai/atsh-typert-protocol` | The package owns the Typert Remote protocol, decorators, bindings, codecs, lookups, and context contracts. It is not generic type metadata. |
...
Keep `@atlasai/atsh-tools` and `ctx.tools`. Keep `@atlasai/atsh-api-gateway`, its `gateway/` folder, `ctx.typertGateway`, and `TypertGatewayService`; that service is a real API gateway. Its internal `TypeRT*` identifiers still follow the `Typert*` spelling rule.

### Workspace instructions, telemetry, identity, and launch environment

| Former | Current | Reason |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` owns multiple workspaces, but Client `ctx.workspaces` already has an incompatible type. Both declarations merge into the same Cordis `Context` interface at compile time even though their runtime contexts are separate. The role suffix states the host service and avoids that collision. Keep `@atlasai/atsh-workspace`, `WorkspaceRegistry`, `Workspace`, and `workspace.*` wire names. |
| `@atlasai/atsh-workspace-context`, `context/workspace-context/` | `@atlasai/atsh-agent-instructions`, `context/agent-instructions/` | The package loads hierarchical `AGENTS.md` and `CLAUDE.md` files for the agent. It is not general workspace context. |
...
| `TelemetryOtel`, `TelemetryMode`, plugin `telemetry-otel` | `OpenTelemetrySessionBackend`, `SessionTelemetryMode`, plugin `session-telemetry-otel` | The provider name states both the OpenTelemetry mechanism and session scope. Keep the package names `atsh-session-telemetry` and `atsh-session-telemetry-otel`. |
...
| `session/user-id/`, `@atlasai/atsh-user-id` | `identity/anonymous-user-id/`, `@atlasai/atsh-anonymous-user-id` | The value is a random correlation id shared by telemetry, feedback, and DeepSeek requests. It is neither a Session concern nor an authenticated user identity. |
...
| `util/environment/`, `@atlasai/atsh-environment` | `util/launch-environment/`, `@atlasai/atsh-launch-environment` | The package captures one immutable layered snapshot at launch. It is not a general environment API. |
...
| Former | Current | Reason |
|---|---|---|
| `@atlasai/atsh-tool-schedule`, `schedule/tool-schedule/`, plugin `tool-schedule` | `@atlasai/atsh-schedule`, `schedule/schedule/`, plugin `schedule` | The package owns the durable Schedule domain, persistence barriers, management tools, timers, follow-ups, and runtime lifecycle. `tool-` describes only one part. |
...
| `WorkflowService`, `ctx.workflows` | `WorkflowEngine`, `ctx.workflowEngine` | One engine parses and executes workflow programs. The plural key wrongly suggests a registry. Keep `@atlasai/atsh-workflow` and workflow events and tools. |
| `@atlasai/atsh-workflow-workerthread`, `WorkerWorkflowEngine` | `@atlasai/atsh-workflow-worker-thread`, `WorkerThreadWorkflowEngine` | `worker thread` is the precise Node mechanism and the repository spelling uses the full words. |
| `@atlasai/atsh-goal-session`, `goal/goal-session/` | `@atlasai/atsh-goal-round-driver`, `goal/goal-round-driver/` | The plugin drives same-session Goal Rounds. It neither stores goals nor defines sessions. Keep `GoalService`, goal source, events, and contracts. |
| `packages/compact/` | `packages/compaction/` | The group is a noun-domain family. `compact` remains the user command verb. |
| `@atlasai/atsh-compact`, `ctx.compact`, `CompactService` | `@atlasai/atsh-compaction`, `ctx.compaction`, `CompactionEngine` | The object runs the compaction algorithm and lifecycle. It is an engine, not a generic service. |
...
| `@atlasai/atsh-compact-basic`, `BasicCompactService`, public `BasicCompact*` | `@atlasai/atsh-compaction-basic`, `BasicCompactionEngine`, corresponding `BasicCompaction*` | `basic` is plain but honest. `compaction-llm` adds no information because LLM use is already part of the current implementation family. |
| `@atlasai/atsh-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `@atlasai/atsh-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | The plugin is an actor that prunes tool results. The noun `pruner` names that role. |
...
| `@atlasai/atsh-settings-local`, `SettingsLocal` | `@atlasai/atsh-settings-file`, `FileSettingsProvider` | The implementation is file-backed through the filesystem seam. `file` states the mechanism; `local` does not. |
...
| Former | Current | Reason |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | The server owns HTTP routes and WebSocket upgrade routes. `Web` leaves room for both; `Http` is too narrow here. Keep `packages/host/webserver`, `@atlasai/atsh-host-webserver`, `WebRoute`, and `WebUpgradeRoute`. |
...
| `@atlasai/atsh-session-title-first-message-llm`, cadence `first-message` | `@atlasai/atsh-session-title-first-prompt-llm`, cadence `first-prompt` | The trigger is the first user prompt, not any message in the session log. |
| `@atlasai/atsh-session-title-all-messages-llm`, cadence `all-user-messages` | `@atlasai/atsh-session-title-all-prompts-llm`, cadence `all-prompts` | The backend refreshes from user prompts. `all messages` wrongly includes assistant and tool events. |
| `@atlasai/atsh-code-runtime-worker`, `WorkerCodeRuntime` | `@atlasai/atsh-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | The implementation uses a Node worker thread. `worker` alone is too broad. |
...
| Former | Current | Reason |
|---|---|---|
| `@atlasai/atsh-fs-policy` | `@atlasai/atsh-fs-observation-policy` | The package defines which filesystem observations authorize later effects. It is not the complete filesystem or sandbox policy. |
...
| `@atlasai/atsh-skill-local`, `LocalSkillProvider`, provider id `local` | `@atlasai/atsh-skill-filesystem`, `FileSystemSkillProvider`, provider id `filesystem` | The provider discovers skill files through `ctx.fs`, which can be local or remote. The mechanism is filesystem access, not locality. |
...
| `@atlasai/atsh-subagent-spawn`, `SpawnProvider` | `@atlasai/atsh-subagent-spawn-in-process`, `SpawnInProcessProvider` | This provider starts a child agent in the current process. The configured provider id remains `spawn`. |
| `@atlasai/atsh-subagent-fork`, `ForkProvider` | `@atlasai/atsh-subagent-fork-in-process`, `ForkInProcessProvider` | This provider forks an agent in the current process. The configured provider id remains `fork`. |
| `@atlasai/atsh-subagent-inprocess`, `subagent-inprocess/` | `@atlasai/atsh-subagent-in-process-driver`, `subagent-in-process-driver/` | The package contains common in-process driving logic, not a third provider. |
| Private `SdkProvider` in `atsh-subagent-dsh-sdk` | `SdkSubagentProvider` | The repeated package qualifier is intentional, and the class must say that it provides subagents through the SDK. |
...
| `@atlasai/atsh-web-fetch-local`, `LocalFetchProvider`, `LocalFetchLimits`, provider id `local-http` | `@atlasai/atsh-web-fetch-http`, `HttpFetchProvider`, `HttpFetchLimits`, provider id `http` | This provider performs direct HTTP fetches. `local` says where code happens to run, not which mechanism it provides. |

Keep `@atlasai/atsh-subagent-dsh-sdk`, its provider id `atsh-sdk`, external ACP, Codex, and Claude Code provider families, the subagent tool package names, the main filesystem package and backends, filesystem tools and events, and the skill badge and tool packages.

### Hooks, guards, plan mode, extensions, and diagnostics

| Former | Current | Reason |
|---|---|---|
| `@atlasai/atsh-hooks-claude`, `ClaudeHookConfig`, `parseClaudeConfig`, dialect `claude` | `@atlasai/atsh-hooks-claude-code`, `ClaudeCodeHookConfig`, `parseClaudeCodeConfig`, dialect `claude-code` | The hook bridge targets Claude Code, not every Anthropic or Claude product. |
| `@atlasai/atsh-repeat-tool-guard`, plugin/source `repeat-tool-guard` | `@atlasai/atsh-repeat-tool-reminder`, plugin/source `repeat-tool-reminder` | The plugin adds a model reminder. It does not block or enforce a guard decision. |
| `@atlasai/atsh-timeout-policy` | `@atlasai/atsh-tool-call-timeout-policy` | The full `tool-call` qualifier names what the policy limits without calling the plugin a model-facing tool. Keep its `guard/timeout-policy/` directory and plugin id `timeout-policy`; the `packages/*/tool-*` catalog convention still applies only to packages that register tools. |
...
| `InvariantService` | `InvariantRegistry` | The object owns registered invariant checks. Keep `@atlasai/atsh-invariants` and `ctx.invariants`. |
...
| Former | Current | Reason |
|---|---|---|
| `util/paths/`, `@atlasai/atsh-paths` | `util/home-paths/`, `@atlasai/atsh-home-paths` | The helpers resolve paths under the Harness home. They are not a general path library. Keep the individual function names when they already state the returned path. |
| `util/retention/`, `@atlasai/atsh-retention` | `util/output-retention/`, `@atlasai/atsh-output-retention` | The policy retains command and tool output. It is not a general data-retention framework. |
| `E2BSandboxService` | `E2BRuntime` | The class creates, reuses, and disposes the E2B execution environment used by filesystem and subprocess adapters. It is broader than one sandbox handle and narrower than a generic owner. Keep `@atlasai/atsh-e2b`, `ctx.e2b`, and the `e2b/` group. |
| `@atlasai/atsh-frontend-static` | `@atlasai/atsh-host-frontend-static` | The package is the Host plugin that serves the frontend assets. The prefix distinguishes it from frontend application code. |
...
| `@atlasai/atsh-jsonrpc-demo` | `@atlasai/atsh-sdk-jsonrpc-demo` | The example demonstrates the runtime SDK over JSON-RPC. It belongs to the one SDK meaning. |
| `@atlasai/atsh-frontend` | `@atlasai/atsh-web-frontend` | The application is the web frontend. Keep its physical `apps/web/` folder. |

Keep atomic-write, brand, native-command, timeout utility, directory-picker, `atsh-base`, `atsh-web-app`, app boot, CLI names, and the `headless` package, bundle, and example identity. `headless` is the intended product essence and may later support more than one-shot execution.
...
| `@atlasai/atsh-client-ui-model` | `@atlasai/atsh-client-ui-model-selection` | The package controls the model selection for a session. The singular `model` name is too broad. |
...
| `@atlasai/atsh-client-ui-models` | `@atlasai/atsh-client-ui-settings-models` | This package owns the Models settings panel. Keep `ModelsSettingsStore`; it holds one settings view model with data operations and subscriptions and is a real store. |
| `@atlasai/atsh-client-ui-plugin-config`, `client/ui-plugin-config/` | `@atlasai/atsh-client-ui-settings-plugins`, `client/ui-settings-plugins/` | This package owns the Plugins settings section, not a general plugin-configuration system. The target joins the `ui-settings-*` family and uses the section's plural product name. |
...
| `@atlasai/atsh-client-ui-plugins`, `client/ui-plugins/`, Loader id `ui-plugins`, `client-ui-plugins-invariant` | `@atlasai/atsh-client-ui-settings-plugin-inventory`, `client/ui-settings-plugin-inventory/`, Loader id `ui-settings-plugin-inventory`, `client-ui-settings-plugin-inventory-invariant` | This later package owns the read-only Plugin Inventory tab in the Plugins settings section. `ui-plugins` is too broad and does not distinguish the inventory from editable plugin settings. |
...
| `@atlasai/atsh-client-ui-question` | `@atlasai/atsh-client-ui-user-questions` | The UI presents the user-question seam, not an arbitrary question domain. |
| `@atlasai/atsh-client-ui-command`, `ui-command/` | `@atlasai/atsh-client-ui-commands`, `ui-commands/` | The package presents and runs a collection of commands. |
| `@atlasai/atsh-client-ui-directory-picker`, `client/ui-directory-picker/`, Loader id `ui-directory-picker`, `client-ui-directory-picker-invariant` | `@atlasai/atsh-client-ui-directory-picker-browse`, `client/ui-directory-picker-browse/`, Loader id `ui-directory-picker-browse`, `client-ui-directory-picker-browse-invariant` | The Client packages now contain separate `browse` and `native` directory-picker presentations. The unqualified package is the browse implementation, not their shared definition. The target matches the Host backend family and changes no boundary. |
...
- Keep `@atlasai/atsh-api-gateway`, `ctx.typertGateway`, and `TypertGatewayService`.
- Keep session projection names. A projection is not only a reducer function.
- Keep `@atlasai/atsh-session-stats`, `sessionStats`, and `SessionStatsProjection`. They accurately name whole-session statistics and the maintained read model that carries them.
...
- Keep `atsh-subagent-dsh-sdk` and provider id `atsh-sdk`; the repeated qualifier prevents ambiguity.
...
- Keep `@atlasai/atsh-client-ui-directory-picker-native`; its suffix names the native-chooser presentation beside the renamed `-browse` variant. Keep `SURFACE_PACKAGES`; within the directory-picker auto selector it is the package map for the Client presentation half, contrasted with `BACKEND_PACKAGES`.
- Keep `@atlasai/atsh-host-plugin-inventory`, `ctx.pluginInventory`, the `pluginInventory/list` Remote, and the `PluginInventory*` payload types. They accurately name the Host-owned read-only inventory; only the adapter class and the overly broad Client presentation names change.
...
**Rename `atsh-compact-basic` to `atsh-compaction-llm`.** Rejected. `LLM` adds no distinction in the current backend family. `basic` is less ambitious and does not claim an algorithm that does not exist.
