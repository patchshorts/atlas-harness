...
import type { PreToolDecision, ToolExecution } from '@atlasai/atsh-tools'
...
Runnable leaves load their plugin trees from `examples/*/cordis.yml`; the root `demo:*` scripts and those leaf directories are the authoritative inventory. The product `atsh` launcher owns Web and one-shot headless execution, ACP leaves use [`@atlasai/atsh-acp-demo`](../../packages/examples/acp-demo), and JSON-RPC leaves use [`@atlasai/atsh-sdk-jsonrpc-demo`](../../packages/examples/jsonrpc-demo). The headless snapshot leaf mounts [`@atlasai/atsh-agent-spine-demo`](../../packages/examples/agent-spine-demo) and JSONL persistence explicitly, then drives them through an example-owned test fixture rather than a shipped app package.
...
| Plan mode | [`@atlasai/atsh-plan-mode`](../../packages/plan/plan-mode/README.md) — logged `plan/mode` state, the `plan:policy` guidance section, `/plan [message]` entry, `/plan off` direct exit, and the user-reviewed `exit_plan_mode` exit; enforcement stays on the independent sandbox/approval axes |
