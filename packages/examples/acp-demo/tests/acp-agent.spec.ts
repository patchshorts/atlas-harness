...
import { agentEvents } from '@atlasai/atsh-agent'
import { TOOL_ORDER_REST } from '@atlasai/atsh-system-prompt'
import type { Message } from '@atlasai/atsh-llm'
import { SessionId } from '@atlasai/atsh-session'
import * as acpAgent from '../src/index.ts'

/**
 * In-process unit coverage for the @atlasai/atsh-acp-demo composition:
...
async function isolatedSkillsConfig(catalogDescriptionMaxLength?: number): Promise<NonNullable<acpAgent.Config['skills']>> {
  ...
  return {
    filesystem: { atshHome: join(home, '.atsh'), agentsHome: join(home, '.agents') },
...
async function withIsolatedSkillHomes<T>(run: () => Promise<T>): Promise<T> {
  ...
  process.env.ATSH_HOME = join(home, '.atsh')
  process.env.ATSH_AGENTS_HOME = join(home, '.agents')
  try {
    return await run()
  } finally {
...
describe('dsh-acp-demo composition', () => {
  ...
  it('forwards skill config and atshHome into agent-spine-demo', async () => {
    const skills = await isolatedSkillsConfig(6)
    const ctx = await mount({ provider: 'mock', model: 'mock', persona: 'hi', atshHome: skills.filesystem!.atshHome!, skills, workspaceContext: false })
