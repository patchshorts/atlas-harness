...
import { boot } from '@atlasai/atsh-app-boot'
...
import type {} from '@atlasai/atsh-fs-e2b'
import type {} from '@atlasai/atsh-bash-local'
import type {} from '@atlasai/atsh-lsp-stdio'
import type {} from '@atlasai/atsh-terminal-bash'
...
try {
  ...
  const sleeping = ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "printf 'ATSH_SLEEP_%s\\n' READY; sleep 30",
    submit: true,
  })
  let sleepReadyOutput = ''
  const sleepReadyDeadline = Date.now() + 5_000
  while (!sleepReadyOutput.includes('ATSH_SLEEP_READY\n')) {
    sleepReadyOutput += sleeping.readOutput().delta
    if (sleepReadyOutput.includes('ATSH_SLEEP_READY\n')) break
  ...
  const stubborn = await ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "bash -c 'trap \"\" TERM; exec sleep 30' & printf 'ATSH_STUBBORN_PID=%s\\n' \"$!\"",
    submit: true,
  }).done
  const stubbornMatch = /ATSH_STUBBORN_PID=([1-9][0-9]*)/.exec(stubborn.viewport)
