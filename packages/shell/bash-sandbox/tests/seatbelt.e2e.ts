...
import { LocalSandboxProvider } from '@atlasai/atsh-sandbox-local'
import { SandboxPolicyService } from '@atlasai/atsh-sandbox-policy'
import { seatbeltProfileArgs } from '@atlasai/atsh-sandbox-local/src/profiles.ts'
 ...
 * confinement is covered by `@atlasai/atsh-sandbox-local`. Skips off macOS or when
...
describe.skipIf(!seatbeltUsable)('bash-sandbox: real Seatbelt confinement through ctx.shell', () => {
  ...
  it('evaluates BASH_ENV only after Seatbelt confines the inner Bash', async () => {
    ...
    await writeFile(hook, [
      'printf hook > "$ATSH_BASH_ENV_INSIDE"',
      'printf escaped > "$ATSH_BASH_ENV_OUTSIDE"',
      '',
    ].join('\n'))
    const bash = await sandboxedBash(workdir, 'workspace-write')
    ...
    await bash.run(bash.resolve({
      ...
      env: { BASH_ENV: hook },
      dshEnv: {
        ATSH_BASH_ENV_INSIDE: insideProbe,
        ATSH_BASH_ENV_OUTSIDE: outsideProbe,
      },
    }))
