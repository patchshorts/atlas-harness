...
npm scope 为 `@atlasai/atsh-*`；Cordis `Service` 子类和函数插件通过 `ctx.effect()`、`ctx.on()` 或 `ctx.waterfall()` 注册。规则见[包](AGENTS.md)与[根规则](../AGENTS.md#conventions)。

## 层级结构

包按组置于 `packages/<group>/<pkg>/`；包名仍为 `@atlasai/atsh-<pkg>`。**组 README 负责包／ctx 键映射。**
...
| [`bundle/`](bundle/README.md) | 可安装的 `atsh --profile` 补丁层 | 产品：稳定 API |
