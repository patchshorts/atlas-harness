/**
 * `atsh plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `atsh.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `atsh.bundle` joins the layer stack; a
 ...
 * gained its `atsh.bundle` declaration in a newer version.
 * @module @atlasai/atsh/plugin
 */
 ...
 * @returns true when the package manifest declares `atsh.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.atsh?.bundle?.patch !== undefined
}

/**
 * Reconcile `atsh.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `atsh.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.atsh?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no atsh.bundle — installed as a plain dependency, not a profile layer `
  ...
  if (!changed) return
  after.atsh = { ...after.atsh, profile: { ...after.atsh?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `atsh` was invoked from.
...
/**
 * Run one `atsh plugin` invocation: init if needed, forward to pnpm, reconcile.
