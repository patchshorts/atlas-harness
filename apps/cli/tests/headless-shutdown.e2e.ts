...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, resolveExampleLaunch } from '@atlasai/atsh-loader-smoke'
...
async function runHeadlessPtySmoke(): Promise<string> {
  ...
  try {
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      ...
      private: true,
      dependencies: {},
      atsh: { profile: { bundles: ['@atlasai/atsh-base', '@atlasai/atsh-headless'] } },
    }, undefined, 2))
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        ATSH_HOME: home,
        ATSH_AGENTS_HOME: join(cwd, '.agents'),
        DEEPSEEK_API_KEY: 'keyless-shutdown-no-call',
        ATSH_TELEMETRY_DISABLED: '1',
        ATSH_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
    ...
    if (result.timedOut) {
      throw new Error(`atsh headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`atsh headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('atsh: observing at ')
