...
describe('loadEnv', () => {
  it('loads variables from .env in the given dir', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'ATSH_APP_BOOT_SPEC_VAR=loaded\n')
    const warn = vi.fn()
    loadEnv(NAME, dir, warn)
    expect(process.env['ATSH_APP_BOOT_SPEC_VAR']).toBe('loaded')
    expect(warn).not.toHaveBeenCalled()
    delete process.env['ATSH_APP_BOOT_SPEC_VAR']
  })
  ...
  it('defaults dir to the process cwd and warn to a stderr write', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'ATSH_APP_BOOT_SPEC_DEFAULTS=yes\n')
    ...
    expect(process.env['ATSH_APP_BOOT_SPEC_DEFAULTS']).toBe('yes')
    delete process.env['ATSH_APP_BOOT_SPEC_DEFAULTS']
...
describe('loadLayeredEnv', () => {
  ...
  it.each([
    ['a harness switch', 'ATSH_PERMISSION_MODE=danger-full-access\n'],
    ['the executable search path', 'PATH=/tmp/evil\n'],
    ['a module preload', 'NODE_OPTIONS=--require /tmp/evil.js\n'],
    ['a skill root', 'ATSH_AGENTS_HOME=/tmp/injected\n'],
...
describe('boot', () => {
  ...
  it('exposes atshHomePath to Loader config expressions', async () => {
    const dir = tmp()
    const atshHome = join(dir, 'home')
    vi.stubEnv('ATSH_HOME', atshHome)
    ...
    writeFileSync(join(dir, 'cordis.yml'), [
      ...
      '  name: ./capture.mjs',
      '  config:',
      "    path: !!js atshHomePath('sessions')",
      '',
    ].join('\n'))
    let ctx: Context | undefined
    try {
      ctx = await boot(NAME, join(dir, 'cordis.yml'))
      expect(ctx.get('capturedPath')).toBe(join(atshHome, 'sessions'))
    } finally {
      await ctx?.fiber.dispose()
      vi.unstubAllEnvs()
    }
...
describe('addHarnessSourceSection', () => {
  ...
  const EXPECTED = `The DeepSeek Harness implementation checkout is at ${SOURCE_ROOT}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend ATSH itself.`
