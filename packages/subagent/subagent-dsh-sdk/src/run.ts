 ...
 * runtime over stdio JSON-RPC through `@atlasai/atsh-sdk-client` and owns
 * cancellation and quiescent disposal. Structure mirrors the ACP backend
 * (`@atlasai/atsh-subagent-acp`): publish after the child handshake,
 ...
 *
 * @module @atlasai/atsh-subagent-dsh-sdk/run
 */

import { randomUUID } from 'node:crypto'
import { DeepSeekHarness, type HarnessNotification } from '@atlasai/atsh-sdk-client'
import type { ContentBlock } from '@atlasai/atsh-llm'
import { SessionId, type SessionEvent, type TurnEndReason } from '@atlasai/atsh-session'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@atlasai/atsh-subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@atlasai/atsh-subagent'
import { scrubbedParentEnv } from '@atlasai/atsh-subprocess'
...
export interface SdkRunSpec {
   ...
   * runtime's own `DEEPSEEK_API_KEY`, or `ATSH_CORDIS_CONFIG`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `ATSH_*` fact survives while ambient namesakes never leak.
   */
