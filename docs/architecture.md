...
There is no privileged core to patch: you extend atsh by mounting a plugin beside the others, and registrations are effects that unwind when their plugin unloads.

## Profiles and bundles

A running `atsh` is a plugin tree composed at boot from ordered layers.
...
Each declares itself in its own `package.json` under a `atsh` field: `atsh.profile` lists a profile's bundles, and `atsh.bundle` points at a bundle's patch file.
