...
} from '@atlasai/atsh-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@atlasai/atsh-typert-protocol'
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@atlasai/atsh-tools', 'ToolInput')).toBe('@atlasai/atsh-tools#ToolInput')
    expect(typertPackageKey('@atlasai/atsh-tools', 'host')).toBe('@atlasai/atsh-tools#host')
    expect(ctx.typert.get('@atlasai/atsh-tools#ToolInput')).toMatchObject({
      package: '@atlasai/atsh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@atlasai/atsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@atlasai/atsh-tools', 'host')).toMatchObject({
      key: '@atlasai/atsh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@atlasai/atsh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@atlasai/atsh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@atlasai/atsh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@atlasai/atsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@atlasai/atsh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@atlasai/atsh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@atlasai/atsh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@atlasai/atsh-tools#ToolInput')).not.toBe(projected)
  })
