...
describe('CI workflow', () => {
  ...
  it('keeps a required Wine Windows job, a non-blocking native Windows job with failover, and a master-only standby', () => {
    ...
    expect(windowsNative['runs-on']).toContain('ATSH_CI_FAILOVER_WINDOWS')
    expect(windowsNative['runs-on']).not.toContain('ATSH_CI_FAILOVER_LINUX')
    ...
    // and the verdict job resolve their pool through ATSH_CI_FAILOVER_LINUX,
    // never the Windows switch.
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers]] as const) {
      expect(typeof job['runs-on']).toBe('string')
      expect(job['runs-on'], `${jobName} runs-on must use the Linux failover switch`).toContain('ATSH_CI_FAILOVER_LINUX')
      expect(job['runs-on'], `${jobName} runs-on must not use the Windows failover switch`).not.toContain('ATSH_CI_FAILOVER_WINDOWS')
      expect(job['runs-on']).toContain('vm-backup')
    }
    expect(aggregate['runs-on']).toContain('ATSH_CI_FAILOVER_LINUX')
    expect(aggregate['runs-on']).not.toContain('ATSH_CI_FAILOVER_WINDOWS')
  ...
  it('is manual-only and fails loud before running the focused live suite', () => {
    ...
    expect(e2b).toMatchObject({
      env: {
        ...
        ATSH_E2E_MAX_WORKERS: '1',
        ATSH_EXAMPLE_MODE: 'lib',
      },
    })
...
describe('Python release workflows', () => {
  ...
  it('exposes the native wheel builder to the release caller with normalized versions', () => {
    ...
    expect(JSON.stringify(manylinuxSmoke)).toContain('-e ATSH_TELEMETRY_DISABLED')
  })
})
