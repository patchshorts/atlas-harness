/**
 * Real-process tests for `@atlasai/atsh-pwsh-local`: the LOCAL subprocess
...
import { PwshLocalExecutor, ENCODING_PREAMBLE, candidatePwshPaths, resolvePwshPath } from '@atlasai/atsh-pwsh-local'
import LocalSubprocessRuntime from '@atlasai/atsh-subprocess-local'
import SubprocessRuntime from '@atlasai/atsh-subprocess'
import type { SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@atlasai/atsh-subprocess'
import { MAX_TIMER_DELAY_MS } from '@atlasai/atsh-timeout'
import type { ShellProcess } from '@atlasai/atsh-shell'
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.run', () => {
  ...
  it('resolve() carries stdin/env/dshEnv onto the spec, and run() threads them to the command', async () => {
    ...
    const spec = bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:SEAM_VAR][$env:ATSH_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
      dshEnv: { ATSH_SEAM_VAR: 'dsh-ok' },
    })
    ...
    expect(spec.dshEnv).toEqual({ ATSH_SEAM_VAR: 'dsh-ok' })
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    ...
    const proc = bash.start(bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:BG_VAR][$env:ATSH_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
      dshEnv: { ATSH_BG_VAR: 'bg-dsh-env' },
    }))
