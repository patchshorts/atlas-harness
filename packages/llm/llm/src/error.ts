 ...
 * Also hosts {@link LlmError} — the package's typed LLM failure class — so
 * cost/budget modules can extend it without importing the service entry point
 * (which would create an ESM cycle once the service wires those modules).
 * @module @atlasai/atsh-llm/error
 */
 ...
 *
 * Lives in `error.ts` (not the service entry point) so cost/budget modules can
 * extend it without importing the entry — the entry imports those modules,
 * which would otherwise be an ESM cycle evaluated at class-definition time.
 */
