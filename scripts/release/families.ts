 ...
 * owns: `atsh` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@atlasai/atsh-root'
...
class DshFamily extends ReleaseFamily {
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`atsh release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@atlasai/atsh', binPath: 'lib/bin.js' }
}
