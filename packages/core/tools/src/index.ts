 ...
 * execution pipeline.
 * @module @atlasai/atsh-tools
 */

import { Context, Service } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@atlasai/atsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@atlasai/atsh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@atlasai/atsh-llm'
import { assertNever, deepFreeze, HarnessError } from '@atlasai/atsh-llm'
import type { Agent } from '@atlasai/atsh-agent'
import { snapshotJsonValue } from '@atlasai/atsh-session'
import type { JsonValue, UserMessage } from '@atlasai/atsh-session'
import type { ToolProviderResult } from '@atlasai/atsh-system-prompt'
import type { CodeRuntime } from '@atlasai/atsh-code-runtime'
...
export type { JsonValue } from '@atlasai/atsh-session'
...
// lives in its own UI-facing module; re-export it so `@atlasai/atsh-tools`
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@atlasai/atsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@atlasai/atsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@atlasai/atsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @atlasai/atsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('atsh.tool.execution') as ToolExecutionToken
}
