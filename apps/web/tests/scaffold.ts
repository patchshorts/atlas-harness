...
// Boots the REAL web composition — the atsh-base and atsh-web-app bundle
...
// downlink, api-gateway, agent loop, tools, and persistence. Modes ride $ATSH_SNAPSHOT:
// replay (default, keyless: normally disables the llm-deepseek row and
// inserts atsh-llm-replay in providers mode), record (real adapter + key,
...
//
// Composition divergences from `atsh web`, all deliberate, all via include
...
import { scrubRequestHeaders, stabilizeFixtureMessageIds } from '@atlasai/atsh-acp-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
  healProfilesModuleFallback,
  loadOverlayPatches,
} from '@atlasai/atsh-app-boot'
import { atshHomePath } from '@atlasai/atsh-home-paths'
import { settingsNamespace } from '@atlasai/atsh-settings'
import { LlmAdapter } from '@atlasai/atsh-llm'
import type {
  LlmModelInfo, LlmProviderInfo, LlmResolvedModelInfo, StreamChunk,
} from '@atlasai/atsh-llm'
import type { ReplayHandle } from '@atlasai/atsh-llm-replay'
import { installLlmReplay, parseSessionLog } from '@atlasai/atsh-llm-replay'
import SessionStore, {
  packChunkRuns,
...
import type {} from '@atlasai/atsh-host-webserver'
import type {} from '@atlasai/atsh-agent'
import { provideCmdline } from '@atlasai/atsh-cmdline'
...
// } from '@atlasai/atsh-client-ui-settings-models'
...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: 'DeepSeek Harness 目前的 0.1 版本仍处在面向 Harness 开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 DeepSeek Harness 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球 Harness 开发者加入 ATSH 插件生态。',
    continueLabel: '继续',
  },
} as const

/** Snapshot mode for the lane, from $ATSH_SNAPSHOT (same vocabulary as the other snapshot suites). */
...
export function webSnapshotMode(): WebSnapshotMode {
  const value = process.env.ATSH_SNAPSHOT
  if (value === undefined || value === '' || value === 'replay') return 'replay'
  if (value === 'record' || value === 'refresh') return value
  throw new Error(`ATSH_SNAPSHOT must be replay, record, or refresh; got ${JSON.stringify(value)}`)
}

/** The shipped composition under test: the atsh-base and atsh-web-app bundle patches over the empty profile root. */
...
export interface WebScaffold {
  ...
  /** Isolated harness home the settings/credentials rows write ($ATSH_HOME double). */
  harnessHome: string
...
export interface LaunchOptions {
  ...
  /**
   * Replay fixture (session.jsonl) served by the inserted atsh-llm-replay row
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const workspaceCwd = await realpath(await mkdtemp(join(tmpdir(), 'atsh-web-e2e-ws-')))
  // Isolated harness home: the settings/credentials rows resolve $ATSH_HOME
  // paths at load, and an in-process boot must NEVER touch the developer's
  // real ~/.atsh document or credential file.
  const harnessHome = options.harnessHome ?? join(workspaceCwd, '.atsh-home')
  ...
  // session is created. Without this a developer's real ~/.atsh/skills silently
  // enters replay requests and goldens while CI sees none. `ATSH_HOME` follows
  ...
  const skillRootEnvironment = {
    ATSH_HOME: harnessHome,
    ATSH_AGENTS_HOME: join(workspaceCwd, '.agents-home'),
    ATSH_BUNDLED_SKILL_DIR: join(workspaceCwd, '.bundled-skills'),
  }
  ...
  try {
    persistenceRoot = await mkdtemp(join(tmpdir(), 'atsh-web-e2e-sessions-'))
  } catch (error) {
  ...
  // (bundle patches in atsh.profile.bundles order), applied over the SAME empty root (a
  ...
  const patches: PatchOptions[] = [
    ...
    // Only the shipped root: a developer's own `~/.atsh/.agent-presets` must not be
    ...
    // storage-json's yml root is anchored to the real $ATSH_HOME; pin the row
    // to an absolute temp root (removed with the workspace at close) so tests
    // never write the user's harness home.
    { id: 'storage-json', config: { root: join(workspaceCwd, '.atsh-storages') } },
    // Skill discovery is model-visible input. Pin every host-level root inside
    // the owned temp world so ~/.atsh, ~/.agents, and a bundled-root env setting
    ...
    // enabled against the same empty temp workspace, preserving the real seam.
    {
      id: 'skill-filesystem',
      config: {
        atshHome: join(workspaceCwd, '.atsh-home'),
    ...
    // to the production OTLP endpoint (or whatever ATSH_TELEMETRY_OTLP_URL
    ...
    // (apps/web IS @atlasai/atsh-web-frontend); only the URL line is silenced.
    ...
    { id: 'settings', config: { atshHome: harnessHome } },
    { id: 'credentials', config: { atshHome: harnessHome } },
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@atlasai/atsh-host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@atlasai/atsh-client-ui-directory-picker-browse' },
    ] },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@atlasai/atsh-tool-cordis' },
      ] }]
      : [],
  ...
  try {
    ...
    ctx.provide('atshHomePath', atshHomePath)
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run ATSH_SNAPSHOT=refresh pnpm run test:web to generate it`)
  }
