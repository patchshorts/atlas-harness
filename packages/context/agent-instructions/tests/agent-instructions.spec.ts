...
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@atlasai/atsh-llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@atlasai/atsh-session'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@atlasai/atsh-agent'
import AgentLoop from '@atlasai/atsh-agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@atlasai/atsh-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
} from '@atlasai/atsh-agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  it('treats ENOTDIR while probing a host candidate as confirmed absence', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd: root, atshHome: homeFile })
  ...
  it('loads user-global first, then every root-to-cwd candidate in precedence order', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd, atshHome: home })
  ...
  it('re-reads content after a same-version, same-size rewrite', async () => {
    ...
    try {
      ...
      expect(await loadBaselineInstructions({ cwd, atshHome: home, maxBytes: 65536 })).toBeUndefined()

      const leaf = join(cwd, 'AGENTS.md')
      await write(leaf, 'first')
      const first = await loadBaselineInstructions({ cwd, atshHome: home, maxBytes: 65536 })
      expect(first?.text).toContain('first')
      const again = await loadBaselineInstructions({ cwd, atshHome: home, maxBytes: 65536 })
      ...
      const second = await loadBaselineInstructions({ cwd, atshHome: home, maxBytes: 65536 })
  ...
  it.skipIf(process.platform === 'win32')('skips a file that becomes unreadable after discovery without failing the request', async () => {
    ...
    try {
      ...
      const loaded = await loadBaselineInstructions({ cwd, atshHome: home, maxBytes: 65536 })
  ...
  it('follows a symlinked instruction file to its target content', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd: root, atshHome: home })
      const loaded = await loadBaselineInstructions({ cwd: root, atshHome: home, maxBytes: 65536 })
  ...
  it('follows a symlinked instruction file through ctx.fs to its target content', async () => {
    ...
    try {
      ...
      await mountWorkspaceContext(ctx, { atshHome: home, maxBytes: 65536 })
  ...
  it('disables baseline loading when the byte budget is zero', async () => {
    ...
    try {
      ...
      await expect(loadBaselineInstructions({ cwd: root, atshHome: home, maxBytes: 0 })).resolves.toBeUndefined()
      await expect(loadBaselineInstructions({ cwd: root, atshHome: home, maxBytes: 65536, maxSourceBytes: 0 })).resolves.toBeUndefined()
      await expect(loadBaselineInstructions({
        cwd: root, atshHome: home, maxBytes: 65536, maxSourceBytes: Infinity,
  ...
  it('defaults atshHome and uses cwd itself as root when no project marker exists', async () => {
    const root = await tempRepo()
    const emptyHome = await tempRepo()
    // Isolate the default-home fallback: blank ATSH_HOME is treated as unset, and
    // the home dirs point at an empty dir so the default ~/.atsh holds no global
    // scope. Windows homedir() reads USERPROFILE (not HOME), so both must be
    // stubbed or a real ~/.atsh/AGENTS.md would otherwise leak in.
    vi.stubEnv('ATSH_HOME', '')
  ...
  it('honors ATSH_HOME when atshHome is not configured explicitly', async () => {
    const root = await tempRepo()
    const envHome = await tempRepo()
    try {
      await write(join(envHome, 'AGENTS.md'), 'env global rule')
      vi.stubEnv('ATSH_HOME', envHome)

      const files = await discoverBaselineInstructionFiles({ cwd: root })

      expect(files).toEqual([{ absolutePath: join(envHome, 'AGENTS.md'), displayPath: '$ATSH_HOME/AGENTS.md' }])
    } finally {
      vi.unstubAllEnvs()
      await rm(root, { recursive: true, force: true })
  ...
  it('labels the default ATSH home as ~/.atsh when HOME points at the configured default', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.dsh/AGENTS.md'), 'global default rule')

      // A set ATSH_HOME would override the homedir default and relabel the home.
      vi.stubEnv('ATSH_HOME', '')
      vi.resetModules()
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@atlasai/atsh-agent-instructions')
      const files = await isolated.discoverBaselineInstructionFiles({ cwd: root })

      expect(files.map(file => file.displayPath)).toEqual(['~/.atsh/AGENTS.md'])
    } finally {
      vi.unstubAllEnvs()
      vi.doUnmock('node:os')
      vi.resetModules()
  ...
  it('expands a configured ~/.atsh home to the operating-system home directory', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      ...
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@atlasai/atsh-agent-instructions')
      const files = await isolated.discoverBaselineInstructionFiles({ cwd: root, atshHome: '~/.atsh' })

      expect(files).toEqual([{ absolutePath: join(home, '.dsh/AGENTS.md'), displayPath: '~/.atsh/AGENTS.md' }])
    } finally {
      vi.doUnmock('node:os')
      vi.resetModules()
      await rm(root, { recursive: true, force: true })
  ...
  it('deduplicates user-global instructions when atshHome points at the project root', async () => {
    const root = await tempRepo()
    try {
      await mkdir(join(root, '.git'), { recursive: true })
      await write(join(root, 'AGENTS.md'), 'same file')

      const files = await discoverBaselineInstructionFiles({ cwd: root, atshHome: root })

      expect(files).toEqual([{ absolutePath: join(root, 'AGENTS.md'), displayPath: '$ATSH_HOME/AGENTS.md' }])
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
...
describe('workspace context request injection', () => {
  ...
  it('preserves a visible baseline when its source is unavailable during resume', async () => {
    ...
    try {
      ...
      await ctx.plugin(workspaceContext, { atshHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
      await composeBaselinePrefix(ctx, original)
  ...
  it('does not promote an unchanged budget-omitted baseline file during resume', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { atshHome: home, maxBytes: 700 })
  ...
  it('recomposes the baseline when candidate precedence changes between resumes', async () => {
    ...
    try {
      ...
      await write(join(root, 'CLAUDE.md'), 'claude rule')
      await mountWorkspaceContext(originalCtx, { atshHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
  ...
  it('reuses an inserted but unadmitted baseline after session recovery and plugin reload', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      const fiber = await mountWorkspaceContext(ctx, { atshHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
  ...
  it.each([
    ...
    try {
      ...
      await resumedCtx.plugin(workspaceContext, { atshHome: home, maxBytes })
      const resumed = stubAgent(root, [...original.session.events])
  ...
  it('reports a changed user-global instruction through the visible-scope reconcile path', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { atshHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
  ...
  it('retains a visible baseline after a plugin remount', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      const fiber = await mountFileToolsAndWorkspaceContext(ctx, { atshHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
  ...
  it('restores a compacted baseline on a hot plugin remount', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await ctx.plugin(LocalFileSystem, { cwd: '/' })
      const fiber = await ctx.plugin(workspaceContext, { atshHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
      const baseline = baselineEvents(agent)[0]
  ...
  it('deduplicates one AGENTS.md that is both user-global and the project-root candidate', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { atshHome: root, maxBytes: 65536 })
      const agent = stubAgent(root)
  ...
  it.each([10, 120])('does not expose state markers when baseline content is omitted at %i bytes', async (maxBytes) => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { atshHome: home, maxBytes })
      const agent = stubAgent(root)
  ...
  it('keeps the direct provider API usable without an operation signal', async () => {
    ...
    try {
      ...
      const rendered = await loadBaselineInstructions({ cwd: root, atshHome: home, maxBytes: 65536 }, fs)
  ...
  it('rejects a provider-sized instruction file before reading content', async () => {
    ...
    try {
      ...
      await ctx.plugin(workspaceContext, { atshHome: home, maxBytes: 65536, maxSourceBytes: 4 })
  ...
  it('does not inject anything when maxBytes is zero', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { atshHome: home, maxBytes: 0 })
      const agent = stubAgent(root)
  ...
  it('does not inject an empty agent-instructions message when maxBytes is negative', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { atshHome: home, maxBytes: -1 })
      const agent = stubAgent(root)
  ...
  it('labels a custom atshHome as ATSH_HOME instead of pretending it is ~/.atsh', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      ...
      expect(files.map(file => file.displayPath)).toEqual(['$ATSH_HOME/AGENTS.md'])
    } finally {
      await rm(root, { recursive: true, force: true })
      await rm(home, { recursive: true, force: true })
    }
  ...
  it('does not repeat a candidate metadata probe during one discovery and read pass', async () => {
    ...
    try {
      ...
      const isolated = await import('@atlasai/atsh-agent-instructions')
      await isolated.loadBaselineInstructions({ cwd: root, atshHome: home, maxBytes: 65536 })
      observedStats.clear()
      await isolated.loadBaselineInstructions({ cwd: root, atshHome: home, maxBytes: 65536 })
  ...
  it('skips an unavailable host candidate but still loads its available sibling', async () => {
    ...
    try {
      ...
      const rendered = await isolated.loadBaselineInstructions({ cwd: root, atshHome: home, maxBytes: 65536 })
...
describe('dynamic nested workspace context injection', () => {
  ...
  it('skips visible baseline scopes when baseline scopes are excluded from reconciliation', async () => {
    ...
    try {
      ...
      agent.session.append('user/message', authoritative, { surfaceOp: 'append' })
      const resolved = resolveConfig({ atshHome: home, maxBytes: 65536, localInstructionFileCandidates: [] })
      const cache: InstructionVersionCache = new WeakMap()
  ...
  it('does not mark omitted nested files as pending-loaded', async () => {
    ...
    try {
      ...
      await write(join(root, 'pkg/sub/file.txt'), 'subtree file')
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { atshHome: home, maxBytes: 700 })
      const agent = stubAgent(root)
      await ctx.tools.execute({
        signal: testToolSignal,
  ...
  it('does not attach nested instructions when the byte budget is disabled', async () => {
    ...
    try {
      ...
      await write(join(root, 'pkg/deep/file.txt'), 'hello')
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { atshHome: home, maxBytes: 0 })
  ...
  it('retries a nested instruction touch when only a truncated budget notice was rendered', async () => {
    ...
    try {
      ...
      fs.entries.set(join(root, 'pkg/file.txt'), { type: 'file', content: 'hello' })
      await ctx.plugin(ToolFs)
      await ctx.plugin(workspaceContext, { atshHome: home, maxBytes: 20 })
      const agent = stubAgent(root)
...
describe('workspace context inbox synchronization', () => {
  ...
  it('holds back a dynamic change a one-byte positive render budget cannot represent', async () => {
    ...
    try {
      ...
      await ctx.plugin(workspaceContext, { atshHome: home, maxBytes: 1 })
      const agent = stubAgent(root)
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal,
