...
import type { ShellRunResult, CollectedOutput } from '@atlasai/atsh-shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@atlasai/atsh-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@atlasai/atsh-sandbox'
import { SandboxPolicyService } from '@atlasai/atsh-sandbox-policy'
import { SandboxBashExecutor } from '@atlasai/atsh-bash-sandbox'
...
import type { Config } from '@atlasai/atsh-bash-sandbox'
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'ATSH_WRAP=1', 'bash', '-c', 'printf "%s" "$ATSH_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$ATSH_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.ATSH_ORDER_FILE, "runner\\n");',
