...
Implementations: [`@atlasai/atsh-sandbox-local`](../sandbox-local/) (Linux: `bwrap`, else the per-platform Landlock launcher; macOS: `sandbox-exec`/Seatbelt). Consumers: [`@atlasai/atsh-bash-sandbox`](../../shell/bash-sandbox/) (wraps `['bash', '-c', command]`).
