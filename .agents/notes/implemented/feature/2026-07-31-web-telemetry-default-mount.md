# Agent Note: Default session-telemetry mount (OTel reporting) in the atsh web composition
...
- A developer running `atsh web` without telemetry configuration makes no telemetry network request. An internal deployment sets `DSH_TELEMETRY_MODE` and may point `DSH_TELEMETRY_OTLP_URL` at another collector.
