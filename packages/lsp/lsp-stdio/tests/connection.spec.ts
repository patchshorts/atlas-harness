...
import { LspConnection } from '@atlasai/atsh-lsp-stdio'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit ATSH_* env entries to the child', async () => {
    // A configured ATSH_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'ATSH_LSP_TEST_FACT', ATSH_LSP_TEST_FACT: 'managed' })
