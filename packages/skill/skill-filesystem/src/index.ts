 ...
 *
 * @module @atlasai/atsh-skill-filesystem
 */
...
import type { FileSystem, FsDirEntry, FsTarget } from '@atlasai/atsh-fs'
...
import {
  ...
  type SkillSource,
} from '@atlasai/atsh-skill'

const PROJECT_ATSH_RANK = 100
const PROJECT_AGENTS_RANK = 200
const CUSTOM_RANK = 300
const USER_ATSH_RANK = 400
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  constructor(
    ...
    this.agentsHome = resolve(config.agentsHome ?? process.env.ATSH_AGENTS_HOME ?? join(homedir(), '.agents'))
    ...
    const bundledSkillDir = config.bundledSkillDir
      ?? (this.includeDefaultRoots ? process.env.ATSH_BUNDLED_SKILL_DIR : undefined)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.dsh/skills'), source: 'project-dsh', rank: PROJECT_ATSH_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.atshHome, 'skills'), source: 'user-dsh', rank: USER_ATSH_RANK, skipSystem: true },
