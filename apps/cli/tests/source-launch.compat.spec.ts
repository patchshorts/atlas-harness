...
/**
 * Keyless smoke for SOURCE `atsh` execution: run `apps/cli/src/bin.ts`
 * with the exact production runtime vector (`node --import tsx/esm`, the
 * vector the root `atsh` script invokes directly) and assert the
 ...
 * breaks this gate instead of every developer's `pnpm atsh`; the built-bin
...
describe('atsh SOURCE launcher (node --import tsx/esm)', () => {
  it('launches the source CLI without building', async () => {
    const rootPackage = JSON.parse(await readFile(new URL('../../../package.json', import.meta.url), 'utf8')) as {
      readonly scripts?: Record<string, string>
    }
    expect(rootPackage.scripts?.atsh).toBe('node --import tsx/esm apps/cli/src/bin.ts')
  })
  ...
  it('boots the source entry and requires a profile', async () => {
    ...
    if (result.timedOut) {
      throw new Error(`atsh source launch did not exit within 25s. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
