...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@atlasai/atsh-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@atlasai/atsh-tools'
import type { Agent } from '@atlasai/atsh-agent'
import { Session, SessionId } from '@atlasai/atsh-session'
import type { JsonValue, SessionEventMap } from '@atlasai/atsh-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@atlasai/atsh-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
