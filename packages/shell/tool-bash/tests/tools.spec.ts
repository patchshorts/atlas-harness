...
import { ShellExecutor } from '@atlasai/atsh-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult } from '@atlasai/atsh-shell'
import SystemPrompt from '@atlasai/atsh-system-prompt'
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@atlasai/atsh-tools'
...
import ApprovalService from '@atlasai/atsh-user-approval'
import type { ApprovalOutcome } from '@atlasai/atsh-user-approval'
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @atlasai/atsh-jobs and @atlasai/atsh-tool-jobs')
  })
...
describe('the model-facing bash tool builds its request from named args only (no {...args} forward)', () => {
  ...
  async function setupRecording(withJsonl = false) {
    ...
    await ctx.plugin(LocalJobRegistry)
    await ctx.plugin(ToolTasks)
    await ctx.plugin(BashEnvPlugin, { atshHome: recordingDshHome })
  ...
  it('describes the managed harness environment namespace to the model', async () => {
    ...
    expect(description).toContain('$ATSH_*')
    expect(description).not.toContain('ATSH_SESSION_JSONL')
  })
  ...
  it('injects the session id and JSONL target path into a foreground request', async () => {
    ...
    expect(bash.requests[0]?.dshEnv).toEqual({
      ATSH_HOME: recordingDshHome,
      ATSH_SESSION_ID: 'request-fg',
      ATSH_SESSION_JSONL: path,
      ATSH_SHELL: '1',
    })
  })
  ...
  it('injects the same trusted variables into a background request without forwarding model env', async () => {
    ...
    await ctx.tools.execute({
      ...
      arguments: {
        ...
        env: { ATSH_SESSION_ID: 'spoofed', ATSH_SESSION_JSONL: '/tmp/spoofed' },
      },
      agent,
    })
  ...
  it('injects built-ins and the stable session id when no JSONL locator is available', async () => {
    ...
    const ambient = process.env.ATSH_SESSION_ID
    ...
    expect(bash.requests[0]?.dshEnv).toEqual({
      ATSH_HOME: recordingDshHome,
      ATSH_SESSION_ID: 'request-id-only',
      ATSH_SHELL: '1',
    })
    expect(process.env.ATSH_SESSION_ID).toBe(ambient)
  })
  ...
  it('keeps parent and child agent session environments isolated', async () => {
    ...
    expect(bash.requests.map(request => request.dshEnv)).toEqual([
      {
        ATSH_HOME: recordingDshHome,
        ATSH_SESSION_ID: 'request-parent',
        ATSH_SESSION_JSONL: ctx.sessionPersistence.locate(parent.session.header)?.path,
        ATSH_SHELL: '1',
      },
      {
        ATSH_HOME: recordingDshHome,
        ATSH_SESSION_ID: 'request-child',
        ATSH_SESSION_JSONL: ctx.sessionPersistence.locate(child.session.header)?.path,
        ATSH_SHELL: '1',
      },
    ])
    expect(bash.requests[0]?.dshEnv?.ATSH_SESSION_JSONL).not.toBe(bash.requests[1]?.dshEnv?.ATSH_SESSION_JSONL)
  })
