/**
 * Scripted fake ACP agent bin for `atsh-acp-snapshot`'s unit specs. Speaks
 ...
 * `behavior.json` sitting NEXT to the `$ATSH_SNAPSHOT_FILE` fixture, so a spec
...
interface ScriptedLog {
  /** Path relative to `$ATSH_SNAPSHOT_SESSIONS_ROOT`, e.g. `project/session/session.jsonl`. */
  file: string
  /**
...
interface Behavior {
  ...
  /** Echo the `ATSH_SNAPSHOT_*` env the harness set as a chunk (spec-side env-plumbing assertions). */
  echoEnv?: boolean
...
const sessionsRoot = process.env.ATSH_SNAPSHOT_SESSIONS_ROOT ?? ''
const fixtureFile = process.env.ATSH_SNAPSHOT_FILE ?? ''
...
async function handlePrompt(id: number | string): Promise<void> {
  ...
  if (behavior.echoEnv === true) {
    chunk(`env:${JSON.stringify({
      mode: process.env.ATSH_SNAPSHOT,
      override: process.env.ATSH_SNAPSHOT_OVERRIDE ?? null,
      childFiles: process.env.ATSH_SNAPSHOT_CHILD_FILES ?? null,
      spillRoot: process.env.ATSH_SNAPSHOT_SPILL_ROOT ?? null,
      // Scenario-supplied deployment env (the `Scenario.env` layering hook).
      permissionMode: process.env.ATSH_PERMISSION_MODE ?? null,
    })}`)
  }
