...
import type { CommandHandle, CommandResult, Sandbox } from '@atlasai/atsh-e2b'
import type {
  SubprocessCollect,
  SubprocessHandle,
  SubprocessOutcome,
...
function commandText(spec: SubprocessSpawnSpec, paths: RemotePaths): string {
  const encoder = `"$atsh_e2b_env_bin" -i "$atsh_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$atsh_e2b_tee" --output-error=warn-nopipe >("$atsh_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$atsh_e2b_tee" --output-error=warn-nopipe >("$atsh_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
  ...
  const inner = [
    ...
    'atsh_e2b_rm=$7',
    'shift 7',
    'atsh_e2b_pgid="$("$atsh_e2b_ps" -o pgid= -p "$$" | "$atsh_e2b_tr" -d " ")"',
    `printf '%s\\n' "$atsh_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t atsh_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$atsh_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$atsh_e2b_env_bin" -i -- "\${atsh_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'atsh_e2b_status=$?',
    `printf '%s\\n' "$atsh_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$atsh_e2b_status"',
  ].join('\n')
  const argv = spec.argv.map(quoteE2BShellArg).join(' ')
  const bootstrap = [
    `mapfile -d '' -t atsh_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'atsh_e2b_env_bin="$(command -v env)"',
    'atsh_e2b_setsid="$(command -v setsid)"',
    'atsh_e2b_bash="$(command -v bash)"',
    'atsh_e2b_node="$(command -v node)"',
    'atsh_e2b_ps="$(command -v ps)"',
    'atsh_e2b_tr="$(command -v tr)"',
    'atsh_e2b_tee="$(command -v tee)"',
    'atsh_e2b_head="$(command -v head)"',
    'atsh_e2b_rm="$(command -v rm)"',
    'for atsh_e2b_tool in "$atsh_e2b_env_bin" "$atsh_e2b_setsid" "$atsh_e2b_bash" "$atsh_e2b_node" "$atsh_e2b_ps" "$atsh_e2b_tr" "$atsh_e2b_tee" "$atsh_e2b_head" "$atsh_e2b_rm"; do',
    '  [[ "$atsh_e2b_tool" == /* && -x "$atsh_e2b_tool" ]] || exit 125',
    'done',
    `exec "$atsh_e2b_env_bin" -i -- "\${atsh_e2b_env[@]}" "$atsh_e2b_setsid" --wait -- "$atsh_e2b_bash" -c ${quoteE2BShellArg(inner)} atsh-e2b "$atsh_e2b_env_bin" "$atsh_e2b_node" "$atsh_e2b_ps" "$atsh_e2b_tr" "$atsh_e2b_tee" "$atsh_e2b_head" "$atsh_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}
