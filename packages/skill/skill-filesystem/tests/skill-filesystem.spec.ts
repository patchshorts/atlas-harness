...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@atlasai/atsh-fs'
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.dsh/skills'), 'same', 'user atsh skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.dsh/skills'), 'same', 'project atsh skill')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project atsh skill')
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { atshHome: join(home, '.atsh'), agentsHome: join(home, '.agents'), watch: false })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousDshHome = process.env.ATSH_HOME
    const previousAgentsHome = process.env.ATSH_AGENTS_HOME
    const previousBundledSkillDir = process.env.ATSH_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.ATSH_HOME = join(envHome, '.atsh')
      process.env.ATSH_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.ATSH_BUNDLED_SKILL_DIR = bundled
      ...
      process.env.ATSH_HOME = join(envHome, 'empty-dsh')
      delete process.env.ATSH_BUNDLED_SKILL_DIR
      process.env.ATSH_AGENTS_HOME = join(envHome, 'empty-agents')
      ...
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        ...
        invalidate() {},
      }, { atshHome: join(envHome, 'empty-dsh') }).name).toBe('filesystem')
    } finally {
      if (previousDshHome === undefined) {
        delete process.env.ATSH_HOME
      } else {
        process.env.ATSH_HOME = previousDshHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.ATSH_AGENTS_HOME
      } else {
        process.env.ATSH_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.ATSH_BUNDLED_SKILL_DIR
      } else {
        process.env.ATSH_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
