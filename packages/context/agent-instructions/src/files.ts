 ...
 *
 * @module @atlasai/atsh-agent-instructions/files
 */

import { createReadStream } from 'node:fs'
import { stat } from 'node:fs/promises'
import { dirname, isAbsolute, join, relative, resolve } from 'node:path'
import type { FileSystem, FsInfo, FsTarget, FsVersion } from '@atlasai/atsh-fs'
import { assertNever } from '@atlasai/atsh-llm'
import { atshHomeDisplay } from '@atlasai/atsh-home-paths'
...
async function discoverInstructionFiles(
  ...
  const userGlobal = join(config.atshHome, USER_GLOBAL_FILE)
  ...
  switch (userGlobalProbe.kind) {
    case 'present':
      addFile({
        ...
        displayPath: userGlobalDisplayPath(config.atshHome),
...
export async function probeScopeInstruction(
  ...
  const file: ProbedInstructionFile = {
    absolutePath,
    displayPath: directory === USER_GLOBAL_DIRECTORY ? userGlobalDisplayPath(resolved.atshHome) : relativeDisplay(projectRoot, absolutePath),
    target,
...
function userGlobalDisplayPath(atshHome: string): string {
  return `${atshHomeDisplay(atshHome)}/AGENTS.md`
}
