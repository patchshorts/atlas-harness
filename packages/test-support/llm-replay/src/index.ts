 ...
 * @module @atlasai/atsh-llm-replay
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs'
import { delimiter as pathDelimiter } from 'node:path'
import type { Context } from '@deepseek-ai/cordis'
import type {} from '@atlasai/atsh-compaction'
import { decodeStorageRecord } from '@atlasai/atsh-session'
...
import { LlmAdapter, LlmError, ReasoningEffortId, assertNever, resolveRetryPolicy } from '@atlasai/atsh-llm'
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.ATSH_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $ATSH_SNAPSHOT_FILE)')
  }
  validateConfiguredModalities(config.providers)
  const overrideFile = config.overrideFile ?? process.env.ATSH_SNAPSHOT_OVERRIDE
  const childEnv = process.env.ATSH_SNAPSHOT_CHILD_FILES
