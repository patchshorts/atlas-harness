...
- `@atlasai/atsh-shell-env` is a new shipped package; `atsh-tool-bash`'s `dshHome` config moved there, so compositions mounting the shell tools must also mount `shell-env` (the spine bundles do).
