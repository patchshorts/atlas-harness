 ...
 * leaf-level diff.
 * @module @atlasai/atsh-settings-file
 */
...
import { SettingsProvider, deepEqualJson, type SettingsNamespace } from '@atlasai/atsh-settings'
...
export function resolveSpec(config: Config): ResolvedSpec {
  const filename = resolve(config.path ?? join(resolveDshHome(config.atshHome), 'settings.yaml'))
