 ...
 * zero-build dev path, resolving `@atlasai/atsh-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @atlasai/atsh-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'ATSH_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.ATSH_EXAMPLE_MODE`.
...
export function resolveExampleLaunch(options: ExampleLaunchOptions): ExampleLaunch {
  ...
  // The real-loadable app boots SQLite-backed services; on Node 22+ the VM
  // warns on stderr, polluting captured replay output. Suppress it for every
  // example spawn (mirrors the headless/SDK snapshot configs) so keyless
  // snapshot replays are byte-stable across Node versions.
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated ATSH homes. */
...
export async function runLoaderSmoke(options: LoaderSmokeOptions): Promise<LoaderSmokeResult> {
  ...
  try {
    ...
    const launch = resolveExampleLaunch({
      ...
      env: { ATSH_HOME: join(cwd, '.atsh'), ATSH_AGENTS_HOME: join(cwd, '.agents'), ...options.env },
    })
