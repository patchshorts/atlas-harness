...
import { getOrCreateAnonymousUserId } from '@atlasai/atsh-anonymous-user-id'
import Loader from '@deepseek-ai/cordis-plugin-loader'
import { recordFeedback } from '@atlasai/atsh-command-feedback'
...
// pin ATSH_HOME to a temp dir so the suite never touches the ambient ~/.atsh.
let tempHome: string
let previousDshHome: string | undefined
beforeAll(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'dsh-otel-home-'))
  previousDshHome = process.env.ATSH_HOME
  process.env.ATSH_HOME = tempHome
})
afterAll(() => {
  if (previousDshHome === undefined) delete process.env.ATSH_HOME
  else process.env.ATSH_HOME = previousDshHome
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships session records and the ops shutdown marker through the real SDK pipeline', async () => {
    ...
    const ledger = records.filter(r => r.scope === '@atlasai/atsh-session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@atlasai/atsh-session-telemetry-otel/ops')
  ...
  it('drains records enqueued after a timer export began: dispose during an in-flight batch', async () => {
    ...
    const ops = allRecords(captures).filter(r => r.scope === '@atlasai/atsh-session-telemetry-otel/ops')
    expect(ops).toHaveLength(1)
    expect(ops[0]!.record.attributes).toContainEqual({ key: 'telemetry.op', value: { stringValue: 'shutdown' } })
  })
