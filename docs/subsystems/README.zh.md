...
| [atlas-additions.md](atlas-additions.md) | Atlas 增量能力族：`ctx.memoryStore` / `llmRouter` / `llmCache` / `kgraph` / `accounting` / `coordination` / `research` / `factory` 服务键及其事件作用域 |
...
| [subprocess.md](subprocess.md) | 子进程 seam：完全显式的 `SubprocessSpawnSpec`、基于偏移的输出读取器、不含分类的 `SubprocessOutcome`，以及受管 `ATSH_*` 环境词汇 |
...
| [client-modules.md](client-modules.md) | Web 插件表：`atsh.client` 声明、`WebBootGraph` 线上组合、bundle 路由与 index 转换 |
