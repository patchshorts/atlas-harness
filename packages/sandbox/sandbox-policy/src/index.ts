 ...
 * remain session-free.
 *
 * @module @atlasai/atsh-sandbox-policy
 */
...
import { canonicalPath, type SandboxExecutionPolicy, type SandboxMode } from '@atlasai/atsh-sandbox'
...
function renderPolicyContext(policy: SandboxExecutionPolicy): string {
  switch (policy.mode) {
    case 'read-only':
      return 'Current ATSH file policy: read-only. Any available operation enforced by the ATSH file sandbox cannot modify files in the standing mode. Do not refuse a required modification from this policy alone: try an available tool normally and follow any denial and escalation guidance it returns.'
    case 'workspace-write':
      return `Current ATSH file policy: workspace-write. Any available operation enforced by the ATSH file sandbox may modify files under the session workspace: ${JSON.stringify(policy.workspaceRoot)}. Some platform temporary areas may also be writable.`
    case 'danger-full-access':
      return 'Current ATSH file policy: danger-full-access. The ATSH file sandbox does not restrict file modifications by available operations.'
