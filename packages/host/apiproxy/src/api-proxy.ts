...
import { installModelSelection } from '@atlasai/atsh-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@atlasai/atsh-agent'
import type {} from '@atlasai/atsh-agent-presets/types'
import { AttachmentError } from '@atlasai/atsh-attachment'
import type { ImageAttachmentRef } from '@atlasai/atsh-attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@atlasai/atsh-llm'
import { errorChain } from '@atlasai/atsh-llm'
import type { ContentBlock, MessageSource } from '@atlasai/atsh-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@atlasai/atsh-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@atlasai/atsh-session'
import type { SessionPersistence } from '@atlasai/atsh-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@atlasai/atsh-session-query'
import { SubagentError } from '@atlasai/atsh-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@atlasai/atsh-subagent'
import { isUserInvocable } from '@atlasai/atsh-skill'
import type { Workspace, WorkspaceRecord } from '@atlasai/atsh-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@atlasai/atsh-workspace'
...
} from '@atlasai/atsh-agent-presets'
import type { PresetBearingSession } from '@atlasai/atsh-agent-presets'
...
import type { SessionRawArtifact } from '@atlasai/atsh-session-persistence'
import {
...
import type {} from '@atlasai/atsh-jobs'
import type { JobSnapshot } from '@atlasai/atsh-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@atlasai/atsh-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@atlasai/atsh-goal'
import type { GoalRef as CoreGoalRef } from '@atlasai/atsh-goal'
...
import type {} from '@atlasai/atsh-cordis-host-runner/types'
import type {} from '@atlasai/atsh-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@atlasai/atsh-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@atlasai/atsh-settings'
import { credentialRef } from '@atlasai/atsh-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@atlasai/atsh-session-title'
import type { CallId } from '@atlasai/atsh-llm/brand'
import type { ScopeKey } from '@atlasai/atsh-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@atlasai/atsh-user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@atlasai/atsh-user-approval'
...
} from '@atlasai/atsh-user-questions'
import { UserQuestionError } from '@atlasai/atsh-user-questions'
import { DirectoryPickerError } from '@atlasai/atsh-host-directory-picker'
import {
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @atlasai/atsh-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @atlasai/atsh-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @atlasai/atsh-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @atlasai/atsh-credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @atlasai/atsh-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @atlasai/atsh-skill', details: {} })
        }
