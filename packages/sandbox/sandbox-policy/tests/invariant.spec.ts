...
import SessionStore, { type Session, type SessionEvent } from '@atlasai/atsh-session'
import InvariantRegistry, { InvariantError } from '@atlasai/atsh-invariants'
import * as SandboxPolicyInvariant from '@atlasai/atsh-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@atlasai/atsh-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@atlasai/atsh-sandbox-policy',
    })
  })
})
