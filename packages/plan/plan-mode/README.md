...
When the deployment composes [`@atlasai/atsh-judge-gate`](../../guard/judge-gate/README.md), `exit_plan_mode` runs the three-panel judge at plan admission — before the user reviews the plan. A NOT PASS verdict fails the call with the ballot reasons, so the model revises and re-presents; the panel's replan budget (N≤2 per judgment) bounds the loop. Compositions without the gate keep the current behavior, and the gate never reads or mutates session log, message history, or projections — plan artifacts only.
