...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@atlasai/atsh-tool-call-timeout-policy` wrapper.
