...
import type { TypertContribution } from '@atlasai/atsh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@atlasai/atsh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    ...
    const artifact = new FaceModelEmitter(host).emit('@atlasai/atsh-tools')
    ...
    const record = ctx.typert.getPackage('@atlasai/atsh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@atlasai/atsh-tools', 'host')).toBeUndefined()
  })
})
