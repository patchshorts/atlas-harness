 ...
 * @module @atlasai/atsh-jobs-local
 */

import { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import type { Agent } from '@atlasai/atsh-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@atlasai/atsh-scope'
import type { ScopeLayer } from '@atlasai/atsh-scope'
import { deadline, timeoutOf } from '@atlasai/atsh-timeout'
import { JobRegistry, JobId } from '@atlasai/atsh-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@atlasai/atsh-jobs'
 ...
 * `@atlasai/atsh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @atlasai/atsh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @atlasai/atsh-agent)')
    }
