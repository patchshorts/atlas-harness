...
const RELEASE_ENTRY_PACKAGE = '@atlasai/atsh'
...
while time.monotonic() < deadline:
    ...
    if not termination_sent and b"atsh web: http://127.0.0.1:" in snapshot:
        ready_seen = True
...
if not ready_seen:
    sys.stderr.write("installed atsh web did not reach its ready URL\n")
    sys.exit(124)
actual_exit = os.waitstatus_to_exitcode(status)
if actual_exit != 0:
    sys.stderr.write(f"installed atsh web exited {actual_exit}, expected 0\n")
    sys.exit(125)
`
...
/** Installs one complete bundle outside the workspace and probes the shipped atsh entry. */
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@atlasai/atsh/lib/bin.js')
      assertPathWithin(consumerRoot, bin, 'installed atsh bin')
      ...
      if (version !== this.bundle.manifest.version) {
        throw new Error(
          `installed atsh --version returned ${JSON.stringify(version)}; `
          + `expected ${this.bundle.manifest.version}`,
        )
      }
      this.probeWeb(bin, consumerRoot, environment)
      console.log('publish-npm-baseline: installed atsh entry and Web startup probes passed')
    } finally {
  ...
  private probeWeb(bin: string, consumerRoot: string, environment: NodeJS.ProcessEnv): void {
    if (process.platform === 'win32') {
      throw new Error('installed atsh Web probe requires a POSIX host with python3')
    }
    const result = this.runner.result(
      'python3',
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@deepseek-ai/') || name === '@atlasai/atsh-root')) {
...
function installedArtifactEnvironment(consumerRoot: string): NodeJS.ProcessEnv {
  ...
  environment.ATSH_HOME = resolve(consumerRoot, '.atsh')
  environment.ATSH_AGENTS_HOME = resolve(consumerRoot, '.agents')
  environment.ATSH_TELEMETRY_DISABLED = '1'
