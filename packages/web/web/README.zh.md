...
| 包 | 职责 |
|---|---|
| `@atlasai/atsh-web`（本包） | Service Definition：服务、提供方注册表、选择策略、请求／结果词汇、`WebError` 分类体系 |
| `@atlasai/atsh-web-search-exa` | 搜索提供方：Exa |
| `@atlasai/atsh-web-search-perplexity` | 搜索提供方：Perplexity |
| `@atlasai/atsh-web-fetch-http` | 抓取提供方：匿名公共 HTTP(S) |
| `@atlasai/atsh-tool-web` | Consumer：面向模型的 `web_search`／`web_fetch` 工具 schema，构建于 `ctx.web` 之上 |
...
选择绝不依赖注册、配置或 HMR（热模块替换）顺序。能力要么具有显式提供方 id（配置 `searchProvider`／`fetchProvider`，或由环境变量 `$ATSH_WEB_SEARCH_PROVIDER`／`$ATSH_WEB_FETCH_PROVIDER` 提供相同字段），要么在恰好只注册一个可用提供方时自动选择。`search()`／`fetch()` 会在执行时解析提供方：
