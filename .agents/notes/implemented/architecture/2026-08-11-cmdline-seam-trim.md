...
- **Keeping `enableRuntime` but moving `enableRow` out of `atsh-cmdline`**: relocation fixes the package boundary but keeps the vendored state machine whose semantics (survives reapplication, rollback on failure) must be re-derived at every upstream sync.
...
- A deployment that supervises `atsh --profile headless` with SIGTERM now observes exit 0 instead of 143; the caller sent the signal and sees no answer on stdout.
- The reload chain runs in every `atsh web` process; a deployment that must not expose `/plugins/events` disables the `client-hmr` row in its patch layer.
