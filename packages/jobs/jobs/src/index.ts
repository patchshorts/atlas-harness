 ...
 * process-local registry lives in `@atlasai/atsh-jobs-local`.
 * @module @atlasai/atsh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@atlasai/atsh-jobs is the abstract job registry seam; load an implementation such as @atlasai/atsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
