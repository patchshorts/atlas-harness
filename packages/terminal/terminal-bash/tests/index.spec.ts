...
import AgentRegistry, { Inbox, type Agent } from '@atlasai/atsh-agent'
import SandboxProvider from '@atlasai/atsh-sandbox'
import type { ConfinedArgv, SandboxPolicy } from '@atlasai/atsh-sandbox'
import SandboxPolicyService, { setSandboxMode } from '@atlasai/atsh-sandbox-policy'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@atlasai/atsh-terminal'
import { BashTerminalBackend } from '@atlasai/atsh-terminal-bash'
import * as ptyLocal from '@atlasai/atsh-terminal-bash'
import type { ResolvedConfig } from '@atlasai/atsh-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@atlasai/atsh-terminal-bash/src/session.ts'
import { SubprocessRuntime } from '@atlasai/atsh-subprocess'
import type {
  SubprocessHandle,
  SubprocessSpawnSpec,
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'atsh> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        ATSH_SHELL: '1', ATSH_SESSION_ID: 'agent', ATSH_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('composes the default local session around a spawned terminal', async () => {
    ...
    queueMicrotask(() => { output.write(Buffer.from('\x1b]133;D;0\x07atsh> ')) })
    const backend = new BashTerminalBackend(
      ctx,
      config(),
      async () => terminal,
