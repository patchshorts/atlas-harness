 ...
 * @module @atlasai/atsh-agent-presets
 */

import { stat } from 'node:fs/promises'
import { Context, Service } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { bindScopeParent, createScope, scopeOf, type Scope, type ScopeKey, type ScopeParentBinding } from '@atlasai/atsh-scope'
// Type-only: resolves the `agent/created` lifecycle event this service watches.
import type {} from '@atlasai/atsh-agent'
import { settingsNamespace, type SettingsScope, type default as SettingsService } from '@atlasai/atsh-settings'
...
export class AgentPresets extends Service {
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    this.resolvedRoots = config.includeUserRoot
      ? [...config.roots, { path: atshHomePath(USER_PRESET_DIR), trust: 'user' }]
      : [...config.roots]
