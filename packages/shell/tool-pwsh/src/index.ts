 ...
 * `@atlasai/atsh-pwsh-local`) backs `ctx.shell`; the tool contract is
 ...
 * generic `ctx.jobs` runtime), the managed `ATSH_*` environment through the
 ...
 * exit-status parse from `@atlasai/atsh-shell`.
 *
 * @module @atlasai/atsh-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { defineTool, TOOL_ABORTED } from '@atlasai/atsh-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@atlasai/atsh-tools'
...
import type { SandboxExecutionPolicy, SandboxMode } from '@atlasai/atsh-sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@atlasai/atsh-sandbox'
import type { SandboxPolicyService } from '@atlasai/atsh-sandbox-policy'
import type { ShellRunResult } from '@atlasai/atsh-shell'
import { parseExitStatus } from '@atlasai/atsh-shell'
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Current harness environment facts are exposed through managed `$env:ATSH_*` variables; inspect them when needed. '
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: PwshToolArgs, exec) {
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @atlasai/atsh-jobs and @atlasai/atsh-tool-jobs')
        }
