...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__ATSH_BOOT__. '
  + 'From a repository checkout, run `pnpm atsh web`; an installed package uses `atsh web`. '
  + 'For client-plugin HMR, run `pnpm atsh web` together with `pnpm run dev:web`.'
