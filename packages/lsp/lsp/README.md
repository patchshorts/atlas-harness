...
| Package | Role |
|---|---|
| `@atlasai/atsh-lsp` (this) | Service Definition: the service, provider registry keyed by branded id + extension mapping, per-query selection, request/result vocabulary, the `LspError` taxonomy |
| `@atlasai/atsh-lsp-stdio` | Service Provider: a generic local backend that registers configured stdio language-server providers |
| `@atlasai/atsh-tool-lsp` | Consumer: the model-facing `lsp` tool over `ctx.lsp` |
