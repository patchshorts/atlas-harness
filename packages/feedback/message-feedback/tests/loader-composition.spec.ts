...
import * as StorageDomain from '@atlasai/atsh-storage-domain'
import * as StorageJson from '@atlasai/atsh-storage-json'
import { remoteMethods } from '@atlasai/atsh-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@atlasai/atsh-session', SessionStore],
    ['@atlasai/atsh-session-persistence-jsonl', JsonlSessionPersistence],
    ['@atlasai/atsh-storage', Storage],
    ['@atlasai/atsh-storage-json', StorageJson],
    ['@atlasai/atsh-storage-domain', StorageDomain],
    ['@atlasai/atsh-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      "- name: '@atlasai/atsh-session'",
      "- name: '@atlasai/atsh-session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@atlasai/atsh-storage'",
      "- name: '@atlasai/atsh-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@atlasai/atsh-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@atlasai/atsh-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
