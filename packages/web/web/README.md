...
| Package | Role |
|---|---|
| `@atlasai/atsh-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@atlasai/atsh-web-search-exa` | Search provider: Exa |
| `@atlasai/atsh-web-search-perplexity` | Search provider: Perplexity |
| `@atlasai/atsh-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@atlasai/atsh-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
...
Selection never depends on registration, config, or HMR order. A capability has an explicit provider id (config `searchProvider`/`fetchProvider`, or env `$ATSH_WEB_SEARCH_PROVIDER`/`$ATSH_WEB_FETCH_PROVIDER` feeding the same fields), or auto-selects when exactly one usable provider is registered. `search()`/`fetch()` resolve the provider at execution time:
