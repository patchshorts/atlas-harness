...
密钥是只写的。保存后，页面只会收到脱敏描述符，永远不会收到明文密钥。密钥存储在 `$ATSH_HOME/.credentials.yaml` 中，settings 只保留它的凭据引用。
...
因此自定义提供方下的视觉模型需要加一行。表单没有对应字段；请在 `$ATSH_HOME/settings.yaml` 中给该模型加上 `input`：
