...
import type { Agent, AgentStatus } from '@atlasai/atsh-agent'
import CommandRuntime from '@atlasai/atsh-commands'
import SessionStore, { SessionId } from '@atlasai/atsh-session'
import * as CommandFeedback from '@atlasai/atsh-command-feedback'
...
describe('/feedback real Loader composition through cordis.yml', () => {
  it('boots cordis.yml and records feedback without model-visible output', async () => {
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@atlasai/atsh-commands', CommandRuntime],
      ['@atlasai/atsh-command-feedback', CommandFeedback],
    ])
    ...
    const userId = getOrCreateAnonymousUserId({ env: { ATSH_HOME: root } })
