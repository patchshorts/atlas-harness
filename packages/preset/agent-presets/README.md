...
| `includeUserRoot` | `true` | Append `<atshHome>/.agent-presets` as a `user` root, after every configured root |
...
`includeUserRoot: false` mounts a roster over `roots` alone. A deployment that confines presets to its own directories needs it, and so does any test pinning an exact roster — otherwise the machine's real `<atshHome>` decides what the roster contains.
