/**
 * @atlasai/atsh-web-app — the browser-surface bundle's runtime glue plugin
 * plus the bundle patch (`cordis.patch.yml`, declared by the `atsh.bundle.patch`
...
import { addHarnessSourceSection } from '@atlasai/atsh-app-boot'
import * as FrontendStatic from '@atlasai/atsh-host-frontend-static'
...
/** This atsh installation's root, from either this package's source or built entry. */
...
const ATSH_WEB_URL = 'ATSH_WEB_URL' as const
...
/** Model-visible orientation and acceptance boundary for sessions created through `atsh web`. */
function webSurfacePrompt(webUrl: string): string {
  ...
  return `You are interacting with the user through the DeepSeek Harness Web GUI at ${webUrl}. `
    ...
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only atsh web injects window.__ATSH_BOOT__. '
...
function resolveDistIndex(): string {
  ...
  try {
    return require.resolve('@atlasai/atsh-web-frontend/dist/index.html')
  } catch {
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [ATSH_WEB_URL]: { description: 'Canonical local URL of the DeepSeek Harness Web GUI serving this session.' },
        },
        resolve: () => ({ [ATSH_WEB_URL]: localWebUrl(runtimeCtx) }),
      })
    })
  }
  if (config.printUrl) {
    ...
    const printUrl = (): void => {
      ...
      console.log(`atsh web: ${localWebUrl(ctx)}${lanCandidate === undefined ? '' : ` (LAN: http://${lanCandidate}:${String(port)})`}`)
    }
