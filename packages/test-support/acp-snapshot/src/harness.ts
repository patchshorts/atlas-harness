 ...
 *
 * @module @atlasai/atsh-acp-snapshot/harness
 */
...
export interface RunOptions {
   ...
   * them to `dsh-llm-replay` via `$ATSH_SNAPSHOT_CHILD_FILES` so each child
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const env: NodeJS.ProcessEnv = {
      ...opts.env,
      ATSH_SNAPSHOT: opts.mode,
      ATSH_SNAPSHOT_FILE: opts.fixtureFile,
      ATSH_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      ATSH_SNAPSHOT_SPILL_ROOT: spillRoot,
      ATSH_HOME: join(cwd, '.atsh'),
      ATSH_AGENTS_HOME: join(cwd, '.agents'),
      ...opts.overrideFile !== undefined ? { ATSH_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { ATSH_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
