 ...
 * `atsh --profile` launcher family.
 *
 * A profile is a directory under `$ATSH_HOME/profiles/<name>` holding a
 * `package.json` (out-of-tree plugin dependencies plus the profile manifest
 * `atsh.profile` with its ordered `bundles` list) and a `cordis.patch.yml`
 * (the user's own patch layer, applied after every bundle layer). Bundles are
 * npm packages whose manifest declares
 * `"atsh": { "bundle": { "patch": "./cordis.patch.yml" } }`; the tree is
 * composed by applying each bundle's patch list in `atsh.profile.bundles` order over
 ...
 * first from the atsh installation (the launcher's own package), then from the
 ...
 * flat fallback directory `$ATSH_HOME/profiles/node_modules` (one symlink per
 ...
 * @module @atlasai/atsh-app-boot/profile
 */
...
/** The bundle half of the `atsh` manifest section: what a bundle package exports. */
export interface AtshBundleManifest {
  /** The patch layer this bundle exports, relative to its package root. */
  patch: string
}

/** The profile half of the `atsh` manifest section: what a profile directory composes. */
export interface AtshProfileManifest {
  /** Ordered bundle layer list (package names). */
  bundles?: string[]
}

/**
 * The profile-launcher slice of the `atsh`-owned package.json section. A
 * manifest may declare both roles; other consumers own additional keys.
 */
export interface AtshManifestSection {
  /** Bundle metadata consumed by the profile launcher. */
  bundle?: AtshBundleManifest
  /** Profile metadata consumed by the profile launcher. */
  profile?: AtshProfileManifest
}
...
export interface ProfileManifest {
  ...
  atsh?: AtshManifestSection
}

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `atsh.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `atsh.profile.bundles` order. */
  layers: ProfileLayer[]
 ...
 * @param name - the profile name (`atsh --profile <name>`).
...
export function resolveProfileDir(name: string, home: string = resolveDshHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    ...
    || name === 'node_modules') {
    throw new Error(`atsh: invalid profile name ${JSON.stringify(name)}`)
  }
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  web: ['@atlasai/atsh-base', '@atlasai/atsh-web-app'],
  headless: ['@atlasai/atsh-base', '@atlasai/atsh-headless'],
}

/** Installation-owned bundle tuples normalized to the shipped template. */
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@atlasai/atsh-base', '@atlasai/atsh-web-app', '@atlasai/atsh-headless'],
}

/** The bundle list a `atsh plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@atlasai/atsh-base']

const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this atsh profile, applied after every bundle layer:
 ...
 * @param bundles - the initial `atsh.profile.bundles` layer list.
 */
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      throw new Error(`atsh: ${link} exists and is not a symlink; remove it so atsh can manage the installation fallback`)
    }
...
/**
 * Maintain the flat module fallback `$ATSH_HOME/profiles/node_modules`: one
 * symlink per package in the atsh app's resolvable dependency CLOSURE (BFS
 ...
 * @param installAnchor - absolute path of the atsh app's package.json.
 * @param home - the Harness home; defaults to {@link resolveDshHome}.
 */
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const bundles = manifest.atsh?.profile?.bundles
  ...
  const normalized: ProfileManifest = {
    ...
    atsh: {
      ...manifest.atsh,
      profile: { ...manifest.atsh?.profile, bundles: [...current] },
    },
  }
 ...
 * `@atlasai/atsh-base` (and every other in-box bundle) always comes from
 * the same installation as the running atsh, never from a profile-local copy.
 * Resolution does not require the package to export `./package.json`.
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `atsh.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the atsh app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName}: cannot resolve profile bundle ${JSON.stringify(packageName)} from the atsh installation or ${profileDir}; `
    + `run 'atsh plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}

/**
 * Load a profile: resolve every `atsh.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `atsh.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the atsh app's package.json (first resolution anchor).
 * @param home - the Harness home; defaults to {@link resolveDshHome}.
...
export function loadProfile(
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'atsh plugin --profile ${name} add <package>'`,
      )
    }
    initProfile(dir, template)
  }
  const manifest = normalizeShippedProfile(name, dir, readProfileManifest(binName, dir))
  // A hand-written profile manifest may omit the atsh section entirely.
  const bundles = manifest.atsh?.profile?.bundles ?? []
  const layers = bundles.map((packageName): ProfileLayer => {
    const packageDir = resolveBundleDir(binName, packageName, installAnchor, dir)
    const bundleManifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as ProfileManifest
    const declared = bundleManifest.atsh?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName}: profile bundle ${JSON.stringify(packageName)} declares no atsh.bundle in its package.json`)
    }
