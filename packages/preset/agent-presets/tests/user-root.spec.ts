 ...
 * authors into `<atshHome>/.agent-presets`, the way `dsh-skill-filesystem` owns
 * `<atshHome>/skills`. `includeUserRoot: false` is how a deployment — or a test
 * pinning an exact roster — opts out.
 *
 * `$ATSH_HOME` is repointed per test because the derived root is resolved in the
...
import AgentPresets, { COMPOSITION_FILE, type Config } from '@atlasai/atsh-agent-presets'
