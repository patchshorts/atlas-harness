 ...
 */
const RUNTIME_STORE_EXEMPTION = '@atlasai/atsh-client-runtime/client'
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.ATSH_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.ATSH_BUILD_FACE) === 'host'
...
function buildFace(value: unknown): BuildFace {
  ...
  throw new Error(`tsdown: --env.ATSH_BUILD_FACE must be host or client, received ${String(value)}`)
}
