...
E2B implementation of the [`@atlasai/atsh-fs`](../../fs/fs/README.md) provider contract. It has no config: load [`@atlasai/atsh-e2b`](../e2b/README.md) first, then this service in place of `dsh-fs-local`. The provider uses the owner's remote cwd and SDK handle, so file tools observe the same world as E2B-backed Bash processes.
