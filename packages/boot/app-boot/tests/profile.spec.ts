...
function stageInstallation(bundles: Record<string, { patch?: string; deps?: Record<string, string> }>): string {
  ...
  for (const [name, spec] of Object.entries(bundles)) {
    ...
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      ...
      ...spec.patch === undefined ? {} : { atsh: { bundle: { patch: './cordis.patch.yml' } } },
    }))
...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@atlasai/atsh-base'])
    ...
    expect(readProfileManifest('t', dir).atsh?.profile?.bundles).toEqual(['@atlasai/atsh-base'])
...
describe('manifest round-trip', () => {
  it('writes and reads back, and fails loud on a broken manifest', () => {
    const dir = tmp()
    writeProfileManifest(dir, { name: 'p', atsh: { profile: { bundles: ['a'] } } })
    expect(readProfileManifest('t', dir).atsh?.profile?.bundles).toEqual(['a'])
...
describe('loadProfile', () => {
  it('resolves each atsh.profile.bundles entry to its patch layer in order, plus the user layer', () => {
    const anchor = stageInstallation({
    ...
    // A hand-made profile without the user layer file or atsh section: empty layers, no throw.
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    expect(PROFILE_TEMPLATES.web).toContain('@atlasai/atsh-base')
    try {
      loadProfile('t', 'web', anchor, home)
    } catch {
      // Resolution failure is the plain-Node outcome for this empty anchor.
    }
    expect(readProfileManifest('t', resolveProfileDir('web', home)).atsh?.profile?.bundles)
      .toEqual([...PROFILE_TEMPLATES.web ?? []])
  })

  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@atlasai/atsh-base': { patch: '[]\n' },
      '@atlasai/atsh-web-app': { patch: '[]\n' },
      '@atlasai/atsh-headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@atlasai/atsh-base', '@atlasai/atsh-web-app', '@atlasai/atsh-headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).atsh?.profile?.bundles)
      .toEqual(['@atlasai/atsh-base', '@atlasai/atsh-headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@atlasai/atsh-base', '@atlasai/atsh-web-app', '@atlasai/atsh-headless', 'custom-bundle',
    ])
    loadProfile('t', 'headless', anchor, customHome)
    expect(readProfileManifest('t', custom).atsh?.profile?.bundles).toEqual([
      '@atlasai/atsh-base', '@atlasai/atsh-web-app', '@atlasai/atsh-headless', 'custom-bundle',
    ])
  })

  it('fails loud when a listed bundle declares no atsh.bundle', () => {
    const anchor = stageInstallation({ 'not-a-bundle': {} })
    const home = tmp()
    const dir = resolveProfileDir('demo', home)
    initProfile(dir, ['not-a-bundle'])
    expect(() => loadProfile('t', 'demo', anchor, home)).toThrow('declares no atsh.bundle')
  })
})
