...
import SandboxedFileSystem from '@atlasai/atsh-fs-sandbox'
...
import { snapshotSubagentDescriptor } from '@atlasai/atsh-subagent'
  ...
  it('records the parent sandbox override and the approval pin before publishing a spawn child', async () => {
    ...
    try {
      ...
      const contextText = runtimeContext.data.content
        ...
        .map(block => block.text)
        .join('\n')
      expect(contextText).toContain('Current ATSH file policy: read-only')
