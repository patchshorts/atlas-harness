 ...
 * managed `ATSH_*` collection, abort translation, canonical result projection,
...
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@atlasai/atsh-shell'
...
import type { ShellProcessRead } from '@atlasai/atsh-shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, atshHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, atshHome === undefined ? {} : { atshHome })
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, atshHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed ATSH_* environment', async () => {
    const atshHome = mkdtempSync(join(tmpdir(), 'dsh-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, atshHome)
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    expect(dshEnv?.['ATSH_SHELL']).toBe('1')
    expect(dshEnv?.['ATSH_HOME']).toEqual(expect.any(String))
    expect(dshEnv).not.toHaveProperty('ATSH_SESSION_ID')
  })
