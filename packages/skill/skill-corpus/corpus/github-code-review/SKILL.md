...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.Atlas/.env ] && grep -q "^GITHUB_TOKEN=" ~/.Atlas/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.Atlas/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Atlas AI\",
    \"comments\": [
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Atlas AI. Code looks clean — good test coverage, no security concerns."
