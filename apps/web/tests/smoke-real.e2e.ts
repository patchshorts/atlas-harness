// Real-host smoke: spawn `atsh web` with a real key, walk the full flow
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`atsh web not ready in 90s; output:\n${out}`)) }, 90_000)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`atsh web exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `atsh web` serves every production manifest plugin; until every UI
...
describe('atsh web keyless CLI smoke', () => {
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--port', '0'],
      {
        ...
        env: {
          ...
          ATSH_HOME: join(sessionsDir, '.atsh'),
          ATSH_AGENTS_HOME: join(sessionsDir, '.agents'),
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--port', '0'],
      {
        ...
        env: {
          ...
          ATSH_HOME: join(workspace, '.atsh'),
          ATSH_AGENTS_HOME: join(workspace, '.agents'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
        stdio: ['ignore', 'pipe', 'pipe'],
      },
  ...
  it('ATSH_TOOLS_MODE=code collapses the provider wire tools to run_code with the SDK prompt section', async () => {
    requireDist()
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  beforeAll(async () => {
    ...
    // tsx boot mirrors the runtime half of the root atsh script. Isolate
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    // This scenario spawns its own server against a fresh $ATSH_HOME with the
