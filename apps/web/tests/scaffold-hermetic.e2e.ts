...
import type {} from '@atlasai/atsh-agent-presets'
...
it('isolates replay skill discovery from every ambient host root', async () => {
  ...
  const atshHome = join(ambient, 'dsh-home')
  const agentsHome = join(ambient, 'agents-home')
  const bundled = join(ambient, 'bundled')
  await Promise.all([
    writeSkill(join(atshHome, 'skills'), 'ambient-dsh'),
    writeSkill(join(agentsHome, 'skills'), 'ambient-agents'),
    writeSkill(bundled, 'ambient-bundled'),
  ])

  const originalDshHome = process.env.ATSH_HOME
  const originalAgentsHome = process.env.ATSH_AGENTS_HOME
  const originalBundled = process.env.ATSH_BUNDLED_SKILL_DIR
  process.env.ATSH_HOME = atshHome
  process.env.ATSH_AGENTS_HOME = agentsHome
  ...
  try {
    ...
    try {
      ...
      if (originalDshHome === undefined) delete process.env.ATSH_HOME
      else process.env.ATSH_HOME = originalDshHome
      if (originalAgentsHome === undefined) delete process.env.ATSH_AGENTS_HOME
      else process.env.ATSH_AGENTS_HOME = originalAgentsHome
      if (originalBundled === undefined) delete process.env.ATSH_BUNDLED_SKILL_DIR
      else process.env.ATSH_BUNDLED_SKILL_DIR = originalBundled
