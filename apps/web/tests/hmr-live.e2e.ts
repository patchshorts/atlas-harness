/** Published atsh web + pnpm dev:web → browser HMR, with no page reload. */
...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  ...
  if (!existsSync(binPath)) throw new Error('HMR browser test needs the built atsh bin; run pnpm run build first')
  ...
  try {
    ...
    host = subprocessCtx.subprocess.spawn(spawnSpec(
      ...
      world,
      {
        ...
        ATSH_HOME: join(world, '.atsh'),
      },
    ))
    const baseUrl = await waitForOutput(host, /dsh web: (http:\/\/[^\s]+)/, 'built atsh web')
