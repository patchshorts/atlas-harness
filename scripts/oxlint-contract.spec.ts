...
`
  ...
  // Same child-spawn class root-caused at d80a3a1 (runOxlint builds the
  // runner synchronously; the wall cost scales with parallel-suite CPU
  // contention and deterministically exceeds vitest's default 5000ms under
  // the full 888-file/14220-test lane). Give it the same explicit 30s bound.
  ...
  // This test is genuinely slower than vitest's 5000ms default: it spawns the
  // oxlint binary and the tsx CLI synchronously (runRepositoryOxlint -> tsx ->
  // run-oxlint.ts -> oxlint child) and writes/reads probe files, so its wall
  // cost scales with parallel-suite CPU contention. Under the full 888-file /
  // 14220-test coverage lane it deterministically exceeded the default budget
  // (5x consecutive 5000ms timeouts, 2026-08-24), while the test itself passes
  // in ~2.7s solo. Give it an explicit 30s bound (same convention as the
  // vitest "pass a timeout value" guidance) so it is not killed by budget,
  // only by a genuine hang. Unrelated to the prior workstream corrections; clears the MR gate.
