...
describe('release families', () => {
  it('names one tag for the whole atsh family and one per vendored package', () => {
    const atsh = releaseFamily('atsh')
    const vendor = releaseFamily('vendor')
    const cli = member('apps/cli', '@atlasai/atsh')
    const cordis = { ...member('vendor/cordis', '@deepseek-ai/cordis'), version: '4.0.1' }

    expect(atsh.tagFor(cli)).toBe('dsh-v0.0.1')
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const atsh = releaseFamily('atsh')
    const members = [member('apps/cli', '@atlasai/atsh'), { ...member('apps/web', '@atlasai/atsh-web-frontend'), version: '0.0.2' }]

    expect(() => { atsh.verifyVersions(members) }).toThrow(/must share one version/)
    expect(() => { atsh.verifyVersions([members[0]!]) }).not.toThrow()
  })
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const atsh = releaseFamily('atsh')
    const members = [
      member('packages/a/consumer', '@atlasai/atsh-consumer', { dependencies: { '@atlasai/atsh-library': 'workspace:^' } }),
      member('packages/a/library', '@atlasai/atsh-library'),
      member('packages/a/zebra', '@atlasai/atsh-zebra'),
    ]

    expect(atsh.publishOrder(members).map(entry => entry.name)).toEqual([
      '@atlasai/atsh-library',
      '@atlasai/atsh-consumer',
      '@atlasai/atsh-zebra',
    ])
  })
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const atsh = releaseFamily('atsh')
    const members = [
      member('packages/a/left', '@atlasai/atsh-left', { dependencies: { '@atlasai/atsh-right': 'workspace:^' } }),
      member('packages/a/right', '@atlasai/atsh-right', { dependencies: { '@atlasai/atsh-left': 'workspace:^' } }),
    ]

    expect(() => { atsh.publishOrder(members) }).toThrow(/dependency cycle/)
  })

  it('applies the harness payload policy to atsh and keeps upstream payloads for vendored packages', () => {
    const atsh = releaseFamily('atsh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@atlasai/atsh-library')
    const vendored = member('vendor/cordis', '@deepseek-ai/cordis')

    expect(() => { atsh.validatePayload(harness, ['package/lib/index.js', 'package/src/index.ts']) })
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('atsh').installedEntry).toEqual({ packageName: '@atlasai/atsh', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@atlasai/atsh-library', { files: ['lib/index.js'] }),
