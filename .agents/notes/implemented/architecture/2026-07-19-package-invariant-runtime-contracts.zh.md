...
中央 `atsh-invariants` 服务只负责配置、注册唯一性、子 fiber 生命周期、回滚、dispose（资源释放）和归属到包的失败。它不暴露通用插件形状、服务形状或启动断言 helper，也不导入产品包。
...
| 所有者 | 运行时关系 |
|---|---|
| `atsh-session` | 序号严格递增、轮次/步骤包围关系，以及同一步骤内的工具调用/工具结果配对。 |
| `atsh-agent` | agent（智能体）状态不得重复，并且不能离开终态 disposed。 |
| `atsh-scope` | 作用域事件必须携带 carrier，且路由 subject 保持一致。 |
| `atsh-agent-loop` | 从会话事件日志重建带显式标记的冻结 loop 请求。 |
| `atsh-llm` | 流中块的文法、delta 类型/索引匹配、单次 usage、块闭合和终止 finish。 |
| `atsh-llm-retry` | 持久化重试记录指向当前打开轮次中最近关闭的步骤；每个步骤的记录保持唯一，重试次数单调递增，并且重试次数和非负的定时器延迟均保持在边界内。 |
| `atsh-tools` | pre/execute/post 阶段单调推进，以及最终 execution/result 快照不可变。 |
| `atsh-system-prompt` | 权威 assembly 中 section、工具和 variable 的数据约束。 |
| `atsh-compaction` | 压缩（compaction）start/summary/end 配对、范围端点、token 数量和成功时必须存在 summary。 |
| `atsh-hook-protocol` | 钩子 invocation/result 的关联、dialect、身份和 duration 约束。 |
| `atsh-sandbox-policy` | 持久化 `sandbox/mode` 事件必须使用封闭的 sandbox-mode 词表。 |
| `atsh-fs` | 文件系统决策/观测事件必须携带可用的 target 和 version 身份。 |
| `atsh-goal` | 持久化目标快照保持来源归属、渲染内容、修订号、生命周期和时间戳关系，并保证已准入的 Round 连续编号。 |
| `atsh-goal-round-driver` | 目标来源的继续执行消息必须匹配根据此前持久化目标状态重建的提示词。 |
| `atsh-subagent` | 提供方 add/remove 和 child start/end 事件必须保持身份与配对。 |
| `atsh-permission-presets` | 持久化 permission 决策必须引用当前 permission 表中的 preset。 |
| `atsh-user-approval` | approval asked/decided 记录按 call 配对，并使用有效 outcome 和 policy。 |
| `atsh-workflow` | 工作流和 child-agent start/end 事件保持 run metadata、身份、outcome、数量和 error 关系。 |
| `atsh-jobs` | 当前与终态 task 快照保持 id/kind、owner、status 和 timestamp 关系。 |
| `atsh-tool-todo` | 持久化全量快照使用唯一且已 trim 的条目和封闭 status。 |
| `atsh-time-context` | 标注插件来源的时钟 reading 必须匹配会话当前打开的轮次、下一个步骤开始前的位置和 elapsed baseline；渲染时间必须可解析，且不得晚于对应事件。 |
