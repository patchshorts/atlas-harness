...
import LlmRuntime, { createUserMessage, LlmAdapter, LlmError, resolveRetryPolicy  } from '@atlasai/atsh-llm'
import type { GenerateOptions, ResolvedRetryPolicy, StreamChunk } from '@atlasai/atsh-llm'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@atlasai/atsh-llm-retry', retry],
    ['@atlasai/atsh-agent-loop', AgentLoop],
  ])
...
describe('real Loader composition', () => {
  ...
  it('loads provider-supplied policy and records recovery through the shipping loop', { timeout: 60_000 }, async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@atlasai/atsh-llm-retry'",
      "- name: '@atlasai/atsh-agent-loop'",
    ])
