...
- **`ctx.appExit` 由启动器持有**：在 `atsh` 启动器之外启动 headless profile 会在激活时明确报错，直到宿主提供该退出请求。
