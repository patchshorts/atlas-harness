/**
 * Shared profile boot for every `atsh` surface: resolve the profile, stack its
 * patch layers (bundle layers in `atsh.profile.bundles` order, the profile's
 ...
 * @module @atlasai/atsh/profile-boot
 */
...
import { ATSH_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@atlasai/atsh-launch-environment'
...
/**
 * The home-level user patch layer (`$ATSH_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$ATSH_HOME` may be set by the test or launcher after import.
...
/** Absolute path of this atsh installation's package.json (both anchors: src/ and lib/ sit one level under apps/cli). */
export const INSTALL_ANCHOR = fileURLToPath(new URL('../package.json', import.meta.url))

/** The session-telemetry row id the ATSH_TELEMETRY_DISABLED switch targets. */
const TELEMETRY_ROW_ID = 'session-telemetry-otel'

/** The empty root entry list every profile tree patches over. */
const PROFILE_ROOT_CONFIG = `# atsh profile root — an empty entry list. The tree is composed as patches:
# each bundle in package.json's atsh.profile.bundles, then cordis.patch.yml, then any
 ...
 * switch set.
 * @param disabledEnv - the raw `ATSH_TELEMETRY_DISABLED` value (`undefined` when unset).
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$ATSH_HOME/cordis.patch.yml`), applied after the profile's own. */
 ...
 * `atsh.profile.bundles` order (the base bundle gates the shell stacks by
 * platform on its own rows), the profile's user layer, the home-level user
 * layer (`$ATSH_HOME/cordis.patch.yml` — machine-local preferences that apply
...
function composeProfile(
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.ATSH_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const ctx = await boot(NAME, rootConfig, structuredClone(allPatches(composed)), (hostCtx) => {
    ...
    hostCtx.provide(ATSH_LAUNCH_ENVIRONMENT_KEY, options.environment)
