...
| `ATSH_CWD` | Agent workspace for bash and filesystem tools |
| `ATSH_CONTEXT_WINDOW` | Context capacity recorded for the `ATSH_MODEL` catalog entry in the minimal variant |
| `ATSH_MAX_TOKENS_AS_SUCCESS` | `true` (default) accepts token-limited results; `false` reports them as errors |
| `ATSH_MODEL` | Default model used by `minimal.py`; `--model` takes precedence |
| `ATSH_SESSION_ROOT` | JSONL session directory |
| `ATSH_SYSTEM_PROMPT` | Deployment-provided coding persona |

Pass the config path through the Python SDK's `cordis` option or `ATSH_CORDIS_CONFIG`. The bundled executable already carries every plugin named by this file; the target machine does not need Node.js.

## Minimal variant

[`minimal.cordis.yml`](minimal.cordis.yml) is the complete standalone counterpart of the Web `minimal` preset. `ATSH_SYSTEM_PROMPT` selects its system prompt, with `You are a helpful software engineer assistant.` as the fallback. It suppresses every system-prompt runtime-context contribution for fresh sessions and mounts no context-compaction plugin. Its model-facing tools are exactly:
