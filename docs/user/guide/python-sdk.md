...
# export ATSH_MODEL=deepseek-v4-flash
# export ATSH_SYSTEM_PROMPT='You are a helpful software engineer assistant.'
```
...
| Property | Value |
|---|---|
| System prompt | `ATSH_SYSTEM_PROMPT`, falling back to `You are a helpful software engineer assistant.` |
| Model in `minimal.py` | `--model`, then `ATSH_MODEL`, then `deepseek-v4-flash` |
...
| Session persistence | Uncompressed JSONL under `ATSH_SESSION_ROOT` |
