...
import {
  ...
  type CommandResult,
  type Sandbox,
} from '@atlasai/atsh-e2b'
import type E2BRuntime from '@atlasai/atsh-e2b'
import type { SubprocessSpawnSpec } from '@atlasai/atsh-subprocess'
import E2BSubprocessRuntime from '@atlasai/atsh-subprocess-e2b'
...
class FakeSandbox {
  ...
  alive = true
  zombieOnly = false
  ambient = 'PATH=/ambient/bin\0KEEP=safe\0UNICODE=你好\0NPM_TOKEN=secret\0ATSH_STALE=old\0BROKEN\0=bad\0'
...
function runtime(fake: FakeSandbox, getSandbox: () => Promise<Sandbox> = async () => fake.sandbox): E2BRuntime {
  return {
    cwd: '/workspace',
    runtimeRoot: '/workspace/.atsh-e2b',
    getSandbox,
...
describe('E2BSubprocessHandle', () => {
  it('starts asynchronously, keeps secrets out of the command, and supports deferred piped stdin/output', async () => {
    ...
    }), '/workspace/.atsh-e2b/processes/one')
    ...
    expect(controlEnvs?.HOME).toMatch(/^\/\.atsh-e2b-control-/)
    ...
    const command = fake.commandsSeen.find(value => value.includes('exec "$atsh_e2b_env_bin" -i'))!
    expect(command).toContain('"$atsh_e2b_setsid" --wait -- "$atsh_e2b_bash" -c')
    expect(command).not.toContain('DEEPSEEK_API_KEY')
    expect(command).not.toContain('ATSH_MODE')
    ...
    expect(command).not.toContain('${!atsh_e2b_name}')
    ...
    expect(environmentProbe).toContain('test -n "$atsh_e2b_home" -a -d "$atsh_e2b_home"')
    expect(environmentProbe).not.toContain('"$PWD"')
    expect(command).toContain('mapfile -d')
    expect(command).toContain('atsh_e2b_node="$(command -v node)"')
    expect(command).toContain('"$atsh_e2b_env_bin" -i "$atsh_e2b_node" -e')
    expect(command).toContain('"$atsh_e2b_env_bin" -i -- "${atsh_e2b_env[@]}" "$@"')
    expect(command).toContain('exec "$atsh_e2b_env_bin" -i -- "${atsh_e2b_env[@]}"')
    ...
    expect(fake.writtenFiles[0]).toEqual([
      '/workspace/.atsh-e2b/processes/one/pid',
      '/workspace/.atsh-e2b/processes/one/exit-code',
      '/workspace/.atsh-e2b/processes/one/environment',
      '/workspace/.atsh-e2b/processes/one/stderr.log',
    ])
    expect(fake.writtenFileData.get('/workspace/.atsh-e2b/processes/one/environment')).toBe(
      'PATH=/bin\0UNICODE=你好\0HOME=/home/user\0FOO-BAR=hyphen-value\0--split-string=literal-value\0DEEPSEEK_API_KEY=explicit-secret\0ATSH_MODE=test\0',
    )
    ...
    expect(fake.removed).toContain('/workspace/.atsh-e2b/processes/one/stderr.log')
  ...
  it('removes a spill once the complete stream exceeds its cap', async () => {
    ...
    const command = fake.commandsSeen.find(value => value.includes('atsh_e2b_tee='))!
    expect(command).toContain('"$atsh_e2b_head" -c 3')
    expect(command).toContain('/runtime/oversize/stdout.log')
    expect(command).toContain('"$atsh_e2b_tee" --output-error=warn-nopipe')
