 ...
 * silently changes composition. Example configs and the atsh Web composition
...
// These example files are overlays consumed by the built atsh app, so their bare
...
const CHOOSER_PACKAGE = '@atlasai/atsh-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@atlasai/atsh-host-directory-picker-native',
  '@atlasai/atsh-host-directory-picker-browse',
  '@atlasai/atsh-client-ui-directory-picker-browse',
  '@atlasai/atsh-client-ui-directory-picker-native',
]
 ...
 * `atsh.client` block, and the node half of a surface plugin is an empty
 ...
 * `atsh.client` declaration disagree.
 */
function validateClientHalvesDeclared(): string[] {
  return globSync('packages/client/*/package.json', { cwd: root }).flatMap((manifestPath) => {
    ...
    const declaresClient = manifest.atsh?.client !== undefined
    if (shipsClient === declaresClient) return []
    return [shipsClient
      ? `${manifestPath}: exports "./client" but declares no atsh.client, so its browser half is never served`
      : `${manifestPath}: declares atsh.client but exports no "./client" entry to serve`]
  })
}
 ...
 *
 * Both have happened. `shell-env` in a preset realm left `ATSH_WEB_URL` reaching
...
function validateAppResolution(): string[] {
  ...
  // atsh app's own dependency surface — the profile module fallback mirrors it.
 ...
 * the tsconfig `paths` facade to a `.ts`/`.tsx` source file. The `atsh` source
