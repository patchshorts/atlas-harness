...
import type { TerminalSendOperation } from '@atlasai/atsh-terminal'
...
describe('terminal-bash real shell', () => {
  it('persists cwd and environment across sends, scrubs secrets, and closes', async () => {
    const previous = process.env.ATSH_TEST_SECRET
    process.env.ATSH_TEST_SECRET = 'must-not-leak'
    try {
      const { ctx, root, agent } = await harness('danger-full-access')
      const created = await ctx.terminals.spawn(agent, { type: 'shell', name: 'main', cwd: root })
      expect(created.motd).toContain('atsh> ')
      ...
      if (previous === undefined) delete process.env.ATSH_TEST_SECRET
      else process.env.ATSH_TEST_SECRET = previous
    }
  }, 10_000)
  ...
  it('quiesces a disowned same-session descendant after the shell exits naturally', async () => {
    ...
    try {
      const background = ctx.terminals.startSend(agent, created.sessionId, {
        text: `sh -c 'trap "" TERM; printf "%s" "$$" > "$1"; sleep 60' atsh "${pidFile}" & disown`,
        submit: true,
      })
