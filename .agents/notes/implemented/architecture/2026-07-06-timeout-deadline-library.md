...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `atsh-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `atsh-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `atsh-timeout` (`idleWatchdog`) |
| Clear the timer | `atsh-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `atsh-timeout` (`timeoutOf`) |
