...
import { createBudgetGuard, LlmBudgetError, type BudgetGuard } from './cost/guard.ts'
import { CostLedger, DEFAULT_SESSION } from './cost/ledger.ts'
...
export { BUDGET_EXCEEDED_CODE, createBudgetGuard, LlmBudgetError } from './cost/guard.ts'
export type { BudgetDecision, BudgetGuard } from './cost/guard.ts'
export * from './cost/rates.ts'
export * from './cost/ledger.ts'
export * from './routing.ts'

/**
 * Optional cost accounting for the llm service. Additive and config-gated:
 * absent config leaves every call exactly as it behaves today.
 */
export interface LlmCostConfig {
  /**
   * Per-model rate table in USD cents per million tokens. An absent table
   * prices nothing: every model costs 0 cents and is flagged as unrated.
   */
  rates?: ModelRateTable
  /**
   * Hard per-session ceiling in USD cents. A session that has spent at
   * least the budget refuses every further call with {@link LlmBudgetError}
   * before the llm/stream waterfall. Absent means no budget; the ledger
   * still records usage when `rates` is present.
   */
  budgetCents?: number
}

/** Payload of the 'llm/cost' event: one usage chunk just recorded. */
export interface LlmCostEvent {
  /** Accounting session the cost was recorded into (never absent). */
  sessionKey: string
  /** Model the usage chunk belonged to. */
  model: string
  /** The usage chunk that was recorded. */
  usage: TokenUsage
  /** The session total after this record, in USD cents. */
  spentCents: number
}
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
    ...
    /**
     * Emitted after one usage chunk is recorded into the per-session cost
     * ledger. Carries the session total after the record. Listeners observe
     * and never veto; the event is advisory accounting state.
     * @param event - the recorded chunk and its session total.
     * @mode emit
     */
    'llm/cost'(event: LlmCostEvent): void
...
export class LlmRuntime extends Service {
  ...
  >()
  private readonly costLedger: CostLedger | undefined
  private readonly budgetGuard: BudgetGuard | undefined

  constructor(ctx: Context, config: LlmCostConfig = {}) {
    super(ctx, 'llm')
    if (config.rates !== undefined || config.budgetCents !== undefined) {
      const ledger = new CostLedger(config.rates)
      this.costLedger = ledger
      if (config.budgetCents !== undefined) {
        this.budgetGuard = createBudgetGuard(ledger, config.budgetCents)
      }
    }
  }
  ...
  private streamWithRegistration(
    ...
    this.refuseIfOverBudget(options)
    const ledger = this.costLedger
    const fallback = ledger === undefined
      ? () => this.adapterStream(options, prepared)
      : () => this.withCostTap(ledger, options, this.adapterStream(options, prepared))
    return this.ctx.waterfall(this, 'llm/stream', options, fallback)
  }

  /** Synchronous hard stop before the waterfall: an over-budget session refuses. */
  private refuseIfOverBudget(options: GenerateOptions): void {
    const guard = this.budgetGuard
    if (guard === undefined) return
    const decision = guard.check(options.model, options.sessionKey)
    if (decision.allowed) return
    const session = options.sessionKey ?? DEFAULT_SESSION
    throw new LlmBudgetError(
      `llm: session "${session}" has exceeded its budget — refusing model "${options.model}"`,
    )
  }

  /**
   * Pass-through usage tap: observes `usage` chunks into the cost ledger and
   * yields every chunk unchanged, in order, with its original identity.
   */
  private async * withCostTap(
    ledger: CostLedger,
    options: GenerateOptions,
    inner: AsyncIterable<StreamChunk>,
  ): AsyncGenerator<StreamChunk> {
    for await (const chunk of inner) {
      if (chunk.type === 'usage') this.recordCost(ledger, options, chunk.usage)
      yield chunk
    }
  }

  /** Record one usage chunk and publish the session's new total. */
  private recordCost(ledger: CostLedger, options: GenerateOptions, usage: TokenUsage): void {
    ledger.record(usage, options.model, options.sessionKey)
    this.ctx.emit('llm/cost', {
      sessionKey: options.sessionKey ?? DEFAULT_SESSION,
      model: options.model,
      usage,
      spentCents: ledger.spentCents(options.sessionKey),
    })
  }
}
