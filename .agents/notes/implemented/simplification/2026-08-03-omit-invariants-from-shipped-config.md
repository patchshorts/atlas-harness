# Agent Note: Omit runtime invariants from shipped atsh config
...
The built CLI config-dump test checks both shipped surfaces and rejects either the service entry or any `@atlasai/atsh-*/invariant` entry.
...
- Ordinary `atsh` TUI and Web runs install no invariant listeners or trace state and cannot fail through `InvariantError`.
