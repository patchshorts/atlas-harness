...
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@atlasai/atsh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@atlasai/atsh-base', '@atlasai/atsh-web-app'])
    const profile = loadProfile('atsh', 'web', anchor, home)
    ...
    for (const name of ['@atlasai/atsh-pwsh-sandbox', '@atlasai/atsh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@atlasai/atsh-base'])
    const profile = loadProfile('atsh', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
