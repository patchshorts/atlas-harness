...
import { LocalPtySession } from '@atlasai/atsh-terminal-bash/src/session.ts'
import type { ResolvedConfig } from '@atlasai/atsh-terminal-bash/src/config.ts'
import type { TerminalSendOperation, TerminalSessionStatus, TerminalSignal } from '@atlasai/atsh-terminal'
import type {
  SubprocessOutcome,
  SubprocessTerminalHandle,
  SubprocessTerminalSignal,
} from '@atlasai/atsh-subprocess'
import { TerminalError } from '@atlasai/atsh-terminal'
import type {
  ProcessIdentity,
  ProcessInspector,
} from '@atlasai/atsh-subprocess-local/src/process-inspector.ts'
...
async function initialize(session: LocalPtySession, terminal: FakeTerminal): Promise<void> {
  ...
  terminal.emitData('\x1b]133;D;0\x07atsh> ')
...
describe('LocalPtySession readiness and output', () => {
  ...
  it('cancels with foreground-group SIGINT, observes AbortSignal, and contains write failures', async () => {
    ...
    terminal.emitData('\x1b]133;D;130\x07atsh> ')
    await vi.advanceTimersByTimeAsync(10)
  ...
  it('does not attribute a delayed prior prompt to the current send', async () => {
    ...
    terminal.emitData('\x1b]133;D;0\x07atsh> printf \'PID=%s\\n\' "$!"\r\n')
    await vi.advanceTimersByTimeAsync(20)
    expect(settled).toBe(false)
