...
`resolveDshHome()` 解析 DeepSeek Harness 的单根主目录。优先级从高到低为：显式配置的路径、`$ATSH_HOME`、`~/.atsh`。harness 将所有用户数据保存在同一根目录下。

`atshHomePath(...segments)` 使用 Node 的平台路径规则，将子路径段拼接到解析后的主目录下。不传入任何路径段时，返回主目录本身。

`atshHomeDisplay()` 以符号方式表示当前根目录，用于面向用户的路径：默认主目录表示为 `~/.atsh`，任何已配置的主目录表示为 `$ATSH_HOME`。它绝不会泄露机器的绝对路径。

`ATSH_HOME_DIR_NAME` 定义默认用户数据目录名：`.dsh`。

`defaultAtshHome()` 使用 Node 的平台路径规则，将操作系统主目录与 `.dsh` 拼接，并返回默认 DeepSeek Harness 主目录。
