...
const SESSION_PACKAGE = '@atlasai/atsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@atlasai/atsh-session/types'
 ...
 * top-level declaration (in `@atlasai/atsh-session`) and any declaration
 * merge inside a `declare module '@atlasai/atsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @atlasai/atsh-session/known-event-types',
    ' */',
    '',
    '/**',
