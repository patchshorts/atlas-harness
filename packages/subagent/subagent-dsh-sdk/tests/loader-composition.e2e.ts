...
import { resolveExampleLaunch, runLoaderSmoke } from '@atlasai/atsh-loader-smoke'
...
describe('SDK subagent cwd inheritance through a real cordis.yml', () => {
  it('runs the child runtime in the parent session workspace', async () => {
    ...
    const { stderr } = await runLoaderSmoke({
      ...
      env: {
        ATSH_TEST_CHILD_COMMAND: childLaunch.command,
        ATSH_TEST_CHILD_ARGS: JSON.stringify(childLaunch.args),
        ATSH_TEST_CHILD_ENV: JSON.stringify({
