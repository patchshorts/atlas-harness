 ...
 * @module @atlasai/atsh-subprocess
 */

import { Context, Service } from '@deepseek-ai/cordis'
import { ATSH_ENV_PREFIX } from './types.ts'
import type { SubprocessHandle, SubprocessSpawnSpec } from './types.ts'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from './types.ts'

export { ATSH_ENV_PREFIX } from './types.ts'
export type {
  CollectedOutput,
  DshEnvironment,
  DshEnvironmentKey,
 ...
 * `ATSH_*` names — the canonical base every harness child starts from. `PATH`,
 ...
 * credential or current `ATSH_*` fact goes through the spec's explicit `env`,
 * which merges after this scrub). Both scrubs match case-insensitively:
 * Windows environment names are case-insensitive, so a parent `atsh_*` entry
 * would otherwise survive and read back as `$env:ATSH_*` in the child;
 * deliberate lowercase `atsh_*` names on POSIX are implausible. Exported as a plain function so spawners
...
export function scrubbedParentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (value !== undefined && !SENSITIVE_ENV_PATTERN.test(key) && !key.toUpperCase().startsWith(ATSH_ENV_PREFIX)) env[key] = value
  }
  return env
}
