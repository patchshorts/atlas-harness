 ...
 * Also carries the golden-rule guarantee for the loop's taint pass (FR-9):
 * taint verification is a pure derived filter over the final output clause —
 * it never writes to the session log or message history. Purity is asserted
 * in `taint.spec.ts`; this companion documents the responsibility.
 * @module @atlasai/atsh-agent-loop/invariant
 */

import type { Context } from '@deepseek-ai/cordis'
import { isAgentLoopRequest, type GenerateOptions } from '@atlasai/atsh-llm'
import type { InvariantFailure, InvariantInstaller } from '@atlasai/atsh-invariants'
import { foldRequestHeader } from '@atlasai/atsh-session'

const PACKAGE_NAME = '@atlasai/atsh-agent-loop'
