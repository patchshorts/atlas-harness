/**
 * Enforce the MIT license declaration for repository-owned ATSH npm packages.
...
const ATSH_PACKAGE_NAME = /^@deepseek-ai\/dsh(?:-|$)/

/** Result of checking every ATSH package reachable through the root workspace list. */
export interface DshPackageLicenseReport {
  /** Number of ATSH package manifests checked. */
  packageCount: number
...
/**
 * Check every ATSH npm package declared by the repository workspace.
...
export function inspectDshPackageLicenses(root: string): DshPackageLicenseReport {
  ...
  for (const file of workspaceManifestPaths(root)) {
    ...
    if (typeof name !== 'string' || !ATSH_PACKAGE_NAME.test(name)) continue
...
if (process.argv[1] && import.meta.filename === resolve(process.argv[1])) {
  ...
  if (report.failures.length > 0) {
    process.stderr.write('verify-dsh-package-licenses: non-MIT ATSH package declarations found:\n')
    for (const failure of report.failures) process.stderr.write(`  ${failure}\n`)
    process.exitCode = 1
  } else {
    process.stdout.write(
      `verify-dsh-package-licenses: ${String(report.packageCount)} ATSH package(s) checked; all declare MIT.\n`,
    )
  }
}
