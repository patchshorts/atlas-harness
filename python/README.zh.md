...
用于以子进程方式驱动 Atlas Harness (atsh)——DeepSeek Harness 的分支谱系——的 Python 包。客户端 SDK 通过 stdio 使用按行分隔的 JSON-RPC 与内置运行时通信。
...
Python 发行包保留上游名称（`deepseek-harness-*`，模块 `deepseek_harness*`）。这是经过权衡的兼容性决策：重命名会破坏上游 SDK 现有使用方的导入兼容性。这些发行包面向 Atlas Harness，并跟随其发布线。
