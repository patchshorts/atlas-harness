...
import { canonicalPath } from '@atlasai/atsh-sandbox'
...
const PROMPTS = [
  ...
  'Does the ATSH file sandbox currently restrict file operations? Answer directly in one sentence. Do not call tools.',
...
function runtimeContexts(events: readonly SessionEvent[]): string[] {
  return events.flatMap((event) => {
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@atlasai/atsh-system-prompt') return []
...
describe('web e2e: current sandbox policy reaches the model before tools', () => {
  ...
  it.skipIf(MODE === 'record')('records cache-safe current policy before the corresponding model behavior', async () => {
    ...
    expect(systems[0]).not.toContain('Current ATSH file policy:')
    ...
    expect(contexts[0]).toContain('Current ATSH file policy: read-only. Any available operation enforced by the ATSH file sandbox cannot modify files in the standing mode.')
    ...
    expect(contexts[1]).toContain('Current ATSH file policy: danger-full-access. The ATSH file sandbox does not restrict file modifications by available operations.')
    ...
    expect(contexts[2]).toContain(`Current ATSH file policy: workspace-write. Any available operation enforced by the ATSH file sandbox may modify files under the session workspace: ${JSON.stringify(canonicalPath(sessionWorkspace))}. Some platform temporary areas may also be writable.`)
    ...
    expect(contexts[3]).toContain('Current ATSH file policy: read-only.')
