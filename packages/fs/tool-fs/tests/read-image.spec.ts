...
import { CodeRuntime } from '@atlasai/atsh-code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@atlasai/atsh-code-runtime'
import { CallId, LlmAdapter, LlmRuntime } from '@atlasai/atsh-llm'
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, StreamChunk } from '@atlasai/atsh-llm'
import SystemPrompt from '@atlasai/atsh-system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@atlasai/atsh-tools'
import type { Config as ToolConfig } from '@atlasai/atsh-tools'
import LocalFileSystem from '@atlasai/atsh-fs-local'
import * as FsPolicy from '@atlasai/atsh-fs-observation-policy'
import LocalAttachmentStore from '@atlasai/atsh-attachment-local'
import { AttachmentId, AttachmentStore } from '@atlasai/atsh-attachment'
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { atshHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { atshHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { atshHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
