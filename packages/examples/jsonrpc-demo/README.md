...
The first non-empty channel wins: `$ATSH_CORDIS_CONFIG`, then positional `argv[2]`. If neither names an existing file, the bin prints one-line usage to stderr and exits 1; there is no working-directory or built-in fallback. [`dsh-app-boot`](../../boot/app-boot/README.md) makes plugin load failures fatal. This protocol does not use `ATSH_SNAPSHOT`.
...
- **No built-in or default config exists** — every launch must provide `ATSH_CORDIS_CONFIG` or a positional path, and deployment owns the complete plugin tree and stdout discipline.
