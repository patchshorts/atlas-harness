...
import { normalizeTerminalText, TerminalSanitizer } from '@atlasai/atsh-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07atsh> ')).toEqual({ text: 'atsh> ', prompt: true, promptTail: 'atsh> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('atsh> ')).toEqual({ text: 'atsh> ', prompt: false, promptTail: 'atsh> ' })
  })
