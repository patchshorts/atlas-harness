...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'atsh_state=$1',
  'mapfile -d \'\' -t atsh_env < "$atsh_state/environment"',
  'mapfile -d \'\' -t atsh_argv < "$atsh_state/argv"',
  'atsh_output_marker=$(<"$atsh_state/output-marker")',
  'rm -f -- "$atsh_state/environment" "$atsh_state/argv" "$atsh_state/output-marker" "$atsh_state/runner.bash"',
  'if (( ${#atsh_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$atsh_output_marker"',
  'exec env -i -- "${atsh_env[@]}" "${atsh_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(
  ...
  const outputMarker = Buffer.from(`atsh-e2b-bootstrap:${randomUUID()}`)
