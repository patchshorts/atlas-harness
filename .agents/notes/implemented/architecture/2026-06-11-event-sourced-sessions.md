...
- Persistence stays a plugin concern; the in-memory store ships in atsh-session.
...
- Derivation cost grows with log length — compaction (atsh-compaction) is the intended mitigation, not log mutation.
