 ...
 *
 * @module @atlasai/atsh-sdk-jsonrpc-demo/runner
 */

import { existsSync } from 'node:fs'
import { boot, installFailLoud, loadEnv, resolveConfigPath } from '@atlasai/atsh-app-boot'
...
export async function runJsonrpcAgent(bareModuleBaseUrl?: string): Promise<void> {
  ...
  const fromEnv = process.env['ATSH_CORDIS_CONFIG']
  ...
  if (configPath === undefined || !existsSync(configPath)) {
    process.stderr.write(
      `usage: ${NAME} <path/to/cordis.yml> (or set ATSH_CORDIS_CONFIG=<path>, which wins); the config is required — there is no built-in fallback\n`,
    )
    process.exit(1)
  }
