...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `atsh-tool-bash` (`bash`), `atsh-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `atsh-tool-subagent` (`subagent`). Example-only tools are excluded.
