...
function clientConfigs(id = '@atlasai/atsh-client-test') {
  return clientBundle(id, ['lib/types/index.js', 'lib/types/invariant.js'])(
    { env: { ATSH_BUILD_FACE: 'client' } },
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@atlasai/atsh-client-test', ['lib/types/index.js'])
    const development = bundle({ env: {} }).find(config => config.platform === 'browser')
    const artifact = bundle({ env: { ATSH_BUILD_FACE: 'client' } })
...
describe('client bundle purity gate', () => {
  ...
  it('leaves platform table entries and non-scoped specifiers alone', () => {
    expect(resolveId('@atlasai/atsh-client-ui-slots')).toBeNull()
    expect(resolveId('@atlasai/atsh-client-web-react')).toBeNull()
    expect(resolveId('@atlasai/atsh-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects retired table entries (web-react/store left the 8-entry seed)', () => {
    expect(() => resolveId('@atlasai/atsh-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@atlasai/atsh-host-apiproxy/api')).toBeNull()
    expect(resolveId('@atlasai/atsh-session/surface')).toBeNull()
    expect(resolveId('@atlasai/atsh-brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@atlasai/atsh-goal/remote')).toBeNull()
    expect(() => resolveId('@atlasai/atsh-goal')).toThrow(/purity/)
    expect(() => resolveId('@atlasai/atsh-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@atlasai/atsh-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @deepseek-ai leak', () => {
    expect(() => resolveId('@atlasai/atsh-agent')).toThrow(/purity/)
    expect(() => resolveId('@atlasai/atsh-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike (the rewrite arm is gone)', () => {
    expect(() => resolveId('@atlasai/atsh-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@atlasai/atsh-client-runtime')).toThrow(/purity/)
    expect(() => resolveId('@atlasai/atsh-client-ui-layout/client')).toThrow(/purity/)
  })

  it('carries exactly one documented temporary exemption: runtime/client (store engine pending rehoming)', () => {
    expect(resolveId('@atlasai/atsh-client-runtime/client')).toBeNull()
    const clientChannels = CLIENT_EXTERNALS.filter(
      entry => entry.startsWith('@deepseek-ai/') && entry.endsWith('/client'))
    expect(clientChannels).toEqual(['@atlasai/atsh-client-runtime/client'])
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@atlasai/atsh-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://dsh.test/plugins/@atlasai/atsh-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@atlasai/atsh-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@atlasai/atsh-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://dsh.test/plugins/@atlasai/atsh-client-connection/client.js.map')
