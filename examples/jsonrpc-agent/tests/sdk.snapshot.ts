 ...
 * the REAL `dsh-jsonrpc-agent` runtime (per `ATSH_EXAMPLE_MODE`) through the
 * REAL `@atlasai/atsh-sdk-client`, drives one turn over stdio JSON-RPC,
 * and pins the SDK `RunResult`, the complete notification stream, and the
 * persisted session logs. Replay serves recorded model
 * responses via `llm-replay` (`cordis.snapshot.yml`); `ATSH_SNAPSHOT=record`
 * re-records against the live API; `ATSH_SNAPSHOT=refresh` replays committed
...
import { resolveExampleLaunch } from '@atlasai/atsh-loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@atlasai/atsh-sdk-client'
...
const mode = process.env.ATSH_SNAPSHOT ?? 'replay'
...
const SCENARIOS: SdkScenario[] = [
  ...
  },
  {
    ...
    environment: { ATSH_SYSTEM_PROMPT: MINIMAL_SYSTEM_PROMPT },
...
async function runScenario(scenario: SdkScenario): Promise<{
  ...
  const env: Record<string, string> = {
    ...
    ATSH_CORDIS_CONFIG: recording
      ? scenario.configs?.live ?? liveConfig
      : scenario.configs?.replay ?? replayConfig,
    ATSH_SESSION_ROOT: sessionsRoot,
    ATSH_CWD: cwd,
    ATSH_SNAPSHOT: mode,
    NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
    ...parentFixture === undefined ? {} : {
      ATSH_SNAPSHOT_FILE: parentFixture,
      ...childFixtures.length > 0 ? { ATSH_SNAPSHOT_CHILD_FILES: childFixtures.join(delimiter) } : {},
    },
