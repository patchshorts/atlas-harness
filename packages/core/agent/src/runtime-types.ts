 ...
 * turn/step boundaries remain `@atlasai/atsh-session` events.
 *
 * @module @atlasai/atsh-agent
 */

import type { Context } from '@deepseek-ai/cordis'
import type { Scoped } from '@atlasai/atsh-scope'
import type { LlmCallConfig, LlmFailure, ResolvedRetryPolicy } from '@atlasai/atsh-llm'
import type { AgentCancelCause, Session, SessionId, UserMessage } from '@atlasai/atsh-session'
export type { AgentCancelCause } from '@atlasai/atsh-session'
import type { Inbox } from './inbox.ts'
import type { InboxTarget } from './types.ts'
import type {} from '@atlasai/atsh-system-prompt'
declare module '@atlasai/atsh-system-prompt' {
...
export interface AgentOptions {
  ...
  maxTokens?: number
  /**
   * Opt-in FR-9 taint gate: when true, the loop drops any final output clause
   * untraceable to the turn's tool-result triples before the message commits.
   * Off by default — general conversational output does not trace to extracted
   * triples, so an on-by-default gate would drop normal answers. Enable only
   * for extraction-then-composition turns whose output must trace to facts.
   */
  enableTaintGate?: boolean
}
...
declare module '@deepseek-ai/cordis' {
  interface Events {
     ...
     * Scope-filtered dispatch (`@atlasai/atsh-scope`): agent-scoped listeners receive only that agent.
     * @mode emit
     */
