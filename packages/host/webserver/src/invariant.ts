/**
 * Package-owned invariant companion for `@atlasai/atsh-host-webserver`.
 * @module @atlasai/atsh-host-webserver/invariant
 */
...
const PACKAGE_NAME = '@atlasai/atsh-host-webserver'
...
const install: InvariantInstaller = (ctx, fail) => {
  ctx.on('internal/plugin', () => {
    ...
    const probe = { kind: 'exact' as const, path: '/__atsh_invariant_probe__', handler: () => {} }
    try {
      server.register(probe)()
      server.register(probe)()
      const upgradeProbe = { path: '/__atsh_invariant_upgrade_probe__', handler: () => {} }
