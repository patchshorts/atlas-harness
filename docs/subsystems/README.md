...
| [atlas-additions.md](atlas-additions.md) | the Atlas additive capability families: `ctx.memoryStore` / `llmRouter` / `llmCache` / `kgraph` / `accounting` / `coordination` / `research` / `factory` service keys and their event scopes |
...
| [subprocess.md](subprocess.md) | the subprocess seam: fully-explicit `SubprocessSpawnSpec`, offset-based output readers, unclassified `SubprocessOutcome`, and the managed `ATSH_*` environment vocabulary |
...
| [client-modules.md](client-modules.md) | the web plugin table: `atsh.client` declarations, `WebBootGraph` wire composition, the bundle route and index tap |
