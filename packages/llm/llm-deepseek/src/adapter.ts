...
import { attributionHeaders, CONTEXT_WINDOW_EXCEEDED_CODE, isContextWindowExceededError, isQuotaExceededError, LlmAdapter, LlmError, ProviderRequestId, QUOTA_EXCEEDED_CODE, ReasoningEffortId } from '@atlasai/atsh-llm'
import type {
  GenerateOptions,
  LlmModelInfo,
  LlmProviderInfo,
...
import { idleWatchdog, timeoutOf } from '@atlasai/atsh-timeout'
import type { AnonymousUserId } from '@atlasai/atsh-anonymous-user-id'
...
export class DeepSeekAdapter extends LlmAdapter {
  ...
  private async * request(
    ...
    // Bound the INITIAL request with the same idle window the stream uses, so a
    // wedged connection (ESTABLISHED socket, no bytes flowing — seen hanging
    // indefinitely against OpenRouter) cannot stall the agent forever. The
    // stream body reads are already limited by `streamIdleTimeoutMs`; the
    // handshake fetch was not, leaving the POST able to hang with no deadline.
    const connectGuard = AbortSignal.timeout(connection.streamIdleTimeoutMs)
    ...
    try {
      response = await fetch(`${connection.baseURL}/chat/completions`, {
        ...
        headers,
        body: payload,
        signal: AbortSignal.any([signal, connectGuard]),
      })
    } catch (error: unknown) {
      // The outer stream distinguishes caller cancellation and watchdog expiry.
      if (signal.aborted) throw error
      // Connect/idle timeout on the initial POST: treat like a transport fault.
      if (connectGuard.aborted) {
        throw new LlmError(
          `DeepSeek API request to ${connection.baseURL} did not complete within ${connection.streamIdleTimeoutMs}ms`,
          'TIMEOUT',
          { cause: error },
        )
      }
