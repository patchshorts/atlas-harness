...
describe('dsh-subagent-dsh-sdk provider', () => {
  ...
  it('scrubs ambient credentials but forwards explicit config env', async () => {
    process.env.ATSH_TEST_AMBIENT_SECRET_KEY = 'leak-me-not'
    try {
      const ctx = await setup({
        FAKE_ECHO_ENV: 'ATSH_TEST_AMBIENT_SECRET_KEY,DEEPSEEK_API_KEY',
      ...
      expect(answer).toContain('ATSH_TEST_AMBIENT_SECRET_KEY=\n')
      expect(answer).toContain('DEEPSEEK_API_KEY=explicit-child-key')
      await run.dispose()
      await ctx.fiber.dispose()
    } finally {
      delete process.env.ATSH_TEST_AMBIENT_SECRET_KEY
    }
  })
