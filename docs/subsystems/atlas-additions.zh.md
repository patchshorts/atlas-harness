...
[English](atlas-additions.md) | 中文

本 fork 在上游 harness 主干（上游 SHA 见 [UPSTREAM.md](../../UPSTREAM.md)）之外挂载的附加能力族。每个能力族是 `packages/` 下的新包，通过 Cordis 声明合并注册自己的 `ctx.*` 服务键，并以附加行方式组合进 bundle patch 层与 standard preset。冻结的上游主干（`vendor/`、`packages/core`、`packages/session`、`packages/compaction`、`packages/preset`）从不修改；冻结文件清单（`FROZEN_FILES.sha256`）证明其逐字节一致。

服务键与包源码的对应关系：

| `ctx` 键 | 包 | 用途 |
|---|---|---|
| `memoryStore` | `packages/memory/memory` | 语义记忆接缝（默认 SQLite，pgvector 配置门控），含 recall/retain/reflect 工具 |
| `llmRouter` | `packages/router/router` | 基于 `llm/stream` waterfall 的能力门控路由，带持久化调用日志 |
| `routerTrainer` | `packages/router/trainer` | 消费 `router/call-logged` 的训练样本队列 |
| `llmCache` | `packages/cache/cache` | LLM 调用面的精确哈希 + 语义缓存两层 |
| `kgraph` | `packages/kgraph/kgraph` | OKR 知识图谱接缝，含会话自动构建器 |
| `accounting` | `packages/accounting/accounting` | 令牌账本、信用发放、工具边界的预算上限 |
| `coordination` | `packages/coordination/coordination` | 基于既有 subagent 注册表的 C2 + 共享状态通道 |
| `research` | `packages/research/research` | xurl 帖子搜索 + arXiv 论文搜索/获取 |
| `factory` | `packages/factory/factory` | 计划契约注册表、确定性 BAR 评审、ralph 工具的角色目标 |
| `skills`（语料） | `packages/skill/skill-corpus` | 208 个扁平化 SKILL.md 文件，经 skill provider 暴露 |

事件作用域 `accounting/*`、`cache/*`、`coordination/*`、`factory/*`、`research/*`、`router/*` 由这些能力族发出；其载荷类型归各包 `src/types.ts` 所有。
