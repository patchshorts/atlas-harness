 ...
 * harness home resolved by {@link resolveDshHome} (`$ATSH_HOME` > `~/.atsh`),
 ...
 * machine: every process sharing one `$ATSH_HOME` reports the same id, and
 ...
 *
 * @module @atlasai/atsh-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `ATSH_HOME`; defaults to `process.env`. */
