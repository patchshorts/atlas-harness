...
const wordmark = readFileSync(resolve(import.meta.dirname, '../public/wordmark.svg'), 'utf8')
  .trim()
  .replace('<svg ', '<svg class="atsh-wordmark" ')
 ...
 */
const siteStyle = `
.atsh-lockup { display: inline-flex; align-items: center; gap: 8px; min-width: 0; }
.atsh-wordmark { display: block; height: 22px; width: auto; color: var(--vp-c-text-1); }
.atsh-tag {
  display: inline-flex;
  align-items: center;
...
function siteTitle(previewTag: string): string {
  return `<span class="atsh-lockup">${wordmark}<span class="atsh-tag">${previewTag}</span></span>`
}
