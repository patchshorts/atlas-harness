...
import { FsError, FsTargetKey } from '@atlasai/atsh-fs'
...
beforeEach(async () => {
  ...
  base = await mkdtemp(join(homedir(), '.atsh-fssbx-'))
...
describe('workspace-write containment', () => {
  ...
  it('a write to the platform temp area lands (parity with the bash runner grant)', async () => {
    const path = join(await mkdtemp(join(tmpdir(), 'atsh-fssbx-tmp-')), 'temp.txt')
