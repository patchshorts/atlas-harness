 ...
 * @module @atlasai/atsh-tool-bash-persistent
 */
...
import type { TerminalReadResult, TerminalSendResult, TerminalSessionId } from '@atlasai/atsh-terminal'
...
const SHELL_PROMPT = '__ATSH_PERSISTENT_BASH_PROMPT__ '
...
function markers(): CommandMarkers {
  ...
  return {
    start: `__ATSH_PERSISTENT_BASH_START_${nonce}__`,
    end: `__ATSH_PERSISTENT_BASH_END_${nonce}:`,
  }
}
...
function wrapCommand(command: string, marker: CommandMarkers): string {
  ...
  return `printf '%s\\n' ${quoteForBash(marker.start)}; eval -- ${quoteForBash(command)}; __atsh_persistent_bash_status=$?; printf '%s%s\\n' ${quoteForBash(marker.end)} "$__atsh_persistent_bash_status"`
}
