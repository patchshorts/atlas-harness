 ...
 * @module @atlasai/atsh-subagent-acp/run
 */
...
import { AssistantOutputFold } from '@atlasai/atsh-subagent'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@atlasai/atsh-subagent'
import type { SubprocessHandle, SubprocessSpawnSpec } from '@atlasai/atsh-subprocess'
...
export interface AcpRunSpec {
   ...
   * Explicit `ATSH_*` entries are deployment-owned facts for the child harness
   * (e.g. `ATSH_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and ATSH_* names
