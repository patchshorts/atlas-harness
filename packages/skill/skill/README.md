...
The registry does not render model guidance or register model-facing tools. [`@atlasai/atsh-tool-skill`](../tool-skill) consumes `ctx.skills` to provide durable session catalogs and the `skill` tool, so providers remain independent of model-facing behavior.
