...
import LlmRuntime, { createUserMessage, CallId, LlmError, StreamChunk  } from '@atlasai/atsh-llm'
import SessionStore, { SessionId, TurnEndReason } from '@atlasai/atsh-session'
...
describe('agent loop', () => {
  ...
  it('throws when maintenance is requested while the agent already has active work', async () => {
    const ctx = await harness(new MockAdapter([]))
    const agent = ctx.agentLoop.create(SessionId('maintenance-double-request'), {
      provider: 'mock',
      model: 'mock',
    })
    const started = Promise.withResolvers<undefined>()
    const finish = Promise.withResolvers<undefined>()
    const first = agent.runMaintenance(async () => {
      started.resolve(undefined)
      await finish.promise
    })
    await started.promise

    // A second maintenance request must fail while the first is still active;
    // the agent never runs two overlapping maintenance phases.
    expect(() =>
      agent.runMaintenance(async () => undefined),
    ).toThrow(`agent "${agent.id}" already has active work`)
  ...
  it('materializes changed runtime context at the history tail without rewriting the system header', async () => {
    ...
    const contextEvents = () => agent.session.events.flatMap(event =>
      event.type === 'user/message'
        ...
        && event.data.source.plugin === '@atlasai/atsh-system-prompt'
        ? [event]
        : [])
  ...
  it('re-emits unchanged runtime context when a surface replacement removed the retained snapshot', async () => {
    ...
    expect(adapter.requests[1]?.messages.some(message =>
      ...
      && message.source.plugin === '@atlasai/atsh-system-prompt')).toBe(true)
  })
  ...
  it('clears compacted runtime context after the active set becomes empty', async () => {
    ...
    const clearing = adapter.requests[1]?.messages.find(message =>
      message.source.kind === 'plugin'
      && message.source.plugin === '@atlasai/atsh-system-prompt')
  ...
  it('does not clear runtime context after an unrelated replacement', async () => {
    ...
    expect(adapter.requests[0]?.messages.some(message =>
      message.source.kind === 'plugin'
      && message.source.plugin === '@atlasai/atsh-system-prompt')).toBe(false)
  })
  ...
  it('replaces a malformed retained runtime-context message with the current complete snapshot', async () => {
    ...
    agent.session.append('user/message', createUserMessage({
      ...
      source: { kind: 'plugin', plugin: '@atlasai/atsh-system-prompt' },
    }), { surfaceOp: 'append' })
